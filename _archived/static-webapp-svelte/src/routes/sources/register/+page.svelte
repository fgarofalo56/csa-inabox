<script lang="ts">
	...
	let ownerTeam = $state('');
	let ownerCostCenter = $state('');
	let tagEntries = $state<{ key: string; value: string }[]>([{ key: '', value: '' }]);
	...
	// Source type options — must match SourceType enum in shared contracts
	const sourceTypes: { value: SourceType; label: string }[] = [
		...
		{ value: 'adls_gen2', label: 'ADLS Gen2' },
	...
	function buildRegistration(): SourceRegistration {
		// Build tags as Record<string, string>
		const tagsRecord: Record<string, string> = {};
		for (const entry of tagEntries) {
			const k = entry.key.trim();
			if (k) tagsRecord[k] = entry.value.trim();
		}
	...
	const stepValid = $derived.by(() => {
		switch (currentStep) {
			case 1:
				return !!(sourceName && sourceType && ownerName && ownerEmail && ownerTeam && domain);
			case 2:
			...
			{#if currentStep === 1}
				<div class="card space-y-6">
					...
					<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
						...
						<div>
							<label for="domain" class="label-field">Domain *</label>
							<input id="domain" type="text" bind:value={domain} required class="input-field mt-1" placeholder="e.g., Finance, HR, Engineering" />
						</div>
					</div>
					...
					<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
						...
						<div>
							...
							<input id="ownerTeam" type="text" bind:value={ownerTeam} required class="input-field mt-1" placeholder="e.g., Data Engineering" />
						</div>
						<div>
							<label for="ownerCostCenter" class="label-field">Cost Center</label>
							<input id="ownerCostCenter" type="text" bind:value={ownerCostCenter} class="input-field mt-1" />
						</div>
					</div>
					...
					<div>
						<label class="label-field">Tags (key-value pairs)</label>
						<div class="mt-1 space-y-2">
							{#each tagEntries as entry, i}
								<div class="flex gap-2">
									...
									<button
										...
										class="rounded-md px-2 text-gray-400 hover:text-red-500"
							...
							{/each}
							<button type="button" onclick={addTagEntry} class="text-sm font-medium text-azure-600 hover:text-azure-700">
								+ Add tag
							</button>
						</div>
					</div>
					...
					<div class="divide-y divide-gray-200 rounded-lg border border-gray-200">
						...
						<div class="grid grid-cols-2 gap-4 p-4">
							...
							<div>
								...
								<p class="text-sm text-gray-900">{domain}</p>
							</div>
						</div>
						<div class="grid grid-cols-2 gap-4 p-4">
							<div>
							...
							<div>
								<p class="text-xs font-medium uppercase text-gray-500">Team</p>
								<p class="text-sm text-gray-900">{ownerTeam}</p>
							</div>
						</div>
						<div class="grid grid-cols-2 gap-4 p-4">
							<div>
