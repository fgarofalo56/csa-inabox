...
| **Workshop** (operational analyst apps)                  | **Built CSA Loom surface: `workshop-app` (Atelier)** — Azure Container Apps + Synapse + ontology binding | Shipped as a Loom item type with `WorkshopAppEditor`: binds an ontology (`/bind-ontology`), runs actions against Synapse SQL (`/run-action`), hosts on Container Apps. Azure-native (no Power Apps/Power BI workspace). See [parity matrix](../fiab/parity/palantir-migration-surfaces.md). | L      | `apps/fiab-console/lib/editors/palantir-editors.tsx → WorkshopAppEditor`, `apps/fiab-console/app/api/items/workshop-app/`, `platform/fiab/bicep/modules/admin-plane/container-platform.bicep`, `docs/fiab/parity/palantir-migration-surfaces.md` |
...
| **Functions** (user-defined TypeScript/Python functions) | Azure Functions + Fabric user data functions + platform functions library                               | Foundry Functions move to Azure Functions (general purpose) or Fabric user data functions (analytic compute)                                        | M      | `csa_platform/functions/aiEnrichment/functions/`, `csa_platform/functions/eventProcessing/`, `csa_platform/functions/validation/`                                                                                               |
| **AIP (AI Platform) — LLM chat + workflows**             | Azure AI Foundry + Azure OpenAI + agentic enrichment patterns                                           | AIP's agent-workflow + grounded-chat surface maps to Azure OpenAI models + AI Foundry orchestration; gap: no first-party chat UX yet (see CSA-0008). **AIP-Logic** (typed LLM functions) is shipped — see [Built CSA Loom migration surfaces](#built-csa-loom-migration-surfaces-audit-t29). | L      | `csa_platform/ai_integration/`, `csa_platform/ai_integration/enrichment/`, `csa_platform/ai_integration/rag/`, `csa_platform/ai_integration/model_serving/`                                                                     |
...
| **Slate** (custom HTML forms)                            | **Built CSA Loom surface: `slate-app`** — Azure Static Web Apps (deterministic bundle codegen)          | Shipped as a Loom item type with `SlateAppEditor`: emits a deterministic Azure Static Web Apps bundle (`/generate`). Azure-native. See [parity matrix](../fiab/parity/palantir-migration-surfaces.md).                          | S      | `apps/fiab-console/lib/editors/palantir-editors.tsx → SlateAppEditor`, `apps/fiab-console/app/api/items/slate-app/`, `docs/fiab/parity/palantir-migration-surfaces.md`                                                          |
...
### Built CSA Loom migration surfaces (audit-T29)

Six of the named Palantir surfaces above are **not docs-only mappings** — each is
a shipped CSA Loom **item type** with a focused editor, real BFF routes, and an
Azure-native backend (no Microsoft Fabric / Power BI workspace required, per
`.claude/rules/no-fabric-dependency.md`). The per-control parity matrix lives at
[`docs/fiab/parity/palantir-migration-surfaces.md`](../fiab/parity/palantir-migration-surfaces.md);
the deep feature-by-feature mapping is in
[`palantir-foundry/feature-mapping-complete.md`](palantir-foundry/feature-mapping-complete.md#audit-t29--built-csa-loom-surfaces-supersedes-the-doc-only-mappings-above).

| Palantir surface | Loom item type (slug) | Editor | Azure-native backend | Bicep |
| --- | --- | --- | --- | --- |
| Workshop | `workshop-app` (Atelier) | `WorkshopAppEditor` | Ontology binding (`/bind-ontology`) + Synapse SQL actions (`/run-action`); hosts on Container Apps | `modules/admin-plane/container-platform.bicep` |
| Slate | `slate-app` | `SlateAppEditor` | Deterministic Azure Static Web Apps bundle codegen (`/generate`) | — (SWA) |
| OSDK | `ontology-sdk` | `OntologySdkEditor` | Typed TS/Python + `dab-config.json` codegen (`/generate`); DAB on Container Apps + APIM (`/publish`) | `modules/admin-plane/dab-runtime.bicep` |
| Apollo | `release-environment` (Shuttle) | `ReleaseEnvironmentEditor` | Real ARM deployment history (`/arm`) + promotions (`/promote`); optional Azure Deployment Environments | `modules/admin-plane/devcenter.bicep` |
| Checks | `health-check` | `HealthCheckEditor` | Real Azure Monitor `scheduledQueryRules` over Log Analytics (`/rule`) | Azure Monitor (orchestrator) |
| AIP-Logic | `aip-logic` (Spindle) | `AipLogicEditor` | Typed LLM function invoked against live Azure OpenAI (`/invoke`) | `modules/admin-plane/ai-foundry.bicep`, `modules/ai/foundry-project.bicep` |

All six appear in the `+ New item` catalog under **Fabric IQ** (preview),
default to the Azure-native path with `LOOM_DEFAULT_FABRIC_WORKSPACE` unset, and
show an honest infra-gate MessageBar (naming the exact env var / RBAC grant)
when the backing Azure resource is not yet provisioned (per `.claude/rules/no-vaporware.md`).
...
- **Platform modules most relevant to a Foundry migration:**
    ...
    - `csa_platform/functions/` — cross-domain Azure Functions, notebook utilities
