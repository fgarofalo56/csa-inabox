...
| 26  | **Health Checks**           | Monitoring views with alerts                          | Azure Monitor + dbt source freshness + Data Activator       | M          | **Built: `health-check` (`HealthCheckEditor`)** — see [audit-T29](#audit-t29--built-csa-loom-surfaces-supersedes-the-doc-only-mappings-above) | Built Loom surface wires real Azure Monitor `scheduledQueryRules` over Log Analytics (`/rule`); see `docs/fiab/parity/palantir-migration-surfaces.md` |
...
| #   | Foundry feature              | Description                                      | Azure equivalent                                           | Complexity | CSA-in-a-Box evidence            | Notes                                                                    |
| --- | ---------------------------- | ------------------------------------------------ | ---------------------------------------------------------- | ---------- | -------------------------------- | ------------------------------------------------------------------------ |
| 44  | **Workshop**                 | Low-code operational app builder (60+ widgets)   | Azure Container Apps + Synapse + ontology binding         | L          | **Built: `workshop-app` / Atelier (`WorkshopAppEditor`)** — see [audit-T29](#audit-t29--built-csa-loom-surfaces-supersedes-the-doc-only-mappings-above) | Built Loom surface binds an ontology (`/bind-ontology`), runs actions against Synapse SQL (`/run-action`), hosts on Container Apps — Azure-native (no Power Apps/Power BI workspace); see `docs/fiab/parity/palantir-migration-surfaces.md` |
| 45  | **Slate**                    | Custom HTML/CSS/JS application framework         | Azure Static Web Apps (deterministic bundle codegen)      | M          | **Built: `slate-app` (`SlateAppEditor`)** — see [audit-T29](#audit-t29--built-csa-loom-surfaces-supersedes-the-doc-only-mappings-above) | Built Loom surface emits a deterministic Azure Static Web Apps bundle (`/generate`) — Azure-native; see `docs/fiab/parity/palantir-migration-surfaces.md` |
...
| 58  | **AIP Logic**                    | No-code visual LLM function builder            | Azure OpenAI (typed LLM function)                     | M          | **Built: `aip-logic` / Spindle (`AipLogicEditor`)** — see [audit-T29](#audit-t29--built-csa-loom-surfaces-supersedes-the-doc-only-mappings-above) | Built Loom surface invokes a typed LLM function against live Azure OpenAI (`/invoke`), not Power Automate/Semantic Kernel; see `docs/fiab/parity/palantir-migration-surfaces.md` |
...
| #   | Foundry feature         | Description                                | Azure equivalent                                          | Complexity | CSA-in-a-Box evidence                 | Notes                                                       |
| --- | ----------------------- | ------------------------------------------ | --------------------------------------------------------- | ---------- | ------------------------------------- | ----------------------------------------------------------- |
| 63  | **OSDK**                | Auto-generated TypeScript/Python/Java SDKs | Microsoft Data API Builder (DAB)                          | M          | **Built: `ontology-sdk` (`OntologySdkEditor`)** — see [audit-T29](#audit-t29--built-csa-loom-surfaces-supersedes-the-doc-only-mappings-above) | Built Loom surface generates typed TS/Python SDKs + `dab-config.json` over an ontology (`/generate`); DAB on Container Apps + APIM — Azure-native (no Graph/Fabric REST dependency); see `docs/fiab/parity/palantir-migration-surfaces.md` |
...
| #   | Foundry feature            | Description                                 | Azure equivalent                                         | Complexity | CSA-in-a-Box evidence                 | Notes                                                    |
| --- | -------------------------- | ------------------------------------------- | -------------------------------------------------------- | ---------- | ------------------------------------- | -------------------------------------------------------- |
| 71  | **Apollo**                 | Continuous delivery, zero-downtime upgrades | ARM deployment history + Azure Deployment Environments   | M          | **Built: `release-environment` / Shuttle (`ReleaseEnvironmentEditor`)** — see [audit-T29](#audit-t29--built-csa-loom-surfaces-supersedes-the-doc-only-mappings-above) | Built Loom surface reads real ARM deployment history (`/arm`) + drives promotions (`/promote`); optional Azure Deployment Environments via `LOOM_DEVCENTER_PROJECT` + `modules/admin-plane/devcenter.bicep`; see `docs/fiab/parity/palantir-migration-surfaces.md` |
