...
| DuckDB tier missing (upstream 503) | Fix-it card naming the missing variable | The N2 DuckDB serving tier is also deployed by default (`admin-plane/main.bicep` → `duckdbTierActive`), so this too should not appear on a normal install. If it does, the `loom-duckdb` image is not in the deployment's ACR — run `full-app-deploy-commercial.yml` (Commercial) or `gov-provision-dataplane-images.yml` (GCC-High / IL5), which builds it and wires `LOOM_DUCKDB_URL` |
