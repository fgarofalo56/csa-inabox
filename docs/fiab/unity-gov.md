...
| **Persistence** | ✅ `postgres` — durable, backed up, Entra-only (no DB credential anywhere), multi-writer | ❌ `h2-ephemeral` — an EmptyDir that **loses every catalog object on container restart**, forced `maxReplicas: 1` | `loomBackends.postgresStores = 'enabled'` once PG flexible-server quota is confirmed in usgovvirginia. Postgres **is** available in US Gov Virginia / Arizona / Texas per Learn — this is a subscription quota question, not a service gap. Note that `postgresStores` gates the Unity metastore **and** the DuckLake catalog store as one unit, so flipping it lights both. It does **not** control the Weave/pgvector server, which has its own narrower `loomBackends.weavePostgres` lever (#3711) — see the row below |
| **Weave / pgvector server** (adjacent, easy to confuse with the row above) | ✅ deployed by `admin-plane/main.bicep` | ⚠️ skipped while `postgresQuotaAvailable=false`, so the Ontology editor honest-gates on `LOOM_WEAVE_PG_FQDN` | `observabilityConfig.backendOverrides = { weavePostgres: 'enabled' }` re-enables **just this server** without dragging in the OSS Airflow host (whose image is an unmirrored `docker.io` pull) or the DuckLake/Unity stores. Available on **tenant, single-sub and multi-sub** — `deployAdminPlane = effectiveTopology != 'dlz-attach'`, so the admin plane is present and receives `loomBackends`, and the server lands in the **admin RG** rather than a DLZ RG. On **`dlz-attach` only**, the admin plane is skipped and the root `postgresQuotaAvailable` param is the sole lever, which un-gates Airflow and DuckLake/Unity with it |
