...
| 8 | Indexes | ⚠️ honest-gate | "coming" row — author via `CREATE INDEX …` in the Query tab until inline authoring lands |
| 9 | Keys & constraints | ⚠️ honest-gate | "coming" row — author via `ALTER TABLE …` in the Query tab |
| 10 | Data editing (edit rows) | ⚠️ honest-gate | "coming" row — use the Query tab for DML |
...
Zero ❌, zero fake/stub banners. The honest-gate rows (8–11) name the exact
T-SQL path and render as `Badge color="warning"` "coming" rows with a tooltip,
per `ui-parity.md`.
...
binds `objectId` as `@p0`. **DROP** never interpolates the caller's name: the
route receives an integer `object_id`, the client looks up the matching
`schema.name` in `sys.objects`/`sys.table_types`, and emits DDL built only from
the **catalog-returned, bracket-quoted** identifier (`]` doubled). A bad/
non-matching id yields a 404, not a drop.

## Files

- `apps/fiab-console/lib/azure/sql-objects-client.ts` — catalog enumeration + `sqlConfigGate()` + `dropObject()`
- `apps/fiab-console/lib/azure/azure-sql-client.ts` — `executeParameterized()` (added; reuses the existing TDS+AAD pool)
- `apps/fiab-console/lib/azure/fabric-client.ts` — `getFabricSqlDatabaseConnection()` (added)
- `apps/fiab-console/app/api/sqldb/_shared.ts` — session guard + item-scoped connection resolution + honest gate
- `apps/fiab-console/app/api/sqldb/{tables,views,procedures,functions,table-types,schemas,columns}/route.ts`
- `apps/fiab-console/lib/components/sqldb/sqldb-tree.tsx` — the navigator
- `apps/fiab-console/lib/editors/sql-database-editor.tsx` — wires the tree into the Tables tab

## Verification

`pnpm build` exit 0 (all 7 `/api/sqldb/*` routes registered). Live functional
verification requires a deployment with a reachable SQL server (UAMI as Entra
admin) bound to a Fabric SQL database item, or
...
path returning `503 { code: "not_configured" }`).
