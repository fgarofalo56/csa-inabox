...
| 8c | Delete agent | ✅ | **"Delete agent"** action on the Build-tab SaveBar → `DELETE /api/items/data-agent/[id]` (owner-scoped via `deleteOwnedItem`); confirms, removes the item + its search doc, returns to `/workspaces`. |
