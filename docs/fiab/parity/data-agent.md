...
| 7 | Test / chat pane | ✅ (live, executes) / ⚠️ (infra-gate) | Test tab runs against the **live AOAI deployment** the cross-item Copilot resolves. **The generated per-source query is now actually EXECUTED read-only on the Azure-native backend** (warehouse → Synapse dedicated SQL/TDS, lakehouse → Synapse serverless SQL, KQL → ADX) and the model is **re-prompted with the real rows** so the answer is grounded on actual data — not just a query it "would" run. Each tool shows a `✓ ran · N rows` badge + a result preview, or an honest ⚠ gate per unreachable source (semantic-model DAX needs an XMLA endpoint → gated). Queries are hard-gated read-only (SELECT/WITH only; KQL mgmt/ingest blocked) and capped to 25 rows. No model deployed → honest 503 MessageBar naming `LOOM_AOAI_ENDPOINT` / `LOOM_AOAI_DEPLOYMENT` + the Foundry-hub "Deploy gpt-4o-mini" action. Logic in `lib/azure/data-agent-execute.ts`. |
