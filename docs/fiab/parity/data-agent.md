...
| 7 | Test / chat pane | ✅ (live) / ⚠️ (infra-gate) | Test tab streams against the **live AOAI deployment** the cross-item Copilot resolves. No model deployed → honest 503 MessageBar naming `LOOM_AOAI_ENDPOINT` / `LOOM_AOAI_DEPLOYMENT` + the Foundry-hub "Deploy gpt-4o-mini" action. |
...
| Test chat turn | `POST /api/items/data-agent/[id]/chat` | Loads Cosmos config → `chatGrounded()` → live Azure OpenAI chat/completions on the resolved Foundry-hub deployment. 503 + remediation when no model. |
