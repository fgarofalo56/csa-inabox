...
| E10 | built ✅ | **Row-level security is built** (parity-doc was stale). Navigator has an inline **RLS dialog** (`adx-database-tree.tsx:301-377`, `:755`) plus a parent-owned editor path, backed by `GET/POST /api/adx/rls` → real `.show`/`.alter table ["T"] policy row_level_security enable\|disable "<query>"` with a read-back receipt (`app/api/adx/rls/route.ts`, `showTableRlsPolicy`). Predicate authoring via `kusto-rls-predicate.ts`. |
| E11 | built ✅ | **External tables are built** (parity-doc was stale). Navigator has a structured (no-raw-KQL) **external-table create dialog** + list (`adx-database-tree.tsx:274`, `:697`), backed by `/api/adx/external-tables` → real `.show external tables` / `.create-or-alter external table … kind=delta\|storage` (`kusto-client.ts:967 listExternalTables`, `:998 createExternalDeltaTable`, `:1038 query_acceleration`). |
...
| E13 | built ✅ | Continuous export **list + create + drop** are all built (parity-doc was stale on create/drop). `GET/POST/DELETE /api/adx/continuous-exports` → real `.show continuous-exports` / `.create-or-alter continuous-export NAME over (src) to table ext` / `.drop continuous-export` (`app/api/adx/continuous-exports/route.ts`), wired in the navigator (`adx-database-tree.tsx:26,257`). |
...
| F8 | built ✅ | **Database + table permission management is built** (parity-doc was stale). `adx-rbac-panel.tsx` — a "Manage authorized principals" dialog (Fluent v9 + Loom tokens) — lists/adds/drops **database** (admins/users/viewers/unrestrictedviewers/ingestors/monitors) and **table** (admins/ingestors) principals through the real `POST /api/adx/principals` → `.add`/`.drop database\|table <role> ('<fqn>')` (structured params, no raw KQL). |
