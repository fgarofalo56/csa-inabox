...
| E2 | View **effective** (inherited) grants | ✅ built (dbx) · ⚠️ Loom-resolved (OSS) | "effective" toggle → `GET /grants?effective=true`. Databricks: native `/effective-permissions`. Loom Unity / OSS: the BFF's own inheritance walk (LU-4, `uc-effective-permissions.ts`) — containment chain, `ALL PRIVILEGES` expanded, **non-inheriting ownership** (ancestor owner → `MANAGE` only, per Learn), `USE CATALOG` / `USE SCHEMA` prerequisites evaluated, transitive group membership. **Not asserted to be byte-identical to the Databricks endpoint** — the OSS vocabulary has no `MANAGE`/`BROWSE`/`APPLY TAG`, and every such narrowing comes back as a warning. The narrowing is **unconditional**: a privilege recorded DIRECTLY on the target is filtered exactly like an inherited one, so a grant the backend cannot enforce is never echoed back (it becomes a `warnings[]` line naming it). A live side-by-side against the Databricks endpoint has NOT been run — tracked in issue #2623. |
...
| E2c | **Authorization + audit on the principal probe** | ✅ built | Probing a principal other than yourself resolves that principal's Entra group membership with the Console platform identity, so it is restricted to a tenant admin or to self (`lib/auth/uc-principal-probe.ts`, 403 `principal_probe_forbidden`). Every effective query — allowed AND denied — writes a `uc-access-review` row to the Cosmos `audit-log` container (`lib/azure/uc-access-review-audit.ts`) **and** fans out through `emitAuditEvent` to LoomAudit_CL / the webhook stream. The Cosmos row is scoped on the Entra `tid`, which the Admin → Audit Logs reader now selects on (it previously matched the viewer's `oid` only, so no row was ever retrievable). Visual confirmation in the Admin reader is still tracked in #2623. |
| E2d | **Usage-prerequisite verdicts are never asserted un-verified** | ✅ built | `USE CATALOG` / `USE SCHEMA` report `missing` only when the principal's transitive group closure was actually resolved, or when NOBODY holds the prerequisite (so no membership could supply it). In the unfiltered enumeration, and on the filtered path with Microsoft Graph unavailable, a prerequisite another principal holds reports `unknown` — the pane shows a warning-tinted "unknown USE_CATALOG on CATALOG main" chip rather than a red BLOCKED badge it cannot stand behind. |
