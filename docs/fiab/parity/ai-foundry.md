...
| C3 | **Connections** — list + **create / edit / delete** (AOAI, AI Search, Blob, etc.) | ✅ built (AIF-9) — Connections tab has ＋New connection (typed create-dialog: category picker drives credential fields; Entra ID / managed-identity default = no secret; key-based modes accept ONLY a Key Vault secret identifier) + per-row **Edit** (same typed dialog, name/category locked, secrets re-entered as a KV reference) + per-row Delete (confirm). Real `PUT`/`DELETE` on the workspace connection; edit is a 404-guarded create-or-update `PUT`; `buildConnectionBody` rejects any raw secret on create AND edit. | `GET/POST/PATCH/DELETE /api/foundry/connections`, `foundry-connections-client.ts` |
