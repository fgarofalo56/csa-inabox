...
| `lakehouse/paths/route.ts` | GET | session-only |  | AAS, ADLS, ARM, Azure SQL, Azure Storage, Managed Identity, Power BI |
...
| `apps/fiab-console/app/api/items/_lib/item-crud.ts` | `accessOptsFor`, `createOwnedItem`, `deleteOwnedItem`, `domainScopeFor`, `listAllOwnedItems`, `listOwnedItems`, `loadOwnedItem`, `loadRecycledItem`, `purgeRecycledItem`, `restoreOwnedItem`, `softDeleteOwnedItem`, `updateOwnedItem` |
