...
| F14 | SQL analytics endpoint — T-SQL editor | Synapse Serverless SQL pool (`OPENROWSET`, external tables) | Monaco SQL editor, run, result grid, IntelliSense, view/proc/UDF CRUD | Synapse Comm+Gov | ✅ built | dedicated `synapse-serverless-sql-editor`: object explorer + column IntelliSense + view/proc/iTVF CRUD + Messages pane (T15 done) |
