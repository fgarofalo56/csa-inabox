...
| W2 | Create / edit / delete warehouse (size, type, auto-stop, min/max clusters, Photon, channel, tags, UC, $/hr) | Databricks `POST /warehouses`, `/edit`, `DELETE`; Synapse pool ARM | Create + Edit dialogs (all fields) | Comm/GCC full; Gov = pool SKU/scale | ✅ edit/scale built; **create/delete missing** | **T1** create + delete dialogs + $/hr estimate |
