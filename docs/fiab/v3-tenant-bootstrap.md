...
(`LOOM_ADF_NAME`, `LOOM_MIRROR_SOURCE_LINKED_SERVICE`). The one-time tenant setup
is the *source-side* prerequisite the wizard surfaces as an honest gate. The
**Bronze sink is not a prerequisite at all**: Loom creates
`loom_mirror_sink_adls` itself from `LOOM_BRONZE_URL` with factory
managed-identity auth (`auto-bind-by-default.md` §5), so
`LOOM_MIRROR_ADLS_LINKED_SERVICE` is a brownfield override for estates that hold
a hand-tuned `AzureBlobFS` linked service, never something to set to make
mirroring work.
...
| **Snowflake** | account/host, database, Key Vault connection holding the Snowflake credential, sync mode | **None.** Loom builds the ADF `SnowflakeV2` linked service from the Loom Connection and the ADLS Bronze sink from `LOOM_BRONZE_URL`; the factory MI reads the credential from Key Vault **by reference** (the `adf-keyvault-rbac.bicep` "Key Vault Secrets User" grant, made by the deploy). Snowflake mirrors via an ADF **Copy** pipeline (delete-then-copy full refresh → Bronze Parquet) + a schedule trigger on `LOOM_MIRROR_COPY_CADENCE` (default `1h`). `LOOM_MIRROR_SNOWFLAKE_LINKED_SERVICE` remains a brownfield override. |
...
|---|---|---|
| **Azure SQL DB / MI / SQL Server** | SQL **Change Tracking** delta (CHANGETABLE), or ADF **CDC** resource → Bronze Delta when `LOOM_ADF_NAME` + `LOOM_MIRROR_SOURCE_LINKED_SERVICE` are set (`continuous`). | Watermark = `CHANGE_TRACKING_CURRENT_VERSION`. The Bronze sink linked service is auto-bound. |
...
| **Snowflake** | ADF **Copy** runtime — delete-then-copy full refresh → Bronze Parquet, re-run on the `LOOM_MIRROR_COPY_CADENCE` schedule trigger (`incremental`/`continuous`). `snapshot` = one-time load, no trigger. | Needs only the factory (`LOOM_ADF_NAME`) and the lake (`LOOM_BRONZE_URL`) — both linked services are auto-bound. |

Env vars, all default-empty / `1h`, threaded root `main.bicep` → `admin-plane`:
`LOOM_MIRROR_COPY_CADENCE` is the only knob you would normally touch.
`LOOM_MIRROR_SOURCE_LINKED_SERVICE` is a real prerequisite for the ADF **CDC**
engine (an arbitrary relational connector whose credentials and network path are
yours). `LOOM_MIRROR_SNOWFLAKE_LINKED_SERVICE` and
`LOOM_MIRROR_ADLS_LINKED_SERVICE` are **brownfield overrides** — leave them empty
and Loom binds both itself.

Until a genuinely operator-supplied prerequisite is configured, **Start** returns
a precise gate naming the exact env var + grants (no fake "Running"), and
**Verify** returns a source-specific reachability note (the service-account /
gateway is validated when the copy first runs). All wizard fields render +
persist regardless, so the surface is never empty.
