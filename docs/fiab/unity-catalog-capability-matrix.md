...
| 19 | **System tables** (audit / billing / query history / data classification) | ✅ | ❌ → LU-3 **records** the access trail (BFF choke point → `_auditLog` + `LoomAudit_CL`, readable with `unityAuditKql()` in Sentinel); the in-product reader + pane are a follow-up (no G1 E2E yet). Billing + query history have no Loom Unity equivalent (no DBU meter, no query engine) | `system-tables/route.ts`, `unity-audit.ts` (choke point), `data-classification/route.ts` | SQL-warehouse audit dialogs · Log Analytics / Sentinel | ✅ dbx / ❌ oss (trail written, reader pending) |
