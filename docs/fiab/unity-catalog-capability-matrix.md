...
| 19 | **System tables** (audit / billing / query history / data classification) | ✅ | ⚠️ **access audit built Loom-native (LU-3)**; billing + query history have no Loom Unity equivalent (no DBU meter, no query engine) — the pane names /admin/finops and the engine surfaces | `system-tables/route.ts` (backend-aware), `unity-audit.ts` (choke point + reader), `data-classification/route.ts` | /catalog/unity → System tables · SQL-warehouse audit dialogs | ✅ dbx / ⚠️ oss (audit real, rest named) |
