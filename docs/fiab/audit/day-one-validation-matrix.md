...
| **Warehouse — compute status badge** | **FAIL** | **2026-06-17:** editor shows *"Warehouse compute is Unknown — not Online — resume on the Dedicated SQL pool editor"* while `loompool` is actually **Online** (verified `az synapse sql pool list` → status Online). Status probe doesn't reflect the real ARM pool state → blocks the Run affordance with a false "paused" gate. Day-one bug; fix delegated. |
| **Warehouse — Alerts dialog auto-opens & blocks query editor** | **FAIL** | **2026-06-17:** on fresh editor load the `WarehouseAlerts` dialog (badge *"Databricks SQL Alerts"*) renders **open**, centered over the query editor, and its **Close** button does not dismiss it (reproduced across reload). `alertsOpen` defaults `false` in phase3-editors — mount-state bug. UI overlap (no-vaporware C-grade). Fix delegated. |
