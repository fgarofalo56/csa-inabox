...
| **Lakehouse Inspector — serverless SQL endpoint (T-SQL query)** | **FAIL** | **v0.11:** `synapse-serverless-sql-pool` step = remediation: *"Synapse Serverless rejected CREATE EXTERNAL DATA SOURCE: the specified credential cannot be found or the user does not have permission."* Honest gate w/ precise fix: grant Console UAMI **CONTROL** on `loom_lakehouse` DB + Synapse system MSI **Storage Blob Data Contributor** on the DLZ ADLS (`landing-zone/synapse-storage-rbac.bicep`). **This is P0 #1** — fix = live RBAC grant + bicep. Data is loaded + browsable; only the T-SQL endpoint is gated. |
