...
| **Warehouse — compute status badge** | **FAIL → fix in PR #1445** | **2026-06-17:** editor showed *"compute is Unknown / not Online"* while `loompool` is **Online**. Root cause (PR #1445): the ARM status probe (`synapse-pool-arm.ts`) mixed `LOOM_SYNAPSE_SUB` (admin sub) with `LOOM_DLZ_RG` → 404 → threw → "Unknown" (TDS path uses only the workspace name, so queries still worked). Fix = self-healing coordinate resolution + **Azure Resource Graph discovery by workspace name** across readable subs, and the schema route now distinguishes a real Paused pool (409 gate) from a probe failure (502). Same systemic dlz-attach admin-vs-DLZ coordinate bug as the EH gap. |
| **Warehouse — Alerts dialog auto-opens & blocks query editor** | **FAIL → fix in PR #1445** | **2026-06-17:** on fresh load the `WarehouseAlerts` list dialog rendered **open** over the query editor and **Close** didn't dismiss it. Root cause (PR #1445): the "New alert" editor `Dialog` was nested **inside** the list Dialog's `DialogSurface` → the nested modal's focus-trap/portal kept the parent mounted. Fix = render the editor Dialog as a **sibling** (fixes both phase3 + databricks warehouse editors). |
