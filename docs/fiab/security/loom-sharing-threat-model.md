...
> ## THIS IS THE DATA-PLANE HALF — open findings, stated up front
>
> The control plane (share / recipient CRUD, grants, the suspend kill-switch, the rendered
> server manifest, the audit rows, the tenant-admin redaction, and the `loom-sharing`
> Container App) landed separately. **This change adds the recipient-facing endpoint**
> `/api/delta-sharing/*` — the only code that calls the reference server, and the only path on
> which estate data leaves the boundary.
>
> It was split out because four consecutive review rounds each produced a new exploitable
> finding on this one route (path traversal → audience pin → spoofable throttle key + JWKS
> amplifier → case-colliding share name). Reviewers should treat the rows below as claims to
> re-test, not as settled.
>
> **Open, NOT closed by this change:**
>
> 1. **The endpoint-wide rate limit keys on `trustedClientIp`.** Behind Front Door that resolves
>    to the EDGE NODE, so it is closer to a global ceiling than a per-caller one — one noisy
>    recipient can consume the budget for everyone, and a distributed caller is not bounded
>    per-caller at all. It must key on the VERIFIED principal for authenticated callers and keep
>    the source-derived ceiling only for the anonymous path.
> 2. **No live E2E.** No `delta_sharing` client has ever been run against a booted server, and
>    the image has never been built. Per `no-vaporware.md` / ux-baseline G1 this is not an
>    A-grade claim. Owed before merge: mint a recipient token, `load_as_pandas` the granted
>    share, confirm 403 on another recipient's.
> 3. **`deltaio/delta-sharing-server:0.7.8` base-image CVEs** — §4.1. Internal-only ingress is
>    the mitigation; re-basing onto a patched Corretto 8 runtime is the tracked follow-up.
> 4. **No per-recipient rate limit** (distinct from 1: that is about the KEY, this is about
>    having a per-recipient budget at all).
> 5. **The narrower contract is not yet built.** The reviewer's ask was that the proxy be
>    rebuilt behind a contract where the resolver receives an already-authorized `LoomShare`
>    record and a resolved `SharedTable` and cannot look either up itself. Round 3 moved most of
>    the way there (`upstreamTablePath` builds only from records); what remains is making the
>    share LOOKUP unreachable from the proxy so a future edit cannot reintroduce the
>    authorize-one-record / read-another divergence that round 4 found.
>
> **Closed and carried in from the control plane:** name canonicalisation. Share and recipient
> names are case-insensitive identifiers, canonical at comparison AND at storage
> (`lib/sharing/model.ts` `canonicalSharingName`), proven by
> `lib/sharing/__tests__/name-collision.test.ts` and
> `app/api/delta-sharing/__tests__/case-collision.test.ts` — both of which FAIL against the
> unfixed code.
