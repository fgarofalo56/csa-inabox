...
|---|---|---|---|---|
| S1 | **Spoofing** a recipient | Forge / replay a bearer to `/api/delta-sharing/*` | RS256 JWKS signature verification, issuer pinned to the estate tenant, audience pinned, `exp`/`nbf` with ±5 min skew, `alg` pinned (no `none`/HMAC downgrade). `lib/azure/entra-bearer-verify.ts`; tests sign with a foreign key, a foreign issuer, a foreign audience, and an expired token and assert 401 | Token theft at the recipient. Mitigated by short Entra token lifetimes + revocation in Entra |
...
| R1 | **Repudiation** of a data pull | "We never downloaded that" | Every protocol call writes an `audit-log` row (`kind:'delta-sharing'`) with the recipient, the Entra principal, the share/schema/table, and the outcome — **including denials**, which are the interesting rows | Audit write is best-effort (a Cosmos hiccup must not become a data-availability incident) |
...
| E2 | Elevation — VNet workload → all shares | Any pod in the CAE calls the sharing server directly | Internal ingress + `consoleAllowedCidrs` IP allow-list pinning to the Console subnet, + the bearer it does not hold | With `consoleAllowedCidrs` empty, VNet reachability + a leaked bearer would suffice — set it |
...
| No estate tenant / no pinned audience | Recipient endpoint returns **503**, never 401 — our misconfiguration is not the caller's authentication failure |
