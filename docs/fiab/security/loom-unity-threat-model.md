...
| S-4 | **Catalog persistence** (H2 file / Postgres) | no ingress | file mount, or Postgres AAD-only (LU-1) | `loom-unity-app.bicep` |
...
| **D**enial-of-service | Hostile in-VNet caller floods the single replica | Network narrowing (above) removes the anonymous flood path; the catalog is not internet-reachable in any posture. Replica scale-out is LU-1's Postgres work (H2 is single-writer). |
