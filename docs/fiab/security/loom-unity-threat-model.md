...
| F-8 | The catalog is not in the push-button deploy at all, so the secured posture depends on an operator running a documented command correctly. | HIGH | **CLOSED by #2681.** `admin-plane/main.bicep` now invokes `compute/loom-unity-app.bicep` + `data-plane/loom-unity-postgres.bicep` DEFAULT-ON on every boundary, with `authMode: 'entra'` as a LITERAL at the call site (no caller-supplied off-switch reaches the module from the orchestrator) and `consolePrincipalId` bound so #2974's SCIM auto-bind makes the enforcing catalog usable. Both modules left the `check-bicep-sync` orphan allowlist and all four `LOOM_UNITY_*` vars left the `check-env-sync` allowlist, so a revert now fails CI. §1.2's deferral reason — "it would flip the live Gov catalog's authorization mode, which today means breaking it" — was resolved by #2679 (token exchange) + #2974 (auto-bind + the `unitycatalog-server:0.5.1` overlay that fixes upstream's authorization-on 500 for `GET /permissions`). **Residual, stated plainly:** a Gov deploy ADOPTS the running catalog and repoints it at `loom-unity:$LOOM_UNITY_TAG`. The re-armed image preflight proves that tag EXISTS; it cannot prove it is CURRENT. An adopted image built before the 0.5.1 overlay would enforce and 500. Run `gov-build-images.yml` (`apps=loom-unity`) before the first Gov deploy that carries this change. |
