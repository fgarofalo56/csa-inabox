...
- `lib/azure/databricks-client.ts` — AAD-auth REST against `adb-<workspace-id>.19.azuredatabricks.net`. listWarehouses / getWarehouse / start / stop / executeStatement (polls SUCCEEDED).
...
1. **Databricks SCIM bootstrap** (Task #16): a workspace admin must visit `https://adb-<workspace-id>.19.azuredatabricks.net` once to bootstrap, then POST UAMI as workspace SP:
   ```bash
