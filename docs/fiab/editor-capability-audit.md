# CSA Loom — Editor Capability Audit (v3.15 / v3.16)
...
### Eventstream — **GATED** (needs Event Hubs namespace deployed)
**Backing:** Designed for Event Hubs + Stream Analytics, but the Event Hubs namespace isn't provisioned in this env. Editor renders + persists source/dest config to Cosmos; Run fails honestly with 404 until namespace is deployed.
**Bicep delta needed:** add `Microsoft.EventHub/namespaces` + grant UAMI `Azure Event Hubs Data Owner`.
...
**Backing:** Cosmos persist; Cypher translated to ADX `make-graph` (works when ADX is available); GQL still parsing-only.
...
1. Event Hubs namespace for eventstream / eventhouse Run
