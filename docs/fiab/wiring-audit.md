...
| `cypher-graph`, `gql-graph` | `/api/items/kql-database/{id}/query` (Cypher routed to ADX `make-graph`) | **🟢 LOCAL-AZURE** | Works — verified UAT |
| `vector-store` | `/api/items/vector-store` (Cosmos state) | **🟡 LOOM-COSMOS-ONLY** | Spec is saved, similarity test not wired (deferred) |
...
| `cross-item-copilot` | `/api/copilot/orchestrate`, `/api/copilot/sessions`, `/api/copilot/tools` | **🟢 LOCAL-AZURE** | Works — Loom's own orchestrator + Azure OpenAI |
...
| `data-product-template`, `data-product-instance` | `/api/items/data-product-template/{slug}/instantiate` | **🟢 LOCAL-AZURE** | Instantiate creates real Loom items from a template — works (this is what the Apps catalog does) |
