...
| 2 | `admin-plane/adx-cluster.bicep` SKU (default from `admin-plane/main.bicep` `adxSkuName`) | `Standard_E2a_v4 is not supported in usgovvirginia` | `effectiveAdxSkuName` in `main.bicep`: when a Gov boundary is left on ANY `Dev(No SLA)_*` default, substitute `Standard_E8as_v5+1TB_PS`. **Corrected 2026-08-27 (#4141).** This row previously said the substitute was "the LIVE-verified Gov Dev SKU `Dev(No SLA)_Standard_D11_v2` (tier-preserving 1:1 swap — Basic tier, 1 node)". That is no longer true and could not be made true: Azure Government offers **no Dev(No SLA)/Basic-tier ADX SKU at all**. Measured in-boundary 2026-08-27 (`gov-adx-sku-probe` run 33120409251) — `usgovvirginia` offers 9 SKUs and `usgovarizona` 14, **all Standard tier**, none restricted, and `D11_v2` is in neither. The `Standard_E2ads_v5`/`Standard_E4ads_v5` pair this row also called "LIVE-verified Gov" is likewise absent from both regions. There is no tier to preserve, so the swap is **not** 1:1: Standard tier is a documented 2-node minimum, i.e. two instances where the Dev node was one. | Byte-identical on Commercial/GCC (Gov-only branch; explicit non-Dev SKUs pass through). Gov: SKU + tier + capacity all change — see #4072, #4141, #4142. |
...
### Round 7 (2026-07-10, usgovvirginia)

A re-deploy of `params/gcc-high.bicepparam` into **US Gov Virginia** after the
serialized-PE fix (#1891) let the deployment reach modules that earlier rounds
never got to (`ai-defense`, `apim`, `agent-foundry`), unmasking three template
deltas + one stuck-resource case. #1–#2 are Gov-only branches (Commercial
byte-identical); #3 is a real APIM-in-VNet requirement fixed for **both** clouds;
#4 is not a template defect. VPN (round-6 #4) re-verified on `main` — the
universal audience `c632b3df-…` is present; the round-7 log's VPN error was a
stale local tree, no regression. Grounded in Microsoft Learn:
[relatedAnalyticRuleIds — list of resource ids of Analytic rules](https://learn.microsoft.com/dotnet/api/azure.resourcemanager.securityinsights.models.automationrulepropertyconditionsupportedproperty.incidentrelatedanalyticruleids),
[Supported logs for Microsoft.ApiManagement/service](https://learn.microsoft.com/azure/azure-monitor/reference/supported-logs/microsoft-apimanagement-service-logs),
and [API Management with an internal VNet — NSG requirements](https://learn.microsoft.com/azure/api-management/api-management-using-with-internal-vnet).

| # | Module | Live Gov error | Fix | Commercial impact |
|---|--------|----------------|-----|-------------------|
| 1 | `ai-defense` (`admin-plane/ai-defense.bicep` Sentinel `automationRules`) | `BadRequest: Invalid Analytic rule id 'csa-loom-ai-prompt-injection'` | The `IncidentRelatedAnalyticRuleIds` condition matches the incident's `relatedAnalyticRuleIds`, which Sentinel exposes as **full analytic-rule resource IDs**, not friendly names (Learn). The `propertyValues` used the bare rule names, so the `automationRules` PUT failed preflight. Build the full extension-resource IDs of the two scheduled rules created in `monitoring.bicep` on the same workspace (`${law.id}/providers/Microsoft.SecurityInsights/alertRules/<name>`) and pass those. | Not deployed in Commercial (`defenderForAIEnabled=true` there → whole `ai-defense` + Sentinel-rule path is `!defenderForAIEnabled`-gated), so no impact; the fix is cloud-agnostic and correct regardless. |
| 2 | `apim` (`admin-plane/apim.bicep` `diag-loom-stdz` diagnostic setting) | `BadRequest: Category 'WebSocketConnectionLogs' is not supported.` | `WebSocketConnectionLogs` is a newer APIM resource-log category not yet exposed by Azure Government APIM. `apim.bicep` gains a `boundary` param; the diag `logs` array drops `WebSocketConnectionLogs` in GCC-High/IL5 (via `isSovereign` + `concat`) while keeping the always-supported `GatewayLogs` + `DeveloperPortalAuditLogs`. `boundary` threaded from `main.bicep`. | Byte-identical (Commercial/GCC keep all three categories). |
| 3 | `apim` (`admin-plane/network.bicep` `nsg-snet-apim`, consumed by `apim.bicep` internal-VNet service) | `ManagementApiRequestFailed: Failed to connect to management endpoint apim-…management.azure-api.us:3443 for a service deployed in a Virtual Network` | An internal-VNet APIM cannot provision unless its subnet NSG allows the control-plane management endpoint on **TCP 3443 from the `ApiManagement` service tag** — the generic `DenyInternetInbound` (priority 4000) otherwise drops it because that service tag resolves to public IPs. Added `AllowApimManagementInbound` (3443 from `ApiManagement`, priority 200) + `AllowApimLoadBalancerInbound` (6390 from `AzureLoadBalancer`, priority 210) to the `snet-apim` NSG only (via a name check in the NSG loop). This is a real APIM-in-VNet requirement for **both** clouds, so it is applied **unconditionally**. | Improved-only — same latent from-scratch APIM-in-VNet failure fixed in Commercial (service-tag rules are cloud-agnostic). |
| 4 | `agent-foundry` (`modules/ai/foundry-project.bicep` AIServices account `aifndry-loom-usgovvirginia`) | `AccountProvisioningStateInvalid: Account …/aifndry-loom-usgovvirginia in state Accepted` | **Not a template defect.** The account bicep is byte-identical to the working Commercial deploy (only `publicNetworkAccess`/PE differ + round-6 Gov-valid model versions), so the child project/deployment PUT hit an account **wedged in `Accepted`** (async provisioning that never converged) left by an earlier round's retry. A stuck-in-`Accepted` CognitiveServices account can't be repaired by re-deploy — it must be **deleted out-of-band** and recreated on the next run. Flagged for operator deletion; no bicep change. | No impact (Gov-only stuck resource). |
