...
```mermaid
graph LR
    A[Starter: USB Mic] --> B[Add: Audio Interface]
    B --> C[Upgrade: XLR Mic]
    C --> D[Expand: Multi-track]

    E[Basic: Webcam] --> F[Add: Lighting]
    F --> G[Upgrade: DSLR]
    G --> H[Expand: Multi-cam]

    I[Free: OBS] --> J[Add: Camtasia]
    J --> K[Upgrade: Premiere]
    K --> L[Expand: Full Adobe Suite]
```
