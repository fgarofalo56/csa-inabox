			...
			{#each products as product}
				...
				<div class="card flex flex-col transition-shadow hover:shadow-md">
					...
					<div class="mb-4 grid grid-cols-3 gap-2 rounded-lg bg-gray-50 p-3">
						...
						<div class="text-center">
							<p class="text-lg font-bold text-gray-900">{formatFreshness(product.sla_freshness_minutes)}</p>
							<p class="text-xs text-gray-500">SLA</p>
						</div>
						<div class="text-center">
							<p class="text-lg font-bold text-gray-900">{formatNumber(product.row_count)}</p>
					...
					{#if product.tags.length > 0}
						<div class="mb-4 flex flex-wrap gap-1">
							...
							{#if product.tags.length > 4}
								<span class="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-500">+{product.tags.length - 4}</span>
							{/if}
						</div>
					{/if}
					...
					<div class="flex items-center justify-between border-t border-gray-100 pt-3">
						...
						<a
							href="/access?product_id={product.product_id}"
