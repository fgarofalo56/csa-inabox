	...
	const filteredSources = $derived(
		sources.filter((s) => {
			...
			return (
				...
				s.owner.domain.toLowerCase().includes(q) ||
				(s.description && s.description.toLowerCase().includes(q))
			);
		})
	);

	const domains = $derived([...new Set(sources.map((s) => s.owner.domain))].sort());
				...
				<tbody class="divide-y divide-gray-200">
					{#each filteredSources as source}
						<tr class="transition-colors hover:bg-gray-50">
							...
							<td class="px-6 py-4 text-sm text-gray-600">{source.owner.domain}</td>
							<td class="px-6 py-4">
