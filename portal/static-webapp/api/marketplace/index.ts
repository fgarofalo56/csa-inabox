...
const marketplaceProxy: AzureFunction = async function (
	context: Context,
	req: HttpRequest
): Promise<void> {
	const restOfPath = context.bindingData.restOfPath || '';
	const targetUrl = `${API_BASE_URL}/api/v1/marketplace${restOfPath ? '/' + restOfPath : ''}`;
...
export default marketplaceProxy;
