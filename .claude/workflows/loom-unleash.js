...
for (const t of tasks) {
  ...
  const research = await agent(researchPrompt(t), { model: 'fable', agentType: 'Explore', phase: 'Research', label: `research:${t.id}` })
  const build = await agent(buildPrompt(t, research), { model: 'fable', phase: 'Build', label: `build:${t.id}` })
  const pr = (String(build).match(/PR\s*[=#:]?\s*(\d+)/i) || [])[1] || ''
  if (!pr) { shipped.push({ id: t.id, ok: false, note: String(build).slice(0, 200) }); continue }
  const review = await agent(reviewPrompt(t, pr), { model: 'fable', phase: 'Review', label: `review:${t.id}` })
  if (/^\s*FAIL/i.test(review)) await agent(fixPrompt(t, pr, review), { model: 'fable', phase: 'Review', label: `fix:${t.id}` })
  const polish = await agent(polishPrompt(t, pr), { model: 'fable', phase: 'Polish', label: `polish:${t.id}` })
