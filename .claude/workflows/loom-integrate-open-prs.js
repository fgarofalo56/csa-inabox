...
if (!prs) {
  const survey = await agent(
    ...
    { model: 'sonnet', phase: 'Survey', label: 'survey:open-prs', schema: PR_SCHEMA },
  )
...
for (let i = 0; i < batches.length; i++) {
  const res = await agent(batchPrompt(batches[i], i, batches.length), { phase: 'Integrate', label: `batch:${i + 1}/${batches.length}` })
...
const verify = await agent(
  ...
  { phase: 'Verify', label: 'verify:final-build' },
)
