...
**Loom default backend:** Synapse **Dedicated** SQL pool (warehouse compute) + Synapse **Serverless** SQL (read-only endpoint) + **ADLS Gen2 + Delta** (the lake the warehouse/endpoint reads & writes). This is the **only** warehouse backend — the Fabric Warehouse opt-in was removed (rel-T94) because it was an unbuilt stub; per `no-fabric-dependency.md` the item is 100% functional on Azure with no Fabric.
