...
| Newly filed work | #4549 · #4550 · #4553 · #4554 |
...
a PR number without complaint — so it cannot be the discriminator. The GraphQL
`issueOrPullRequest` field can be, because it returns `__typename`. Inlined here
rather than cited, because the script that produced it lives in gitignored
`temp/` and is therefore **not reproducible by anyone else**:

```bash
# Per number N. Extract the numbers with:
#   grep -oE '#[0-9]{3,5}' .harness/archive/2026-08-08/state.json | sort -u
gh api graphql -F o=fgarofalo56 -F r=csa-inabox -F n=<N> -f query='
...
Run over all 87 — **zero unresolved**, and the run refuses to score if any
lookup fails, so a rate-limited `gh` cannot fabricate a clean total:
...
**Result: 11 DONE · 3 DONE→TRACKED · 3 SUPERSEDED · 5 SPLIT→FILED · 2 SPLIT→RECORDED** — 24 rows, the 18 unmapped plus the 6 reached by no issue.

Five labels. The **previous** revision of this page declared four and then used
nine spellings across the 24 rows, because five rows carried `DONE, <remainder>`
— a shape the legend did not have. The row text was accurate; the legend was
too small. **Fixed by widening the legend, not by flattening the rows**, since
the rows are where the information is.

| label | meaning |
|---|---|
| **DONE** | every ask discharged, **no remainder**; the site that does it is named |
| **DONE→TRACKED** | discharged except a remainder that an **existing** issue or operator question already owns; named inline |
| **SUPERSEDED** | the remainder as a whole is carried by open issues; named |
| **SPLIT→FILED** | partly done, and the live remainder was **filed as a new issue by this work** |
| **SPLIT→RECORDED** | partly done, and the remainder is recorded at the site **with the reason it was not filed** |

`CANNOT-VERIFY` is a **modifier on a specific claim**, not a sixth category — it
marks something that cannot be established from this position, and it always
names the probe that would settle it.

Five rows are SPLIT→FILED: `G3`→#4549, `C16`→#4550, `D17`→#4550, `D3`→#4553,
`C7`→#4554. `D17` and `C16` share the t169 gap, so #4550 covers a concern named
by two tasks. Task titles bundle two or three asks apiece; **the counts are of
tasks, not of asks.**
...
`build_inventory.py:14` — so a mislabel misleads a **counter** without dropping
a row from the automated pass, and a row-reader sees the remainder either way.
That is why the filings matter more than the table, and why every live remainder
is an issue, an operator question, or explicitly explained.
...
|---|---|---|
| `C13` | **DONE — including the residue it flagged** | loom-ui-verify repaired via merged #3076. Its own evidence flagged a product defect as *"Still unfixed"*: `/copilot` rendering **neither** the Ready badge **nor** the honest AOAI gate. That is now discharged, and the two branches are **mutually exclusive on the same state**: `app/copilot/page.tsx:370` renders the `Ready` hero chip when `ready`, and `:431-455` renders a real Fluent `MessageBar` (with `MessageBarActions` — *Recheck* and *Go to AI Foundry*) when `!ready`. `e2e/aoai-target-resolution.spec.ts:307` asserts **one or the other** must appear. |
| `C7` | **SPLIT→FILED** (#4554) — defect done, G1 receipt filed | Merged #3064 (route body-parse + `provisionBackingRg` threaded + wizard mounted on `/workspaces`) and #3065 (register corrections). Mutation-proved at the time. At head the contract suite is `app/api/workspaces/__tests__/create-wizard-contract.test.ts` with **7 `it()` blocks** (`:102 :109 :125 :133 :142 :147 :155`). **Residue:** the reviewer recorded *"NO live-browser G1 receipt — tsc+vitest only"*, blocked then by `C13`, which is now fixed. Filed as **#4554** — per-surface is the convention here (#2581 three semantic-model tabs, #4408 four picker surfaces, #4470 the Copilot UAT rule), so `/workspaces` is the missing row, not a duplicate. |
| `C14` | **SPLIT→RECORDED** — parity half SUPERSEDED, test half recorded | Merged #3071. **Parity docs** (14 docs, 172 MISSING rows) → superseded by open **#3726** (15 admin pages with zero parity doc), **#3725** (4 confirmed-stale docs), **#3722** (ai-red-team "Not A-grade" — untracked), **#3720** (slate-app self-grades D). **Editor tests:** the task named 20 remaining; `lib/editors/__tests__/` now holds **195** files against the 13 spec files it measured, and 6 of the 20 named have a matching test file (`activation-sync`, `cosmos-account`, `data-marketplace`, `digital-twin`, `loom-app`, `mapping-dataflow`). **Filename matching cuts both ways and is weak in both directions** — it does not show the other 14 absent, and it does not firmly show these 6 present: `activation-sync` matched `activation-sync-entity-picker-3526.test.tsx`, which may test a picker rather than the editor. An independent reviewer running the same probe got **5**, not 6, because they keyed on the ledger's `azure-cosmos-account` while the file is `cosmos-account.test.tsx` — the caveat demonstrating itself. Not filed, same reasoning as `C10`: coverage expansion over shipped routes, no observed defect. |
| `E2` | **DONE (merged), corpus number NOT re-measured)** | Merged #3069. Root cause was a chunker emitting `title › innermost-heading` with no product label, so `classifyExcerptProvenance` returned `unlabelled` for whole gold documents; fixed with full `title›H2›H3` ancestry. The mechanism is checkable in the code; the headline **`unlabelled` 1711→1249** is quoted from the merged PR and was **not** independently re-derived here — a corpus-wide eval count is not reproducible from a static read. Flagged rather than left reading as a re-measured fact. |
| `D3` | **SPLIT→FILED** (#4553) — one residue filed, one CANNOT-VERIFY | Merged #3052 — the three degraded apps' pull UAMIs had zero role assignments; grants created, `dependsOn` added. **Residue:** its evidence says to delete the stray Container App `loom-wrangler-h2` after a deploy re-points `LOOM_WRANGLER_ENDPOINT`. The stray is **not an orphan** — `scripts/measure/estate-resume.mjs:114` carries it in the applied `APPS[]` list with `min: 1`, beside `loom-activator` and `loom-onelake`, so the next `estate-resume --apply` **recreates it**. Deleting without removing line 114 silently undoes itself. Filed as **#4553**. Separately **CANNOT-VERIFY**: whether the stray still exists at all is estate state this change cannot observe — probe `az containerapp list -g rg-csa-loom-admin-centralus --query "[?contains(name,'wrangler')].name"`. |
| `P2-DOCS` | **DONE→TRACKED** (#3527, OP-3) | Merged #3070 (18 files, docs only). It found and fixed real drift — 4 foundry-parity ❌ that were actually shipped, a PARITY-MATRIX whose staleness had migrated to 4 sibling docs, an `EXISTING_*` table wrong in **all 7 rows**, and a `brownfield.md` that contradicted itself on dlz-attach. **Residues, both already owned:** 11 of 12 grades left `_TBD_` deliberately rather than fabricated, naming `C1` as the producer → that is open **#3527**; and the clean-sub acceptance procedure written under an explicit *"Status: NOT RUN"* banner → that is **OP-3**. |
...
| `C4` | todo | **DONE→TRACKED** (#2581) | Typed client-route map merged as **#3152** (*"B-R15-17, FINISHLINE C4"*); WS-4.4 object sync merged as #2313; B-R10 slice 1 as #2565. `docs/fiab/decomposition-plan.md:185` records the split. Residual is **#2581, OPEN** and already in the drain ledger. |
...
| `D17` | todo | **SPLIT→FILED** (#4550) — docs half done, t169 filed | R8 greenfield + brownfield walkthroughs exist as separate complete pages: `docs/fiab/deployment/greenfield.md`, `brownfield.md`, `failure-recovery.md`, `resource-groups.md`. **Wizard/doc agreement was re-measured, not assumed** — see below. The t169 half went to #4550; the clean-sub acceptance runs are **OP-3**. |
...
### A published "zero hits" that was produced by a blind instrument

Recorded because it is the same defect class this retirement exists to prevent,
committed by the retirement itself — twice now, in different instruments.

A previous revision of the `D3` row published: *"`grep -rn "loom-wrangler-h2"`
across the repo returns **zero hits** — no tracker anywhere."* That is false.
The probe actually run was:

```bash
grep -rn "loom-wrangler-h2" . --include=*.md --include=*.yml --include=*.bicep --include=*.ts
```

Every real hit is in a `.mjs` or `.json` file. **The include-filter excluded
exactly the file types where the answer lived**, so the instrument could not
have returned a hit no matter what the repo contained — and its silence was
published as evidence of absence.

Re-run with a positive control first, which is what would have caught it:

```
control:  git grep -l "loom-wrangler"     -> rc=0, 35 files   (the path works)
claim:    git grep -n "loom-wrangler-h2"  -> rc=0, 4 hits / 3 files
```

`git grep` reads only tracked files, so three files is a **floor**, not a
ceiling.

**The consequence was not the sentence, it was the disposition.** `D3` moved
from `DONE, residue CANNOT-VERIFY` to `SPLIT→FILED` (#4553), because
`estate-resume.mjs:114` makes the residue a *repo* finding — knowable with no
estate access at all — rather than only an estate-state question.

The sibling lesson, from the same round: the by-type count above was wrong for
the same reason in a different instrument — `gh issue view` **resolves a PR
without complaint**, so a probe that never asked `__typename` could not see the
40 PRs and reported them as issues. Both failures are one shape: **an instrument
that cannot express the answer, reporting a clean result.**

Two independent reviewers found the `grep` error separately, which is the only
reason it did not ship. Standing rule taken from it, applied to every claim in
this document: **run the positive control before publishing the measurement.**

### The filings landed after the drain ledger was built
...
**Four issues.** The test applied to every candidate was the same: *does filing
this add information the drain does not already have?* Per the operator's
instruction, filing work that is already done costs the drain a cycle to
rediscover, which is worse than the loss it was meant to prevent — so a
remainder already owned by an open issue, an operator question, or a standing
debt class was recorded rather than filed.

| issue | from | why it passed the test |
|---|---|---|
| **#4549** | `G3`, then `G2` | Four `gov-provision` lanes `active` and last-red, **two never green in their entire run history**, and no staleness or lane-state guard watches any. The `deploy-integrity.md` R3 / `cloud-parity.md` never-exercised shape. |
| **#4550** | `C16`, `D17` | The CAF function-RG split (t169) is still not built, and `docs/fiab/deployment/resource-groups.md:202` tracked it by the string "FINISHLINE `C11`". That pointer was **already wrong** — C11's reference is #1483, the domain designer — and this retirement would have made it name nothing at all. |
| **#4553** | `D3` | `estate-resume.mjs:114` carries the stray `loom-wrangler-h2` with `min: 1`, so the next resume recreates it. The ledger carrying the delete instruction is being archived **while the mechanism that defeats it stays live** — nothing else connects the two. |
| **#4554** | `C7` | The owed G1 receipt for `/workspaces`. Filed on reconsideration: per-surface **is** the convention here (#2581 three semantic-model tabs, #4408 four picker surfaces, #4470 the Copilot UAT rule), so this is the missing row, not a fourth tracker on a generic debt. |

The earlier reasoning for **not** filing #4554 — *"a standing repo-wide debt with
three open trackers"* — did not survive checking its own citations. All three
are per-surface or per-change. Recorded because the mistake is instructive: a
non-filing justified by a pattern needs the pattern verified, exactly like any
other claim.
...
- Whether the stray `loom-wrangler-h2` still exists in the estate — an
  estate-state question with no estate access from here. #4553 carries the probe
  and is careful to claim only the repo half.
- `C14`'s editor-test counts rest on **filename matching, which is weak in both
  directions** — see that row.
- `E2`'s `unlabelled` 1711→1249 is quoted from a merged PR, **not** independently
  re-derived; a corpus-wide eval count is not reproducible from a static read.
- `C10`'s *"verify EVERY resource"*: 8 of 13 clients and 8 of 17 surfaces are
  covered, and the five uncited are named — but no run established whether the
  uncovered five actually misbehave.
...
- `gh issue view 4495` had **comments=0**, and the drain ledger's single row for
  ...
  the paths. That is what an autonomous pass actually walks. Its relative
  `../blob/main/PRPs/active/finishline-retirement/...` links **404 until this PR
  merges** — expected, since the files do not exist on `main` yet, and they
  resolve on merge rather than needing an edit.
...
falls back to *"No state file (run /harness-init first)"*; `:48` opens the same
