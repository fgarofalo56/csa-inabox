...
| 2.4 | Actions (typed write-back + validation + side effects) | Governed edits | ontology action types | Loom action runtime → real backend writes + audit | η→✅ validation rules SHIPPED + E2E'd (#2200) — per-parameter submission criteria (nonEmpty/gt/gte/lt/lte/eq/neq/regex/in) enforced server-side (422) before write-back; authored in the Typed-model dialog; +9 tests. Live E2E rev 0000340: authored `total > 0` on createOrder → run total=0 blocked ("must be > 0") → total=5 created Order vertex 1125899906842625. Remaining η: webhooks/notify side-effects |
