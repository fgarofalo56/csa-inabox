...
1. ~~**Wire the tier-router for real.**~~ **✅ DELIVERED — re-verified 2026-08-06.** `lib/azure/aoai-chat-client.ts`'s `routeTurn()` calls `routeTurnTier()` on the **shared** client path, so every copilot / agent / data-agent turn is tier-aware, not only the streaming orchestrator. Escalate-only (a light turn is never downshifted), traced `[tier-router]`, admin-tunable via **Copilot & Agents → Model tiers**, and it warns explicitly when the tiers collapse onto one deployment. The browser-E2E receipt (G1) is still owed.
2. **Reasoning-mode for data agents.** Give `data-agent-client.ts` a planner→execute→verify loop on the reasoning tier for multi-hop analytical questions (frontier data agents plan before querying). **Still open.**
