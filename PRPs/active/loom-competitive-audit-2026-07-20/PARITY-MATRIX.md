...
| **Databricks** (Data Intelligence Platform) | **B+ / A−** | UC dual-backend (DBX + OSS-UC) is **A**; Lakeflow DLT/Jobs/SQL + AI-enrichment are A/A−; trails hard on Feature Store (D), Model Serving (C+), LLM fine-tuning (F). |
| **Azure AI Foundry** (Microsoft Foundry) | **B+ / A−** | prompt-flow/eval/content-safety/tracing/AI-search/red-team/agent-eval all real; **tier-router exceeds** Foundry's Model Router *(when wired)*; gap = fine-tuning (F), eval-library breadth, OTel span-tree. |
...
**Composite thesis:** Loom is a genuine **B+ / A−** against every named competitor *individually*, and the only product that is **all of them at once behind one console, one auth, one governance plane, sovereign-capable**. The gaps are concentrated (Direct Lake, Feature Store, Model Serving, fine-tuning, AIP-studio depth, agent-builder polish, tier-router wiring) and closable; the wins (sovereignty, integration seams, backend-swap, cross-item copilot, ontology-over-everything) are structural and un-copyable by any single-product vendor.
...
| **Feature Store / online tables / PIT** | Databricks Feature Engineering | `unity-catalog/online-tables/route.ts` (list) | **D** | **P0 — MISSING** feature-table authoring, PIT-join, feature-lookup-at-serving. |
...
| **Model tier routing** | Foundry Model Router | `model-tier-router.ts` (default-ON, admin-tunable) | **A *(exceeds when wired)* / no-op today** | Task-class routing built-in; **safe NO-OP when no tier deployments configured — the key model-quality lever (see Findings P0).** |
