...
| **Databricks** (Data Intelligence Platform) | **B+ / A−** | UC dual-backend (DBX + OSS-UC) is **A**; Lakeflow DLT/Jobs/SQL + AI-enrichment are A/A−; Feature Store now **A−** (WS-2.1 `feature-table` item) and Model Serving now **A** (WS-1.2); LLM fine-tuning since **CLOSED on `main`** (WS-1.3 `fine-tuning-job` item, re-graded 2026-07-27 — see §5 row). |
| **Azure AI Foundry** (Microsoft Foundry) | **B+ / A−** | prompt-flow/eval/content-safety/tracing/AI-search/red-team/agent-eval all real; **tier-router exceeds** Foundry's Model Router (WIRED into the aoai-chat hot path via `routeTurnTier`, WS-1.1); gap = eval-library breadth, OTel span-tree (fine-tuning since **CLOSED on `main`** — WS-1.3 `fine-tuning-job` item, re-graded 2026-07-27). |
...
**Composite thesis:** Loom is a genuine **B+ / A−** against every named competitor *individually*, and the only product that is **all of them at once behind one console, one auth, one governance plane, sovereign-capable**. The gaps are concentrated (Direct Lake, AIP-studio depth, agent-builder polish) and closable — Model Serving (WS-1.2), tier-router wiring (WS-1.1), Feature Store (WS-2.1 `feature-table`), and LLM fine-tuning (WS-1.3 `fine-tuning-job`, re-graded 2026-07-27) have since CLOSED on `main`; the wins (sovereignty, integration seams, backend-swap, cross-item copilot, ontology-over-everything) are structural and un-copyable by any single-product vendor.
...
| **Derived properties** | Foundry derived props | `phase4/ontology-derived-panel.tsx` (WS-4.2) + computed on read by the object-view route | **A−** | **SHIPPED — re-verified 2026-08-06** (was "F / P0 — MISSING"). Live rollups (count/sum/avg/min/max) over linked objects **and** function-kind derived properties backed by a registered function-on-objects; wizard-driven (no freeform config), persisted to `state.derivedProperties[<objectType>]`. Owed: browser-E2E receipt. |
| Granular/row/property security on objects | Foundry Restricted Views | `phase4/ontology-security-panel.tsx` (WS-4.3) + `lib/foundry/object-security`, enforced in `/objects`, `/objects/[vertexId]/view`, `/resolve` | **A−** | **SHIPPED — re-verified 2026-08-06** (was "D / MISSING as ontology feature… not wired to objects"). Three marking kinds — property (CLS analogue), row (RLS analogue) and action — authored from Entra group pickers and **enforced in the BFF routes**, i.e. wired to objects. Owed: browser-E2E receipt. |
| Dataset→object sync (OSv2) + backfill | Foundry Object Storage v2 | `lib/azure/object-dataset-sync.ts` (WS-4.4) + `/items/ontology/[id]/sync` + `ontology-sync-panel.tsx` | **B+** | **SHIPPED — re-verified 2026-08-06** (was "C / no scaled sync/backfill"). Synapse Serverless/Dedicated → idempotent AGE MERGE → AI Search `loom-object-instances`, with a Cosmos progress doc. Residual: **scheduled/incremental** re-sync (runs are operator-triggered). |
| Ontology proposals / branches | Foundry proposals + Global Branching (May 2026) | — | **F** | MISSING — no staged-model review/approve. **Re-verified still open 2026-08-06**: no `ontologyProposal` / proposal-store / ontology-branch code in `apps/fiab-console`. |
...
| **LLM fine-tuning** | Databricks Mosaic FT / Foundry serverless+managed FT | `fine-tuning-job` item — `lib/azure/fine-tuning-{client,item}.ts` (+ `__tests__/fine-tuning-client.test.ts`), `lib/editors/fine-tuning-job-editor.tsx`, `app/api/foundry/fine-tuning/{,files,[jobId]}` + `app/api/items/fine-tuning-job/[id]/{deploy,events,safety-eval}` routes (+ route test) | **A− (was F)** | **CLOSED on `main` (re-graded 2026-07-27, A7 truth reconcile):** first-class fine-tuning item — job authoring editor, training-file upload, job lifecycle/events, safety-eval, deploy-to-endpoint, all against real AOAI/Foundry FT REST. Was stale after WS-1.3 landed; evidence `PRPs/active/loom-apex/research/prps-audit.md:140-142`. Browser-E2E receipt per the G1 bar still pending like other post-matrix rows. |
