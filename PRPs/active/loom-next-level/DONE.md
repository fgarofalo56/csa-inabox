...
| 4c/4d/openness (Phase-4 completion) | 2026-07-24 | Aggregate FRESH0 on merged main (b4aac59b) → **strict, 0 warnings; all 18 guards clean**. Bundles the whole Phase-4 tail landed since 4b-tail: **4c** N16 code-report + N17 OpenLineage incident console + N18 embedded-RLS SDK (pillar-3 core); **4d** the inbound-migration wave — M1 estate assessment on-ramp (#2516), M2 schema+data copy-in (ADF Copy → Bronze/managed Delta, #2517), M3 code translation (SQL/DAX/report → Loom, honest needs-review diff, #2517); **openness tail** N7a RisingWave streaming-SQL + N7e Trino federation (the single opt-in carve-out) + N8 Tier-3 labs (PRQL / DuckLake / s3-gateway; MinIO/AGPL dropped) (#2519). Re-baselined: route-total 1617→**1638**, toolkit-gap 1343→**1338** (ratchet keeps shrinking), param-cap 232 unchanged. Integration caught + fixed on the combined tree: 4-way env-pin reconcile (181→**185**), unioned runtime-flags/health-coverage/gate registries, merged SQL-Lab toolbar (N8 PRQL language toggle + N7e Trino engine picker; PRQL transpiles first→runnableSql then endpoint select), a doubled Dropdown/Option/Field import, a hardcoded servicebus.windows.net doc-literal (cloud ratchet), and one brittle `/Materialized/` matcher (also matched the tab). Also fixed an env-wide CI bug this wave: the copilot-quality-evals `//`→`||` jmespath crash + a fault-tolerant honest-gate on the evaluator key lookup (#2518). Rolled to BOTH estates off b4aac59b (Commercial loom-roll-and-validate + Gov gov-console-roll). **Openness pillar COMPLETE; program north-star core (N1–N18 + M1–M3 + N7-tier2 + N8-labs) fully landed.** |
