...
V = {
    ...
    3882: ("STALE", "-", "-", "-", "RE-MEASURED 2026-08-31: the red streak IS the detector working - whatif-drift-verdict.mjs exits 1 on REAL drift by design, and today's run 33406666389 measured BOTH estates and refreshed #3191 + #2874. The work is the drift (#3191/#2874, W3), not the detector. CLOSED with evidence."),
    ...
    4238: ("REAL", "XS", "W0", "deploy-A", "Filed from PR #4236's independent review (pre-existing): the roll health gate's *Healthy* substring MATCHES Unhealthy, so an Unhealthy+Running revision passes the gate built to stop it. Same file as #4231's fix - sequence after PR #4236 lands."),
    ...
    4190: ("STALE", "-", "-", "-", "RE-MEASURED 2026-08-31: lane green 5x since PR #4198; the one later failure was a correct pre-roll refusal under lease contention. CLOSED with evidence; defect lives in #4196."),
    4196: ("NEEDS-ESTATE", "S", "W4", "estate", "RE-MEASURED 2026-08-31: core defect FIXED at head by PR #4198 (two-verdict digest step, unknown leaves images in place; loom-dataplane-roll.yml:708-787 cites this issue). Owed: an unknown-path live receipt - W4 window, never an ad-hoc registry race. Residual: cross-workflow lease contention still fails lanes spuriously (~4 events 08-29..08-31, self-healing)."),
    3968: ("REAL", "M", "W0", "deploy-A", "console-bluegreen-roll: 3 consecutive FAILURE, ~3 weeks stale (overlaps U2)"),
    3429: ("STALE", "-", "-", "-", "RE-MEASURED 2026-08-31: lane green 2x (08-22, 08-29) after the misaimed-secret fix; the false-error half fixed by #3431 on 08-14. CLOSED with evidence."),
    ...
    3346: ("NEEDS-ESTATE", "S", "W4", "estate", "RE-MEASURED 2026-08-31: Commercial half FIXED and verified live - loom-dataplane-roll now fires via workflow_run off the build lane (last 3 runs, all green). Gov half: ZERO gcc-high rolls ever; first Gov roll + boot-banner receipt queued to the W4 window with #3060."),
    4144: ("REAL", "M", "W0", "deploy-C", "RE-READ 2026-08-31: six untracked stale/never-run deploy paths reported daily by deploy-staleness with no owner (report-subscriptions never ran for real; loom-uat + loom-verify harnesses 17d+ stale; il5 half overlaps #4073/#4233). Code/ownership fixes here; the catch-up dispatches are estate-touching and pair with the W4 window."),
    # --- W1 live operator bug reports, round 2 (2026-08-31 evening) ------
    # Four reports from the operator in one message; each root-caused the same
    # night, three fixed and MERGED, one (iceberg) root-caused with the fix
    # lane still to open.
    4241: ("REAL", "M", "W1", "brain", "OPERATOR LIVE REPORT: Brain recommendations + node-select detail unreadable. Ten defects measured (content at Caption1/Foreground3, a 90-word banner baking a stale 429 measurement as fact, italic-paragraph value cells, 10px edge labels, an 80px-persistable details pane, meta-first card hierarchy). FIXED + MERGED PR #4245 + LIVE on the estate; G1 browser receipt in flight."),
    4242: ("REAL", "L", "W1", "brain", "OPERATOR LIVE REPORT: recommendations need a Perform action (auto-bind §5 / G2 - a remediation the platform could execute must not be prose). BACKEND MERGED PR #4246: guarded executor registry OUTSIDE lib/brain so all four no-mutation invariants stay intact; guards re-derived server-side incl. snapshot-completeness (the #4015 rule). UI wiring is the remaining half."),
    4243: ("REAL", "M", "W1", "estate-power", "OPERATOR LIVE REPORT: Pause errors out. ROOT CAUSE by elimination (the failure left ZERO server-side trace, which fingerprinted the branch): the drift-refusal 409, manufactured by the console's own throttled reads shrinking the preview population (token embeds the count). FIXED + MERGED PR #4247 (three-way absent/throttled/unreachable classification). Live receipt owed."),
    4244: ("REAL", "M", "W2", "console-api", "Systemic half of #4243: the read-warmer keeps the console UAMI's ARM read budget exhausted (~1,700 throttle lines/30min, sustained). Every interactive ARM read in the console shares it; also produced unrelated ComputeBudgetExceededError 500s."),
    4248: ("REAL", "S", "W2", "console-api", "From PR #4247's review: vmss-client.ts:86-88 composes the Purview SHIR from LOOM_ADMIN_RG+LOOM_SUBSCRIPTION_ID, ignoring the deploy-produced coordinate vars - the same mismatched-coordinates family, in the scaling surface."),
    4240: ("REAL", "XS", "W5", "scripts", "From PR #4237: four files still describe the pre-#4064 'split pair freezes the reconcile' consequence (prose only, behavior unaffected), plus an observability gap - a follower divergence is converged away with no log line."),
