...
**CAPTURED is not PUBLISHED, and the 0/142 reads like a contradiction without
that distinction.** `docs/fiab/help-inventory-2026-08-08.md:172` records
**121 of 142** editor guides as having a *captured landing screenshot*, while
`:169` says screenshot capture (`help-D6`) is operator-gated and was not
attempted, and `:175` records `EDITOR_STEP_IMAGE_COUNTS` as empty — no slug has
a published multi-step capture. So 121 landing captures exist and zero of the
159 tutorial sets are published. Both numbers are true of different things; the
0/142 above is about publication. Stated here because a reader finding 121 next
to 0 will otherwise assume one of them is wrong.
