...
**§P2 VERIFICATION (2026-07-24, grep-confirmed against `main` b4aac59b):**
- ✅ **Already BUILT** (need only a DONE-ledger row, not a wave): **C5** snapshot delivery (`report/[id]/subscriptions` route + honest `deliveryGate`; only the delivery Logic App infra is pending), **U2** report aux-panes (bookmarks/selection/filters in report-designer), **U11** px→token ratchet (`check-no-raw-px.mjs` guard live; drain is opportunistic), **I4** PDP shadow-mode UI (`components/admin/pdp-shadow-report-card.tsx` + `/admin/permissions`), **I8** limits/quotas doc (`docs/learn/08-reference/limits-quotas` + ADR 0020/0021), **DR4** Health hub (`/admin/health` + `health/exercise` + `slo` + `diagnostics/bundle`). **A4** AAS-GA-in-Gov = ground-truth GA (see [[csa_loom_next_level_prp_2026_07_22]]) — a doc/enablement note, not a build.
- ❌ **Real open gaps (schedule as ONE post-completion wave, ~9):** **EXP1** workspace export/import/clone, **A14** collab push transport (today poll-based), **CMK1** Cosmos CMK bicep, **U8** KQL-dashboard depth (params/drillthrough/pages), **U9** canvas fullscreen, **U13** pipeline in-canvas debug/output overlay, **L5** column-level impact CANVAS (only OpenLineage column facets exist), **SC1** repo-wide image-signing gate (cosign exists only for publish-loom-cli), **CH1** dedicated chaos surface (only `markPoolFaultedForDrill` drill-hook today).
- 🔲 **Not re-verified precisely:** U12 (new-item-dialog px/dead-field tail — likely partial), `WS-R` R4–R6/R10–R29 (verify against the live toolkit ratchet — gap is 1338, shrinking, so most are superseded).
