...
| **Trino (Federated SQL engine, N7e)** | `trinodb/trino` | Apache-2.0 | `data-plane/loom-trino-aca.bicep` (DEFAULT-ON, scale-to-zero single node), `data-plane/loom-trino-aks.bicep` (opt-in multi-node scale-out) | ANSI-SQL federation across the Loom Iceberg REST catalog + external connectors, so one statement can join a Loom Iceberg table with an external source. Image published by the Trino project itself (`github.com/trinodb/trino`), pulled UNMODIFIED on top of our own thin config layer — a NOTICE row, not a redistribution. Runs in-boundary/air-gap-safe (no SaaS query federation; SaaS-only connectors stay honestly gated). |
...
| **Apache Airflow — upstream image `apache/airflow`** | `apache/airflow` | Apache-2.0 | `admin-plane/airflow.bicep` | ASF-published Docker Hub image of the `github.com/apache/airflow` tree (Apache License 2.0). Backs the OSS Airflow host (`LOOM_AIRFLOW_ENDPOINT`) — the Azure-native alternative to a Fabric/ADF WOM environment. Pulled UNMODIFIED, so this is a NOTICE row, not a redistribution. **Supply chain (#2682):** the `airflowImage` value is the MIRROR SOURCE-OF-TRUTH and the licence record, not the effective pull — the module composes `${acrLoginServer}/apache/airflow:<tag>`, so the running host always pulls from the estate's own ACR. The upstream coordinate is digest-pinned in `platform/fiab/images/upstream-images.json` and imported by `scripts/ci/mirror-upstream-images.sh` in both clouds. Enforced by `scripts/ci/check-license-inventory.mjs` (LIC0 — licence) and `scripts/ci/check-upstream-image-mirror.mjs` (MIR0 — mirror + digest pinning). |
| **curl — upstream image `curlimages/curl`** | `curlimages/curl` | curl (MIT/X derivative) | `compute/loom-memory-consolidate-job.bicep` | HTTP client for the memory-consolidation scheduled job; license at curl.se/docs/copyright.html — permissive, no copyleft obligation. Pulled UNMODIFIED (NOTICE row). Same ACR-mirror posture as Airflow above: digest-pinned in `platform/fiab/images/upstream-images.json`, mirrored by `scripts/ci/mirror-upstream-images.sh`, effective pull `${acrLoginServer}/curlimages/curl:<tag>`. |
| **Valkey — upstream image `valkey/valkey`** | `valkey/valkey` | BSD-3-Clause | `shared/redis-oss-aca.bicep` | Project-published Docker Hub image of the `github.com/valkey-io/valkey` tree (BSD 3-Clause, `COPYING`). Valkey is the Linux Foundation fork of Redis 7.2.4 and backs the **sovereign** shared Redis substrate (`LOOM_RESULT_CACHE_REDIS`) — Azure Managed Redis is Azure Public cloud only, so GCC-High / IL5 get this today — and GCC when its apps lane lands (#3078), since `redisOssActive` also requires `deployAppsEnabled` — instead of riding the retiring `Microsoft.Cache/redis` to its 2028-10-01 turn-off (#2642). **The `redis` image is deliberately NOT used** — Redis relicensed to RSALv2/SSPL in 2024 and to AGPLv3 in Redis 8, and both are on the LIC0 forbidden list. Pulled UNMODIFIED (NOTICE row). Same ACR-mirror posture as Airflow above: `acrLoginServer` is required with no public-registry branch, so the effective pull is always `${acrLoginServer}/valkey/valkey:<tag>`; digest-pinned in `platform/fiab/images/upstream-images.json` and imported by `scripts/ci/mirror-upstream-images.sh` in both clouds. Enforced by `scripts/ci/check-license-inventory.mjs` (LIC0) and `scripts/ci/check-upstream-image-mirror.mjs` (MIR0). |
| **s3proxy — upstream image `andrewgaul/s3proxy`** | `s3proxy` (ACR mirror of `andrewgaul/s3proxy`) | Apache-2.0 | `data-plane/s3-gateway-aca.bicep` | Author-published Docker Hub image of the `github.com/gaul/s3proxy` tree (Apache License 2.0). Backs the S3-compatible ADLS gateway (`LOOM_S3_GATEWAY_URL`) so s3://-exclusive OSS clients can address the governed lake. **The AGPL-v3 MinIO gateway is deliberately NOT used** — it is on the LIC0 forbidden list. Pulled UNMODIFIED (NOTICE row). Supply chain (#2682): the module REQUIRES an `acrLoginServer` and has no public-registry branch, so the effective pull is always `${acrLoginServer}/s3proxy:<tag>`; the upstream coordinate is digest-pinned in `platform/fiab/images/upstream-images.json`. Enforced by `scripts/ci/check-upstream-image-mirror.mjs` (MIR0). |
