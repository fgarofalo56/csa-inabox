...
Typical use::
    ...
    enforcer = SharingEnforcer(agreements_dir="platform/governance/data_sharing/agreements/")
...
def load_agreement(path: Path) -> SharingAgreement:
    ...
    if expires_str and not expires_str.startswith("{"):
        try:
            ...
            logger.warning("Invalid expiresAt date in %s: %s", path, expires_str)
    ...
    for yaml_path in sorted(agreements_dir.glob("*.yaml")):
        ...
        try:
            ...
            if "{" in agreement.provider_domain or "{" in agreement.consumer_domain:
                logger.debug("Skipping template/placeholder agreement: %s", yaml_path)
                continue
            agreements.append(agreement)
            logger.info(
                "Loaded sharing agreement: %s (%s → %s)",
                agreement.name,
                agreement.provider_domain,
                agreement.consumer_domain,
            )
        except Exception:
            logger.exception("Failed to load sharing agreement from %s", yaml_path)
