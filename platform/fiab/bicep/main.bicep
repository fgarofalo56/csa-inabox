...
@description('Enable Microsoft Information Protection reads via Microsoft Graph for the Console (sensitivity labels + label policies + apply-label evaluation + /admin/batch-labeling real MIP labels). DEFAULTS ON (opt-out): the post-deploy bootstrap grants the Console UAMI InformationProtectionPolicy.Read.All + SensitivityLabel.Evaluate by default, so MIP labels light up day-one. A Tenant Administrator must still click "Grant admin consent" once in Entra → Enterprise applications → Console UAMI → Permissions; until consent is issued every Graph MIP call returns 403 and the surface renders the honest NotConfigured MessageBar (never an empty stub). Set false only to suppress the MIP surfaces entirely.')
...
@description('ADX cluster tuning bag (R0) — passed through to admin-plane/main.bicep adxConfig (typed there as adxConfigT). {} (default) = the shared admin-plane cluster keeps its Dev SKU Dev(No SLA)_Standard_E2a_v4 on Commercial / GCC. On GCC-High / IL5 admin-plane substitutes Standard_E8as_v5+1TB_PS instead, because Azure Government offers NO Dev(No SLA)/Basic-tier ADX SKU AT ALL — measured in-boundary 2026-08-27 (gov-adx-sku-probe run 33120409251: usgovvirginia offers 9 SKUs, usgovarizona 14, every one Standard tier, none restricted). The earlier substitute Dev(No SLA)_Standard_D11_v2, recorded here as LIVE-verified on 2026-07-10, is NOT in either list today and left a Stopped adx-csa-loom-fmezxj unstartable for 17 consecutive gcch runs (#4072, #4141). COST, because this is not a like-for-like swap and cannot be: Standard tier is a documented 2-NODE minimum, so a Gov cluster is two instances where the Dev node was one. Set adxSkuName to move the cluster OFF a SKU Azure has no capacity to start: `az kusto cluster start` answering InsufficientResourcesForSubscription is a capacity condition that no retry and no role grant clears, and until this bag was threaded NOTHING an operator could set reached the SKU — every lane deploys THIS root, and assigning adxConfig in a bicepparam failed BCP259 (#4122). Also carries adxEnableOptimizedAutoscale / adxAutoscaleMinimum / adxAutoscaleMaximum. Future ADX cluster knobs ride THIS bag — never a new top-level param.')
