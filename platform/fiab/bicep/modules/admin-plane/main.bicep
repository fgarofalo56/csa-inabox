...
@description('Azure Analysis Services database/model name. Required alongside loomAasServer for the aas backend.')
    ...
    apps: [
      {
        ...
        env: concat(
          ...
          // Only emitted when wired; absence keeps the AAS path honest-gated
          // while the loom-native default (Cosmos + TMSL) still works.
          !empty(loomAasServer) ? [
            { name: 'LOOM_AAS_SERVER', value: loomAasServer }
          ] : [],
          !empty(loomAasDatabase) ? [
