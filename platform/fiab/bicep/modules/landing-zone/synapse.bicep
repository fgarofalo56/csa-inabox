...
@description('Loom Console UAMI APPLICATION (client) id. Required for the Synapse SQL Administrator grant: when an SPI grants a Synapse-RBAC role to another SPI by OBJECT id, Synapse cannot fetch the app id from Microsoft Graph, producing a BROKEN serverless login ("Login failed for user \'<token-identified principal>\'"). Passing the APP id sidesteps this (Learn: resources-self-help-sql-on-demand#security, Solution 3). Empty = SQL Administrator grant is skipped.')
