...
Optional keys:
  s3ProxyImage           s3proxy REPOSITORY:TAG **without a registry host** (default
                         's3proxy:3.3.0'). Recorded in
                         platform/fiab/images/upstream-images.json as the mirror
                         source-of-truth AND the LIC0 licence record; the deploy
                         workflow imports exactly that ref, BY DIGEST, into the
                         estate ACR. The EFFECTIVE pull is always
                         `<acrLoginServer>/<s3ProxyImage>` — there is deliberately
                         no way to express a public-registry pull here (#2682).
...
// `acrLoginServer` is REQUIRED (no '' default) and the image ref is COMPOSED
// from it, so this module structurally CANNOT emit a public-registry pull. That
// is the whole reason the gateway was withdrawn in PR #2640 round 4: the ref was
// `empty(acrLoginServer) ? 'docker.io/andrewgaul/s3proxy:3.3.0' : '<acr>/…'`, no
// producer supplied the coordinate, so every shipped lane resolved to an
// anonymous Docker Hub pull with `registries: []` — no firewall lease, no Trivy,
// no cosign, and unpullable in an air-gapped enclave. Removing the ternary is
// what makes the fix structural instead of conventional: a caller that forgets
// the coordinate now fails at template validation rather than silently going to
// the internet. The upstream coordinate + its pinned digest live in
// platform/fiab/images/upstream-images.json and are imported by
// scripts/ci/mirror-upstream-images.sh in both clouds.
var acrLoginServer = s3GatewayConfig.acrLoginServer
var s3ProxyImage = string(s3GatewayConfig.?s3ProxyImage ?? 's3proxy:3.3.0')
var image = '${acrLoginServer}/${s3ProxyImage}'
...
resource app 'Microsoft.App/containerApps@2025-02-02-preview' = {
  ...
  properties: {
    ...
    configuration: {
      ...
      // ALWAYS the estate ACR, pulled with the supplied UAMI. There is no
      // credential-free branch any more, because there is no public-pull branch
      // any more (#2682). Admin-enabled registries stay forbidden — this is
      // UAMI-based, always.
      registries: [
        {
