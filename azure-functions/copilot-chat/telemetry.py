...
# Event logger is initialised lazily so module import never fails.
_event_logger: logging.Logger | None = None
_init_attempted = False


def _get_event_logger() -> logging.Logger | None:
    global _event_logger, _init_attempted
    if _init_attempted:
    ...
    try:
        ...
        # Keep events out of the root logger / function host log stream.
...
def track_event(name: str, properties: dict[str, Any] | None = None) -> None:
    """Emit a custom event. Properties become customDimensions in KQL.

    Numeric values are coerced to strings by the AzureEventHandler — if you
    want them queryable as numbers, pass them through ``properties`` and
    use ``toint()`` / ``todouble()`` on the KQL side.
    """
