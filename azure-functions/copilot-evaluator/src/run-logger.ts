 ...
 *
 * B-FN migration (2026-07-27): the E2 evaluator used to run as a Y1 Consumption
 ...
 * not a trusted service, so host keys / timer leases fail), so the evaluator is
 * now an in-VNet `Microsoft.App/jobs` execution. This interface is the exact
 * log/warn/error subset `run-evals.ts` used, so the orchestrator body ported
 * unchanged.
 *
