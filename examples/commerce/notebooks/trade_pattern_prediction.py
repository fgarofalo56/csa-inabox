...
def predict_gdp_growth(df_gdp):
    ...
    gdp_agg["gdp_rolling_4q"] = (
        gdp_agg.groupby("state_name")["total_gdp"].rolling(window=4, min_periods=2).mean().reset_index(drop=True)
    )
...
def score_partner_risk(df_trade):
    ...
    partner_stats = (
        df_trade.groupby("partner_country_name")
        .agg(
    ...
    partner_stats["volatility_score"] = partner_stats["cv"].rank(pct=True) * 40
    partner_stats["concentration_score"] = partner_stats["concentration"].rank(pct=True) * 30
    partner_stats["volume_score"] = (1 - partner_stats["total_trade"].rank(pct=True)) * 30
    partner_stats["risk_score"] = (
        partner_stats["volatility_score"] + partner_stats["concentration_score"] + partner_stats["volume_score"]
    ).round(1)
...
def save_predictions():
    ...
    (risk_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_partner_risk_scores"))
...
def analyze_trade_balance_trends(df):
    ...
    if len(common_idx) > 0:
        ...
        axes[0, 0].bar(
            common_idx, balance_m / 1e9, color=["green" if b > 0 else "red" for b in balance_m.values], width=25
        )
    ...
    country_balance = (
        df_tb.groupby(["partner_country_name", "flow_type"])["trade_value_usd"].sum().unstack(fill_value=0)
    )
    ...
    for col, color, label in [("export_growth", "green", "Export Growth"), ("import_growth", "red", "Import Growth")]:
        if col in yearly_trade.columns:
            axes[1, 1].plot(yearly_trade.index, yearly_trade[col], marker="o", color=color, linewidth=2, label=label)
...
def cluster_trade_partners(df):
    ...
    partner_profile = (
        df.groupby("partner_country_name")
        .agg(
            total_trade=("trade_value_usd", "sum"),
            avg_monthly=("trade_value_usd", "mean"),
    ...
    cluster_feats = ["total_trade", "trade_volatility", "n_commodities", "export_share", "n_transport"]
    ...
    for c in range(optimal_k):
        ...
        axes[0, 0].scatter(
            partner_profile.loc[mask, "total_trade"] / 1e9,
    ...
    for _, row in partner_profile.iterrows():
        axes[0, 0].annotate(
            row["partner_country_name"][:8], (row["total_trade"] / 1e9, row["n_commodities"]), fontsize=6
        )
    ...
    sns.heatmap(
        cluster_norm.T, annot=True, fmt=".2f", cmap="YlOrRd", ax=axes[0, 1], cbar_kws={"label": "Normalized Value"}
    )
    ...
    axes[1, 0].boxplot(
        [partner_profile[partner_profile["cluster"] == c]["export_share"] for c in range(optimal_k)],
    ...
    axes[1, 1].bar(cluster_counts.index, cluster_counts.values, color=sns.color_palette("Set2", optimal_k))
    ...
    print(
        partner_profile.groupby("cluster")
        .agg(
...
def simulate_tariff_impact(df):
    ...
    current_trade = (
        df.groupby(["partner_country_name", "commodity_section", "flow_type"])
        .agg(trade_value=("trade_value_usd", "sum"), quantity=("quantity", "sum"))
        .reset_index()
    )
    ...
    axes[0, 0].bar(scenario_df["tariff_rate"], scenario_df["tariff_revenue"] / 1e9, color="steelblue")
    ...
    axes[0, 1].plot(scenario_df["tariff_rate"], scenario_df["import_change_pct"], "ro-", linewidth=2, markersize=8)
...
cluster_save = partner_clusters[
    ["partner_country_name", "cluster", "total_trade", "n_commodities", "export_share"]
].copy()
cluster_spark = spark.createDataFrame(cluster_save)
cluster_spark = cluster_spark.withColumn("scoring_date", current_date())

(cluster_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_partner_clusters"))
...
(tariff_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_tariff_impact_scenarios"))
