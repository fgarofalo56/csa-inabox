...
def prepare_modeling_data(crashes, highway):
    ...
    df['is_holiday_period'] = df.apply(
        lambda r: int(any(r['crash_month'] == m and abs(r['crash_day'] - d) <= 2
                         for m, d in holiday_months_days)), axis=1
    )
    ...
    state_prior = df.groupby(['state_fips', 'crash_year']).agg(
    ...
    state_prior.columns = ['state_fips', 'crash_year', 'prior_state_crashes',
                            'prior_state_avg_fatals', 'prior_state_dui_rate']
    ...
    if len(highway) > 0:
        hwy_state = highway.groupby('state_code').agg(
        ...
        hwy_state.columns = ['state_fips', 'state_avg_sufficiency',
                              'state_avg_deck_cond', 'state_pct_deficient']
...
def plot_feature_correlations():
    ...
    numeric_features = [
        'fatality_count', 'is_severe', 'posted_speed_limit', 'total_vehicles',
        'crash_hour', 'is_weekend', 'is_night', 'has_dui', 'has_pedestrian',
        'is_rural', 'weather_risk', 'light_risk', 'collision_severity_score',
        'high_speed', 'multi_vehicle', 'prior_state_avg_fatals'
    ]
...
def build_feature_matrix(df):
    ...
    feature_cols = [
        'posted_speed_limit', 'speed_limit_sq', 'vehicles_capped', 'multi_vehicle',
        'crash_hour_sin', 'crash_hour_cos', 'crash_month_sin', 'crash_month_cos',
        'is_weekend', 'is_night', 'is_holiday_period',
        'has_dui', 'has_pedestrian', 'is_rural',
        'high_speed', 'weather_risk', 'light_risk',
        'functional_class_risk', 'collision_severity_score',
        'prior_state_crashes', 'prior_state_avg_fatals', 'prior_state_dui_rate'
    ]
...
def train_classification_models(X, y):
    ...
    models = {
        ...
        'Random Forest': RandomForestClassifier(
            n_estimators=200, max_depth=12, min_samples_leaf=20, random_state=42
        ),
...
def train_regression_models(X, y):
    ...
    models = {
        ...
        'Random Forest': RandomForestRegressor(
            n_estimators=200, max_depth=10, min_samples_leaf=20, random_state=42
        ),
...
def analyze_feature_importance(clf_results, feature_names):
    ...
    best_name = 'Gradient Boosting' if 'Gradient Boosting' in clf_results else \
                max(clf_results.keys(), key=lambda k: clf_results[k]['f1'])
...
def generate_risk_scores(clf_results, df_model, feature_names):
    ...
    ax2.bar(risk_counts.index, risk_counts.values,
            color=[colors.get(c, '#95a5a6') for c in risk_counts.index], alpha=0.85)
    ...
    top_risk = state_features.nlargest(10, 'risk_score')[
        ['state_fips', 'risk_score', 'risk_category']
    ]
    print("\nTop 10 Highest Risk States:")
...
def save_results_to_delta(clf_comparison, reg_comparison, risk_scores, feature_importance):
    ...
    (clf_spark.write
     ...
     .saveAsTable("gold.gld_highway_safety_clf_metrics"))
    ...
    (reg_spark.write
     .mode("overwrite")
     .option("mergeSchema", "true")
     .saveAsTable("gold.gld_highway_safety_reg_metrics"))
    ...
    (risk_spark.write
     .mode("overwrite")
     .option("mergeSchema", "true")
     .saveAsTable("gold.gld_highway_risk_scores"))
