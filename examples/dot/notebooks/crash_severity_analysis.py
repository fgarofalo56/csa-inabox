...
def prepare_crash_data(df):
    ...
    weather_map = {1: 'Clear', 2: 'Rain', 3: 'Sleet/Hail', 4: 'Snow',
                   5: 'Fog', 6: 'Wind', 7: 'Blowing Sand', 8: 'Other',
    ...
    light_map = {1: 'Daylight', 2: 'Dark-Not Lighted', 3: 'Dark-Lighted',
    ...
    collision_map = {0: 'Not Collision', 1: 'Rear-End', 2: 'Head-On',
                     5: 'Angle', 6: 'Sideswipe-Same Dir', 7: 'Sideswipe-Opp Dir',
                     8: 'Rear-to-Rear', 9: 'Other'}
...
def plot_annual_trends():
    ...
    annual_stats = df_prepared.groupby('crash_year').agg({
        'case_id': 'count',
        'fatality_count': 'sum',
    ...
    annual_stats['fatalities_per_crash'] = (
        annual_stats['total_fatalities'] / annual_stats['total_crashes']
    ).round(3)
    ...
    ax1 = axes[0, 0]
    ax1.bar(annual_stats['crash_year'], annual_stats['total_fatalities'],
            color='#e74c3c', alpha=0.8)
    z = np.polyfit(annual_stats['crash_year'], annual_stats['total_fatalities'], 1)
    p = np.poly1d(z)
    ax1.plot(annual_stats['crash_year'], p(annual_stats['crash_year']),
    ...
    # DUI crashes trend
    ax2 = axes[0, 1]
    ax2.bar(annual_stats['crash_year'], annual_stats['dui_crashes'],
    ...
    # Pedestrian crashes
    ax3 = axes[1, 0]
    ax3.bar(annual_stats['crash_year'], annual_stats['pedestrian_crashes'],
    ...
    ax4 = axes[1, 1]
    ax4.plot(annual_stats['crash_year'], annual_stats['fatalities_per_crash'],
...
def plot_temporal_heatmap():
    ...
    plt.title('Fatal Crash Frequency by Day of Week and Hour',
...
def analyze_seasonality():
    ...
    monthly = df_prepared.groupby(['crash_year', 'crash_month']).agg({
        'case_id': 'count',
        'fatality_count': 'sum'
    }).rename(columns={'case_id': 'crashes', 'fatality_count': 'fatalities'}).reset_index()

    monthly_avg = monthly.groupby('crash_month').agg({
        'crashes': ['mean', 'std'],
        'fatalities': ['mean', 'std']
    }).round(1)
    ...
    bars = ax.bar(x, monthly_avg['avg_fatalities'], yerr=monthly_avg['std_fatalities'],
                  capsize=3, color='#e74c3c', alpha=0.8)
    ax.set_xticks(x)
    ax.set_xticklabels(monthly_avg['month_name'])
    ax.set_title('Average Monthly Traffic Fatalities (with Std Dev)',
...
def analyze_state_performance():
    ...
    state_stats = df_prepared.groupby('state_fips').agg({
        'case_id': 'count',
        'fatality_count': 'sum',
        'drunk_driver_count': lambda x: (x > 0).sum(),
        'pedestrians_involved': lambda x: (x > 0).sum(),
    ...
    ax2 = axes[1]
    scatter = ax2.scatter(state_stats['dui_rate'], state_stats['ped_rate'],
                          s=state_stats['total_fatalities'] / state_stats['total_fatalities'].max() * 500,
                          alpha=0.6, c=state_stats['total_fatalities'], cmap='YlOrRd')
...
def analyze_rural_urban():
    ...
    area_stats = df_prepared.groupby('area_type').agg({
        'case_id': 'count',
    ...
    area_stats.columns = ['total_crashes', 'total_fatalities', 'avg_fatalities',
                          'dui_crashes', 'avg_speed_limit', 'avg_vehicles', 'ped_crashes']
    ...
    # Crash distribution
    ax1 = axes[0]
    ax1.pie(area_stats['total_crashes'], labels=area_stats['area_type'],
...
def analyze_environmental_factors():
    ...
    weather_stats = df_prepared.groupby('weather_desc').agg({
        'case_id': 'count',
        'fatality_count': ['sum', 'mean'],
        'total_vehicles': 'mean'
    }).round(2)
    ...
    light_stats = df_prepared.groupby('light_desc').agg({
        'case_id': 'count',
        'fatality_count': ['sum', 'mean']
    }).round(2)
    ...
    # Light conditions
    ax2 = axes[1]
    colors_light = {'Daylight': '#f1c40f', 'Dark-Not Lighted': '#2c3e50',
                    'Dark-Lighted': '#7f8c8d', 'Dawn': '#e67e22', 'Dusk': '#e74c3c'}
...
def analyze_crash_characteristics():
    ...
    speed_stats = df_prepared.groupby('speed_category').agg({
        'case_id': 'count',
        'fatality_count': ['sum', 'mean']
    }).round(2)
    ...
    collision_stats = df_prepared.groupby('collision_type').agg({
        'case_id': 'count',
        'fatality_count': ['sum', 'mean']
    }).round(2)
    ...
    # Speed category
    ax1 = axes[0]
    speed_order = ['Low (≤30 mph)', 'Medium (31-45 mph)', 'Moderate (46-55 mph)',
                   'High (56-65 mph)', 'Very High (>65 mph)']
    ...
    ax1.bar(range(len(speed_sorted)), speed_sorted['crashes'],
            color=colors_speed[:len(speed_sorted)], alpha=0.8)
    ...
    # Add severity labels
    ax1_twin = ax1.twinx()
    ax1_twin.plot(range(len(speed_sorted)), speed_sorted['avg_fatalities'],
                  marker='D', color='darkred', linewidth=2, markersize=8, label='Avg Fatalities')
...
def analyze_dui_patterns():
    ...
    bars = ax1.bar(range(len(dui_time_sorted)), dui_time_sorted['dui_rate'],
    ...
    ax2 = axes[1]
    bars2 = ax2.bar(range(len(dui_dow)), dui_dow['dui_rate'],
                    color=['#e74c3c' if d in ('Friday', 'Saturday', 'Sunday') else '#3498db'
                           for d in dui_dow.index], alpha=0.8)
...
def perform_statistical_tests():
    ...
    valid_speed = df_prepared[(df_prepared['posted_speed_limit'] > 0) &
                              (df_prepared['posted_speed_limit'] < 100)]
    ...
    for test_name, result in results.items():
        ...
        for k, v in result.items():
            if isinstance(v, float):
                print(f"  {k}: {v:.4f}" + (" ***" if k == 'p_value' and v < 0.001 else
...
def save_results_to_delta():
    ...
    (state_spark.write
     ...
     .saveAsTable("gold.gld_crash_severity_by_state"))
    ...
    (monthly_spark.write
     .mode("overwrite")
     .option("mergeSchema", "true")
     .saveAsTable("gold.gld_crash_monthly_patterns"))
