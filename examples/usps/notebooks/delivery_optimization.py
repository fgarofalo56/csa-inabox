...
def load_volume_data():
    ...
    volume_df = volume_df[
        (volume_df['facility_id'].notna()) &
        (volume_df['volume_date'].notna()) &
        (volume_df['is_valid_record'] == True)
    ].copy()
...
def prepare_delivery_data(df):
    ...
    # On-time flag
    df_clean['is_on_time'] = (
        df_clean['actual_delivery_date'] <= df_clean['expected_delivery_date']
    ).astype(int)
    ...
    df_clean['is_interstate'] = (
        df_clean['origin_state'] != df_clean['destination_state']
    ).astype(int)
    ...
    df_clean['is_cross_region'] = (
        df_clean['origin_region'] != df_clean['dest_region']
    ).astype(int)
...
def analyze_on_time_by_product():
    ...
    product_stats = delivered.groupby('product_class').agg(
    ...
    product_stats['on_time_rate'] = (
        product_stats['on_time_count'] / product_stats['total_deliveries'] * 100
    ).round(1)
    ...
    ax1 = axes[0]
    colors = ['#27ae60' if r >= 90 else '#f39c12' if r >= 80 else '#e74c3c'
              for r in product_stats['on_time_rate']]
    bars = ax1.bar(range(len(product_stats)), product_stats['on_time_rate'],
    ...
    ax2 = axes[1]
    ax2.bar(range(len(product_stats)), product_stats['avg_delivery_days'],
    ...
    ax3 = axes[2]
    ax3.bar(range(len(product_stats)), product_stats['avg_days_late'],
...
def analyze_on_time_by_geography():
    ...
    state_stats = delivered.groupby('destination_state').agg(
    ...
    colors = ['#27ae60' if r >= 90 else '#f39c12' if r >= 80 else '#e74c3c'
              for r in top_bottom['on_time_rate']]
    ...
    ax2 = axes[1]
    interstate_stats = delivered.groupby('is_interstate').agg(
        on_time_rate=('is_on_time', 'mean'),
        avg_days=('delivery_time_days', 'mean'),
        count=('tracking_id', 'count')
    ).round(3).reset_index()
    interstate_stats['label'] = interstate_stats['is_interstate'].map(
        {0: 'Intrastate', 1: 'Interstate'}
    )

    x = range(len(interstate_stats))
    bars = ax2.bar(x, interstate_stats['on_time_rate'] * 100,
                   color=['#3498db', '#e67e22'], alpha=0.85)
    ax2.set_xticks(x)
...
def analyze_facility_utilization():
    ...
    type_stats = fac.groupby('facility_type').agg(
    ...
    # Utilization by facility type
    ax1 = axes[0, 0]
    colors = ['#27ae60' if u < 70 else '#f39c12' if u < 85 else '#e74c3c'
              for u in type_stats['avg_utilization']]
...
def analyze_service_compliance():
    ...
    delivered['meets_standard'] = (
        delivered['delivery_time_days'] <= delivered['service_standard_days']
    ).astype(int)

    compliance_by_product = delivered.groupby('product_class').agg(
        total=('tracking_id', 'count'),
    ...
    ax2 = axes[1]
    width = 0.35
    ax2.bar([i - width/2 for i in x], compliance_by_product['avg_service_standard'],
            width, label='Service Standard', color='#27ae60', alpha=0.85)
    ax2.bar([i + width/2 for i in x], compliance_by_product['avg_delivery_days'],
            width, label='Actual Avg', color='#e74c3c', alpha=0.85)
    ax2.set_xticks(x)
