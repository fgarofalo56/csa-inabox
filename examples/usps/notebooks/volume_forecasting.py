...
def create_volume_features(df, facility_info):
    ...
    df_ml["days_to_holiday"] = (
        df_ml["volume_date"]
        .apply(lambda d: min(abs((d - h).days) for h in holidays_2023) if pd.notna(d) else 30)
        .clip(upper=30)
    )
    ...
    if "volume_prior_year_same_day" in df_ml.columns:
        df_ml["yoy_change_pct"] = (
            (
                ...
                / df_ml["volume_prior_year_same_day"].replace(0, np.nan)
                * 100
            )
            .fillna(0)
            .clip(-200, 200)
    ...
    if "volume_prior_week" in df_ml.columns:
        df_ml["wow_change_pct"] = (
            ((df_ml["total_pieces"] - df_ml["volume_prior_week"]) / df_ml["volume_prior_week"].replace(0, np.nan) * 100)
            .fillna(0)
            .clip(-200, 200)
        )
...
def plot_feature_analysis():
    ...
    r2, _ = stats.pearsonr(
        df_ml["volume_rolling_mean_7"].dropna(), df_ml.loc[df_ml["volume_rolling_mean_7"].notna(), "total_pieces"]
    )
...
def build_feature_matrix(df):
    ...
    feature_cols = [
        c for c in df.columns if c not in exclude_cols and df[c].dtype in ["int64", "float64", "uint8", "bool"]
    ]
...
def train_forecasting_models(X, y, df_ml):
    ...
    models = {
        ...
        "Random Forest": RandomForestRegressor(n_estimators=200, max_depth=15, min_samples_leaf=10, random_state=42),
    ...
    for name, model in models.items():
        ...
        with mlflow.start_run(run_name=f"volume_{name.lower().replace(' ', '_')}"):
            ...
            cv_scores = cross_val_score(model, X_tr, y_train, cv=tscv, scoring="neg_mean_absolute_error")
...
def generate_forecasts(model_results, df_ml, scaler):
    ...
    forecast_results["predicted_change_pct"] = (
        (predictions - latest["total_pieces"].values) / latest["total_pieces"].values.clip(1) * 100
    ).round(2)
...
def visualize_forecasts(forecast_results):
    ...
    ax1 = axes[0]
    pc_forecast = (
        forecast_results.groupby("product_class")
        .agg(avg_predicted=("predicted_volume", "mean"), avg_actual=("actual_latest_volume", "mean"))
        .reset_index()
    )
    ...
    width = 0.35
    ax1.bar(
        [i - width / 2 for i in x], pc_forecast["avg_actual"], width, label="Latest Actual", color="#3498db", alpha=0.85
    )
    ax1.bar(
        [i + width / 2 for i in x], pc_forecast["avg_predicted"], width, label="Forecast", color="#e67e22", alpha=0.85
    )
    ax1.set_xticks(x)
...
def save_results_to_delta(forecast_results, comparison_results, feature_importance):
    ...
    (forecast_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_mail_volume_forecasts"))
    ...
    (metrics_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_volume_model_metrics"))
