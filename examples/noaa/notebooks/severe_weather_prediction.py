...
def load_weather_data():
    ...
    weather_df = weather_df[(weather_df["station_id"].notna()) & (weather_df["is_valid_record"] == True)].copy()
...
def prepare_storm_features(storms, weather):
    ...
    high_risk_events = ["TORNADO", "HURRICANE", "TROPICAL STORM", "ICE STORM", "BLIZZARD", "FLASH FLOOD", "WILDFIRE"]
    medium_risk_events = ["THUNDERSTORM WIND", "HAIL", "WINTER STORM", "FLOOD", "HIGH WIND", "HEAVY RAIN"]
    ...
    if len(weather) > 0:
        ...
        state_monthly_wx = (
            weather.groupby(["state_code", "month"])
            .agg(wx_avg_tmax=("TMAX", "mean"), wx_avg_prcp=("PRCP", "mean"))
            .reset_index()
        )
...
def analyze_feature_importance(results, feature_names):
    ...
    best = (
        "Gradient Boosting" if "Gradient Boosting" in results else max(results.keys(), key=lambda k: results[k]["f1"])
    )
...
def train_damage_model(X, y):
    ...
    models = {
        ...
        "Random Forest": RandomForestRegressor(n_estimators=200, max_depth=10, min_samples_leaf=10, random_state=42),
    ...
    for name, model in models.items():
        ...
        with mlflow.start_run(run_name=f"damage_{name.lower().replace(' ', '_')}"):
            ...
            results[name] = {"model": model, "test_mae": mae, "test_rmse": rmse, "test_r2": r2, "predictions": y_pred}
...
def save_results_to_delta(clf_results, reg_results, feature_importance):
    ...
    (clf_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_weather_severity_metrics"))
    ...
    (reg_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_weather_damage_metrics"))
...
print(f"  Significant events: {df_model['is_significant'].sum():,} ({df_model['is_significant'].mean() * 100:.1f}%)")
