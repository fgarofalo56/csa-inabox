...
def load_storm_data():
    ...
    storm_df = storm_df[
        (storm_df["event_id"].notna()) & (storm_df["begin_date"].notna()) & (storm_df["is_valid_record"] == True)
    ].copy()
...
def load_buoy_data():
    ...
    buoy_df = buoy_df[
        (buoy_df["station_id"].notna()) & (buoy_df["observation_date"].notna()) & (buoy_df["is_valid_record"] == True)
    ].copy()
...
def analyze_temperature_trends():
    ...
    monthly_avg = (
        df_prepared.groupby(["year", "month"])
        .agg(
            avg_temp=("TAVG", "mean"),
            max_temp=("TMAX", "max"),
            min_temp=("TMIN", "min"),
    ...
    ax2.bar(range(12), month_avg["mean"], yerr=month_avg["std"], capsize=3, color="#3498db", alpha=0.85)
    ...
    ax3 = axes[1, 0]
    ax3.fill_between(annual_avg["year"], annual_avg["avg_tmin"], annual_avg["avg_tmax"], alpha=0.3, color="#e74c3c")
...
def analyze_precipitation():
    ...
    ax2.bar(
        range(len(seasonal_sorted)),
        seasonal_sorted["avg_daily_precip"],
        color=["#3498db", "#2ecc71", "#f1c40f", "#e67e22"],
        alpha=0.85,
    )
...
def analyze_storm_events():
    ...
    storms["total_casualties"] = (
        storms["injuries_direct"].fillna(0)
    ...
    month_names = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
    ax2.bar(range(12), [monthly_events.get(m + 1, 0) for m in range(12)], color="#c0392b", alpha=0.85)
    ax2.set_xticks(range(12))
    ax2.set_xticklabels(month_names)
    ...
    ax3.barh(range(len(top_damage)), top_damage["total_damage"] / 1e6, color="#8e44ad", alpha=0.85)
    ...
    ax4.barh(range(len(top_casualty)), top_casualty["total_casualties"], color="#e74c3c", alpha=0.85)
...
def analyze_buoy_data():
    ...
    if "sea_surface_temp_c" in buoys.columns:
        ...
        for sid in valid_sst["station_id"].unique()[:5]:
            ...
            ax1.plot(subset["observation_date"], subset["sea_surface_temp_c"], label=sid, linewidth=1.5, alpha=0.8)
    ...
    if "pressure_hpa" in buoys.columns:
        ...
        for sid in valid_pressure["station_id"].unique()[:3]:
            ...
            ax4.plot(subset["observation_date"], subset["pressure_hpa"], label=sid, linewidth=1, alpha=0.7)
...
def perform_statistical_tests():
    ...
    if "PRCP" in df_prepared.columns:
        ...
        if len(annual_prcp) > 2:
            slope, intercept, r_val, p_val, std_err = stats.linregress(annual_prcp["year"], annual_prcp["PRCP"])
            results["Precipitation Trend"] = {"slope_per_year": slope, "r_squared": r_val**2, "p_value": p_val}
...
def save_results_to_delta():
    ...
    if annual_temp is not None:
        ...
        (
            temp_spark.write.mode("overwrite")
    ...
    if storm_stats is not None:
        ...
        (storm_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_storm_event_summary"))
