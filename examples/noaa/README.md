...
> [!IMPORTANT]
> **Cost-safety:** this vertical deploys real Azure resources. Always run `teardown.sh` when you are done. A forgotten workshop environment can run **$150-250/day**.
...
| ---------- | ---------------- | ----- |
| `gld_climate_trends` | TODO: capture after first run | Populated from Silver via dbt `--select tag:gold` |
| `gld_marine_ecosystem_health` | TODO: capture after first run | Populated from Silver via dbt `--select tag:gold` |
| `gld_severe_weather_risk` | TODO: capture after first run | Populated from Silver via dbt `--select tag:gold` |
