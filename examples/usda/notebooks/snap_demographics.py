...
def prepare_snap_analysis_data(snap_data, demographics):
    ...
    monthly_state = (
        snap_data.groupby(["state_code", "enrollment_date"])
        .agg(
            {
    ...
    monthly_state["enrollment_yoy_change"] = (
        monthly_state.groupby("state_code")["current_enrollment"].pct_change(periods=12) * 100
    )
    monthly_state["benefits_yoy_change"] = (
        monthly_state.groupby("state_code")["current_benefits_dollars"].pct_change(periods=12) * 100
    )

    # Calculate moving averages for trend analysis
    monthly_state["enrollment_6mo_avg"] = (
        monthly_state.groupby("state_code")["current_enrollment"]
        .rolling(window=6, min_periods=3)
        .mean()
        .reset_index(drop=True)
    )

    monthly_state["benefits_6mo_avg"] = (
        monthly_state.groupby("state_code")["current_benefits_dollars"]
        .rolling(window=6, min_periods=3)
        .mean()
        .reset_index(drop=True)
    )
...
def analyze_national_trends():
    ...
    national_trends = (
        df_analysis.groupby("enrollment_date")
        .agg({"current_enrollment": "sum", "current_benefits_dollars": "sum", "current_households": "sum"})
        .reset_index()
    )

    # Calculate rates
    national_trends["avg_benefit_per_person"] = (
        national_trends["current_benefits_dollars"] / national_trends["current_enrollment"]
    )
    ...
    # 1. Enrollment trend
    axes[0, 0].plot(
        national_trends["enrollment_date"], national_trends["current_enrollment"] / 1e6, linewidth=2, color="darkblue"
    )
    ...
    p = np.poly1d(z)
    axes[0, 0].plot(national_trends["enrollment_date"], p(x_numeric), "--", alpha=0.7, color="red", label="Trend")
    axes[0, 0].legend()

    # 2. Benefits trend
    axes[0, 1].plot(
        national_trends["enrollment_date"],
        national_trends["current_benefits_dollars"] / 1e9,
        linewidth=2,
        color="darkgreen",
    )
    ...
    axes[1, 0].bar(
        valid_yoy["enrollment_date"], valid_yoy["enrollment_yoy"], alpha=0.7, color="steelblue", label="Enrollment"
    )
...
def analyze_state_patterns():
    ...
    axes[1].scatter(
        latest_state_data["poverty_rate"], latest_state_data["enrollment_per_1000"], alpha=0.7, s=60, color="steelblue"
    )
    ...
    p = np.poly1d(z)
    axes[1].plot(
        latest_state_data["poverty_rate"], p(latest_state_data["poverty_rate"]), "r--", alpha=0.8, label="Trend"
    )
...
def analyze_economic_correlations():
    ...
    snap_vars = ["enrollment_per_1000", "benefits_per_capita", "avg_benefits_per_person"]
    ...
    plt.title("Correlation Matrix: SNAP Metrics vs Demographic/Economic Factors", fontsize=14, fontweight="bold")
    plt.tight_layout()
...
def analyze_time_series_patterns():
    ...
    for state in high_poverty_states:
        ...
        axes[0, 0].plot(state_data["enrollment_date"], state_data["enrollment_per_1000"], label=state, linewidth=2)
    ...
    for state in low_poverty_states:
        state_data = df_analysis[df_analysis["state_code"] == state]
        axes[0, 1].plot(state_data["enrollment_date"], state_data["enrollment_per_1000"], label=state, linewidth=2)
    ...
    axes[1, 0].plot(
        high_poverty_avg.index, high_poverty_avg.values, label="High Poverty States", linewidth=3, color="red"
    )
    axes[1, 0].plot(
        low_poverty_avg.index, low_poverty_avg.values, label="Low Poverty States", linewidth=3, color="blue"
    )
    ...
    axes[1, 1].scatter(volatility_df["poverty_rate"], volatility_df["cv"], alpha=0.7, s=60, color="purple")
...
def analyze_regional_patterns():
    ...
    regional_stats = (
        df_analysis.groupby(["region", "enrollment_date"])
        .agg({"current_enrollment": "sum", "current_benefits_dollars": "sum", "population": "sum"})
        .reset_index()
    )

    regional_stats["enrollment_rate"] = (regional_stats["current_enrollment"] / regional_stats["population"]) * 1000

    # Latest regional comparison
    latest_regional = regional_stats[regional_stats["enrollment_date"] == regional_stats["enrollment_date"].max()]
    ...
    bars = axes[0, 0].bar(
        latest_regional["region"], latest_regional["enrollment_rate"], color=["skyblue", "lightgreen", "coral", "gold"]
    )
    ...
    for region in regional_stats["region"].unique():
        if region != "Other":
            ...
            axes[0, 1].plot(
                region_data["enrollment_date"],
                region_data["enrollment_rate"],
                label=region,
                linewidth=2,
                marker="o",
                markersize=4,
    ...
    regional_demographics = (
        ...
        .agg({"poverty_rate": "mean", "unemployment_rate": "mean", "median_income": "mean", "rural_percent": "mean"})
        .round(1)
    )

    # Heatmap of regional characteristics
    sns.heatmap(regional_demographics.T, annot=True, cmap="RdYlBu_r", ax=axes[1, 1], cbar_kws={"label": "Value"})
...
def analyze_policy_impacts():
    ...
    effectiveness_data = (
        df_analysis.groupby("state_code")
        .agg(
            {
    ...
    effectiveness_data["benefit_adequacy"] = (
        (effectiveness_data["avg_benefits_per_person"] * 12) / effectiveness_data["median_income"] * 100
    )
    ...
    print(
        high_reach_states[["state_code", "program_reach", "poverty_rate", "enrollment_per_1000"]]
        .round(2)
    ...
    print(
        low_reach_states[["state_code", "program_reach", "poverty_rate", "enrollment_per_1000"]]
        .round(2)
        .to_string(index=False)
    )
    ...
    axes[0, 0].axvline(
        effectiveness_data["program_reach"].mean(),
        color="red",
        linestyle="--",
    ...
    axes[0, 1].scatter(
        effectiveness_data["under_18_percent"], effectiveness_data["benefit_adequacy"], alpha=0.7, s=60, color="green"
    )
    ...
    axes[1, 0].scatter(
        effectiveness_data["median_income"] / 1000, effectiveness_data["program_reach"], alpha=0.7, s=60, color="purple"
    )
    ...
    axes[1, 1].scatter(
        effectiveness_data["program_reach"], effectiveness_data["benefit_adequacy"], alpha=0.7, s=60, color="orange"
    )
    ...
    if len(efficient_states) > 0:
        axes[1, 1].scatter(
            efficient_states["program_reach"],
            efficient_states["benefit_adequacy"],
            color="red",
            s=100,
            alpha=0.8,
...
def save_analysis_to_delta(df_analysis, effectiveness_data, regional_stats):
    ...
    (
        analysis_spark_df.write.mode("overwrite")
    ...
    (
        effectiveness_spark_df.write.mode("overwrite")
        .option("mergeSchema", "true")
    ...
    (
        regional_spark_df.write.mode("overwrite")
        .option("mergeSchema", "true")
...
print(
    f"   • Analysis period: {df_analysis['enrollment_date'].min().strftime('%Y-%m-%d')} to {df_analysis['enrollment_date'].max().strftime('%Y-%m-%d')}"
)
...
print(
    f"   • Highest enrollment rate: {highest_enrollment_state['state_code']} ({highest_enrollment_state['enrollment_per_1000']:.1f} per 1000)"
)
print(
    f"   • Lowest enrollment rate: {lowest_enrollment_state['state_code']} ({lowest_enrollment_state['enrollment_per_1000']:.1f} per 1000)"
)
