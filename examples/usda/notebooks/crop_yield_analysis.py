...
def prepare_data(df):
    ...
    def remove_outliers(group):
        ...
        return group[(group["yield_per_acre"] >= lower_bound) & (group["yield_per_acre"] <= upper_bound)]
    ...
    df_clean["yield_3yr_rolling"] = (
        df_clean.groupby(["commodity", "state_code"])["yield_per_acre"]
        .rolling(window=3, min_periods=2)
        .mean()
        .reset_index(drop=True)
    )

    df_clean["yield_5yr_rolling"] = (
        df_clean.groupby(["commodity", "state_code"])["yield_per_acre"]
    ...
    df_clean["yield_trend"] = (
        df_clean.groupby(["commodity", "state_code"]).apply(calculate_trend).reset_index(drop=True)
    )
...
def plot_commodity_trends():
    ...
    for i, commodity in enumerate(commodities):
        if i < len(axes):
            ...
            axes[i].plot(national_avg["year"], national_avg["yield_per_acre"], linewidth=3, label="National Average")

            # Add trend line
            if len(national_avg) > 2:
                z = np.polyfit(national_avg["year"], national_avg["yield_per_acre"], 1)
                p = np.poly1d(z)
                axes[i].plot(national_avg["year"], p(national_avg["year"]), "--", alpha=0.7, color="red", label="Trend")
...
def analyze_state_performance():
    ...
    for i, crop in enumerate(major_crops):
        ...
        for j, (_idx, row) in enumerate(crop_data.iterrows()):
            axes[i].text(
                row["yield_per_acre"] + max(crop_data["yield_per_acre"]) * 0.01,
                j,
                f"{row['yield_per_acre']:.1f}",
                va="center",
                fontweight="bold",
            )

        axes[i].set_title(f"Top 10 States - {crop} Yield ({latest_year})", fontsize=14, fontweight="bold")
...
def analyze_yield_volatility():
    ...
    volatility_data = (
        df_prepared.groupby(["commodity", "state_code"]).agg({"yield_per_acre": ["mean", "std", "count"]}).round(2)
    )
    ...
    sns.heatmap(pivot_cv, annot=True, cmap="RdYlBu_r", center=15, cbar_kws={"label": "Coefficient of Variation (%)"})
    plt.title(
        "Crop Yield Volatility by State and Commodity\n(Coefficient of Variation)", fontsize=16, fontweight="bold"
    )
...
def create_ml_features(df):
    ...
    # Rolling statistics
    for window in [3, 5]:
        df_ml[f"yield_rolling_mean_{window}"] = (
            df_ml.groupby(["commodity", "state_code"])["yield_per_acre"]
            .rolling(window=window, min_periods=2)
            .mean()
            .reset_index(drop=True)
        )

        df_ml[f"yield_rolling_std_{window}"] = (
            df_ml.groupby(["commodity", "state_code"])["yield_per_acre"]
            .rolling(window=window, min_periods=2)
            .std()
            .reset_index(drop=True)
...
def train_forecasting_models(df_ml):
    ...
    models = {
        ...
        "Polynomial Ridge": Pipeline(
            [("poly", PolynomialFeatures(degree=2, include_bias=False)), ("ridge", Ridge(alpha=1.0))]
        ),
    }
    ...
    for name, model in models.items():
        ...
        with mlflow.start_run(run_name=f"crop_yield_{name.lower().replace(' ', '_')}"):
            ...
            cv_scores = cross_val_score(
                model,
                X_train_scaled if name != "Polynomial Ridge" else X_train,
                y_train,
                cv=5,
...
def analyze_feature_importance():
    ...
    importance_df = pd.DataFrame({"feature": feature_cols, "importance": rf_model.feature_importances_}).sort_values(
...
def generate_forecasts(model_results, df_ml, scaler):
    ...
    forecast_data["year_normalized"] = (forecast_year - df_ml["year"].min()) / (
        df_ml["year"].max() - df_ml["year"].min()
    )
    ...
    forecast_results["predicted_change"] = (
        (predictions - forecast_data["yield_per_acre"]) / forecast_data["yield_per_acre"] * 100
    ).round(2)
...
def visualize_forecasts(forecast_results):
    ...
    for i, commodity in enumerate(commodities):
        if i < len(axes):
            ...
            bars = axes[i].bar(
                range(len(commodity_forecasts)), commodity_forecasts["predicted_change"], color=colors, alpha=0.7
            )
...
def save_forecasts_to_delta(forecast_results):
    ...
    # Add metadata
    forecast_df = (
        forecast_df.withColumn("forecast_date", current_date())
        .withColumn("model_used", lit("Random Forest"))
        .withColumn("confidence_level", lit(95))
    )

    # Write to Delta table
    (forecast_df.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_crop_yield_forecasts"))
...
def save_model_metrics(comparison_results):
    ...
    (spark_df.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_model_performance_metrics"))
...
print(
    f"   • Positive forecasts: {positive_forecasts}/{total_forecasts} ({positive_forecasts / total_forecasts * 100:.1f}%)"
)
...
forecast_summary = (
    forecast_results.groupby("commodity").agg({"predicted_change": ["mean", "std", "min", "max"]}).round(2)
)
