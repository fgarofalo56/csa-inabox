...
def load_air_quality():
    ...
    aq_df = aq_df[
        (aq_df["site_id"].notna()) & (aq_df["parameter_name"].notna()) & (aq_df["is_valid_record"] == True)
    ].copy()
...
def load_water_systems():
    ...
    water_df = water_df[(water_df["pwsid"].notna()) & (water_df["is_valid_record"] == True)].copy()
...
def load_toxic_releases():
    ...
    tri_df = tri_df[(tri_df["trifid"].notna()) & (tri_df["is_valid_record"] == True)].copy()
...
def analyze_air_quality_disparities():
    ...
    state_aqi = (
        aq_filtered.groupby(["state_name", "parameter_name"])
        .agg(
            avg_aqi=("aqi", "mean"),
            max_aqi=("aqi", "max"),
    ...
    # CBSA-level analysis
    cbsa_aqi = (
        aq_filtered.groupby(["cbsa_name", "parameter_name"])
        .agg(avg_aqi=("aqi", "mean"), max_aqi=("aqi", "max"), avg_concentration=("arithmetic_mean", "mean"))
        .round(2)
        .reset_index()
    )
    ...
    colors = ["#e74c3c" if a > 50 else "#f39c12" if a > 35 else "#27ae60" for a in pm25_state["avg_aqi"]]
...
def analyze_water_violations():
    ...
    contam_stats = (
        water.groupby("contaminant_name")
        .agg(
    ...
    ax3.bar(
        ...
        type_stats["violations"],
        color=["#3498db", "#e67e22", "#27ae60"][: len(type_stats)],
        alpha=0.85,
    )
    ...
    colors = ["#e74c3c" if r > 3 else "#f39c12" if r > 1.5 else "#27ae60" for r in top_rate["violations_per_system"]]
...
def analyze_toxic_releases():
    ...
    chem_stats = (
        ...
        .sort_values("total_releases", ascending=False)
        .reset_index()
    )
    ...
    ax1.barh(range(len(top_states)), top_states["total_releases_lbs"] / 1e6, color="#e74c3c", alpha=0.85)
    ax1.set_yticks(range(len(top_states)))
    ...
    ax3.barh(range(len(top_chems)), top_chems["total_releases"] / 1e6, color=colors_chem, alpha=0.85)
...
def analyze_cumulative_burden():
    ...
    # Air quality burden
    air_burden = df_air.groupby("state_code").agg(avg_aqi=("aqi", "mean"), max_aqi=("aqi", "max")).reset_index()
    air_burden.columns = ["state", "air_avg_aqi", "air_max_aqi"]

    # Water burden
    water_burden = (
        df_water.groupby("state_code")
        .agg(water_violations=("violation_id", "nunique"), water_systems=("pwsid", "nunique"))
        .reset_index()
    )
    water_burden["violations_per_system"] = (water_burden["water_violations"] / water_burden["water_systems"]).round(2)
    water_burden.columns = ["state", "water_violations", "water_systems", "water_viol_rate"]

    # TRI burden
    tri_burden = (
        df_tri.groupby("state")
        .agg(tri_facilities=("trifid", "nunique"), tri_total_releases=("total_releases", "sum"))
        .reset_index()
    )
    ...
    ax2.bar(
        x,
        top10.get("water_viol_rate_norm", 0),
    ...
    ax2.bar(
        x, top10.get("tri_total_releases_norm", 0), bottom=bottom2, label="Toxic Releases", color="#27ae60", alpha=0.85
    )
    ax2.set_xticks(x)
...
def perform_statistical_tests():
    ...
    if len(df_tri) > 5:
        state_data = (
            df_tri.groupby("state")
            .agg(facilities=("trifid", "nunique"), releases=("total_releases", "sum"))
            .reset_index()
        )
        if len(state_data) > 2:
...
def save_results_to_delta():
    ...
    (aq_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_air_quality_by_state"))

    # Water violations
    ws_spark = spark.createDataFrame(water_state_stats)
    ws_spark = ws_spark.withColumn("analysis_date", current_date())
    (ws_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_water_violations_by_state"))

    # TRI statistics
    tri_spark = spark.createDataFrame(tri_state_stats)
    tri_spark = tri_spark.withColumn("analysis_date", current_date())
    (tri_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_toxic_releases_by_state"))

    # Environmental burden
    burden_spark = spark.createDataFrame(burden_scores)
    burden_spark = burden_spark.withColumn("analysis_date", current_date())
    (
        burden_spark.write.mode("overwrite")
