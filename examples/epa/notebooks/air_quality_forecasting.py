...
def create_aqi_features(df, tri_context):
    ...
    df_ml["max_to_mean_ratio"] = (
        (df_ml["first_max_value"] / df_ml["arithmetic_mean"].replace(0, np.nan)).fillna(1).clip(0, 10)
    )
    ...
    # EWM
    df_ml["aqi_ewm_7"] = df_ml.groupby(group_cols)["aqi"].transform(lambda x: x.ewm(span=7, min_periods=2).mean())

    # --- Industrial context ---
    df_ml = df_ml.merge(tri_context, left_on="state_name", right_on="state", how="left", suffixes=("", "_tri"))
...
def train_exceedance_models(X, y, df_ml):
    ...
    models = {
        ...
        "Random Forest": RandomForestClassifier(n_estimators=200, max_depth=12, min_samples_leaf=10, random_state=42),
    ...
    for name, model in models.items():
        ...
        with mlflow.start_run(run_name=f"exceedance_{name.lower().replace(' ', '_')}"):
            ...
            results[name] = {"model": model, "f1": f1, "auc": auc, "y_pred": y_pred, "y_prob": y_prob}
...
def analyze_feature_importance(reg_results, feature_names):
    ...
    imp_df = pd.DataFrame({"feature": feature_names, "importance": model.feature_importances_}).sort_values(
...
def save_results_to_delta():
    ...
    (reg_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_aqi_regression_metrics"))
    ...
    (cls_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_aqi_exceedance_metrics"))

    # Feature importance
    if feature_importance is not None:
        fi_spark = spark.createDataFrame(feature_importance)
        fi_spark = fi_spark.withColumn("evaluation_date", current_date())
        (fi_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_aqi_feature_importance"))
...
if len(aqi_daily) >= 365:
    ...
    spark_decomp.write.mode("overwrite").saveAsTable("gold.gld_aqi_seasonal_decomposition")
