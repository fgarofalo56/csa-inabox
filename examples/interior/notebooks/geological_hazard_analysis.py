...
def cluster_seismic_zones(df):
    ...
    # Spatial cluster map
    scatter = axes[0].scatter(df['longitude'], df['latitude'], c=df['cluster'],
    ...
    if len(cluster_sizes) > 0:
        axes[1].bar(range(min(len(cluster_sizes), 20)),
                   cluster_sizes.values[:20], color='steelblue', alpha=0.7)
...
def calculate_recurrence_intervals(df):
        ...
        if n_events > 1:
            ...
            recurrence_data.append({
                ...
                'cv_inter_event': round(inter_event_days.std() / inter_event_days.mean(), 3)
                    if inter_event_days.mean() > 0 else np.nan
            })
    ...
    axes[0, 0].semilogy(recurrence_df['magnitude_threshold'],
                        recurrence_df['recurrence_years'], 'bo-', linewidth=2)
    ...
    axes[0, 1].semilogy(recurrence_df['magnitude_threshold'],
                        recurrence_df['annual_rate'], 'rs-', linewidth=2)
    ...
    if len(events_m4) > 2:
        ...
        axes[1, 0].axvline(x=inter_m4.mean(), color='red', linestyle='--',
                          label=f'Mean: {inter_m4.mean():.0f} days')
        axes[1, 0].axvline(x=inter_m4.median(), color='blue', linestyle='--',
                          label=f'Median: {inter_m4.median():.0f} days')
    ...
    axes[1, 1].bar(valid_cv['magnitude_threshold'], valid_cv['cv_inter_event'],
...
def probabilistic_hazard_assessment(df, recurrence_df, cluster_stats):
    ...
    if len(psha_df) > 0:
        prob_matrix = psha_df.pivot(index='magnitude', columns='time_window_years',
                                    values='exceedance_probability')
    ...
    for mag in mag_thresholds:
        ...
        if len(subset) > 0:
            axes[0, 1].plot(subset['time_window_years'],
                          subset['exceedance_probability'],
                          'o-', label=f'M >= {mag}', linewidth=2)
    ...
    density, _, _ = np.histogram2d(df['latitude'], df['longitude'],
                                    bins=[lat_bins, lon_bins])

    im = axes[1, 1].imshow(density.T, origin='lower', aspect='auto', cmap='hot',
...
def compute_spatial_autocorrelation(df, cluster_stats):
    ...
    grid_counts = df_copy.groupby(['lat_bin', 'lon_bin']).agg(
    ...
    if len(x_dev) > 2:
        ...
        axes[0, 0].plot(x_line, slope_m * x_line + intercept_m, 'r-',
                       label=f"Slope (Moran's I) = {slope_m:.3f}")
    ...
    quadrant = np.where((x_dev > 0) & (spatial_lag > 0), 'HH',
              np.where((x_dev > 0) & (spatial_lag < 0), 'HL',
              np.where((x_dev < 0) & (spatial_lag > 0), 'LH', 'LL')))
    ...
    axes[0, 1].bar(quad_counts.index, quad_counts.values,
                  color=[quad_colors.get(q, 'gray') for q in quad_counts.index])
    ...
    axes[1, 0].scatter(grid_counts['lon_bin'], grid_counts['lat_bin'],
                      c=colors_plot, s=grid_counts['n_events'] * 2, alpha=0.7)
    ...
    axes[1, 1].barh(
        ...
        sorted_grid['n_events'], color='coral', alpha=0.7
    )
    ...
    return {'morans_i': morans_i, 'expected_i': expected_i,
            'z_score': z_score, 'grid_counts': grid_counts}
...
def analyze_fault_proximity(df, cluster_stats):
    ...
    for idx, (cluster_id, row) in enumerate(top_clusters.iterrows()):
        ...
        axes[idx].scatter(lons, lats, c=cluster_events['magnitude'],
                         cmap='hot_r', s=cluster_events['magnitude']**2 * 5, alpha=0.6)
        lon_range = np.linspace(lons.min(), lons.max(), 100)
        axes[idx].plot(lon_range, slope * lon_range + intercept, 'b--', linewidth=2,
    ...
    if len(fault_df) > 0:
        ...
        axes2[0].scatter(fault_df['fault_width_km'], fault_df['max_magnitude'],
                        s=fault_df['n_events'], alpha=0.7, c='darkred')
        for _, row in fault_df.iterrows():
            axes2[0].annotate(f"C{int(row['cluster_id'])}",
                            (row['fault_width_km'], row['max_magnitude']), fontsize=8)
        ...
        axes2[1].barh([f"C{int(c)}" for c in fault_df['cluster_id']],
                     fault_df['r_squared'], color='teal', alpha=0.7)
...
def create_risk_map(df, cluster_stats, recurrence_df, spatial_results):
    ...
    grid_risk = df_copy.groupby(['lat_grid', 'lon_grid']).agg(
        n_events=('event_id', 'count'),
        max_magnitude=('magnitude', 'max'),
        avg_magnitude=('magnitude', 'mean'),
        avg_depth=('depth_km', 'mean'),
    ...
    grid_risk['depth_risk_norm'] = (
        (1 - grid_risk['avg_depth'] / max(depth_max, 1)) * 100
    ...
    for level, color in risk_colors.items():
        ...
        axes[0, 0].scatter(subset['lon_grid'], subset['lat_grid'],
                          c=color, s=subset['composite_risk'] * 3,
                          alpha=0.7, label=level, edgecolors='black', linewidth=0.5)
    ...
    axes[0, 1].hist(grid_risk['composite_risk'], bins=30, edgecolor='black',
                   alpha=0.7, color='coral')
    axes[0, 1].axvline(x=grid_risk['composite_risk'].mean(), color='red', linestyle='--',
    ...
    axes[1, 0].set_xticklabels([f"({r['lat_grid']:.0f},{r['lon_grid']:.0f})"
                                for _, r in top_cells.iterrows()], rotation=45, fontsize=7)
    ...
    # Risk level summary
    risk_summary = grid_risk.groupby('risk_level').agg(
        n_cells=('composite_risk', 'count'),
        avg_events=('n_events', 'mean'),
        avg_max_mag=('max_magnitude', 'mean')
    ).reset_index()
    ...
    axes[1, 1].barh(risk_summary_sorted['risk_level'].astype(str),
                   risk_summary_sorted['n_cells'], color=bar_colors_summary)
...
def save_hazard_results():
    ...
    (psha_spark.write
     .mode("overwrite")
     .option("mergeSchema", "true")
     .saveAsTable("gold.gld_psha_exceedance"))

    # Save risk grid
    risk_cols = ['lat_grid', 'lon_grid', 'n_events', 'max_magnitude',
                 'avg_magnitude', 'avg_depth', 'composite_risk', 'risk_level']
    ...
    (risk_spark.write
     .mode("overwrite")
     .option("mergeSchema", "true")
     .saveAsTable("gold.gld_hazard_risk_grid"))
    ...
    if len(fault_results) > 0:
        ...
        (fault_spark.write
         .mode("overwrite")
         .option("mergeSchema", "true")
         .saveAsTable("gold.gld_fault_line_analysis"))
...
if len(recurrence_df) > 0:
    ...
    for _, row in recurrence_df[recurrence_df['magnitude_threshold'].isin([3.0, 4.0, 5.0])].iterrows():
        print(f"  M >= {row['magnitude_threshold']:.1f}: every {row['recurrence_years']:.2f} years "
