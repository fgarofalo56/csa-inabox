...
def analyze_seasonality_and_holidays(df):
    ...
    monthly_total['trend'] = monthly_total['total_visits'].rolling(
        window=12, center=True, min_periods=6
    ).mean()

    # Seasonal component: deviation from trend
    monthly_total['seasonal_ratio'] = (
        monthly_total['total_visits'] / monthly_total['trend'].clip(lower=1)
    )
    ...
    holiday_months = {1: 'New Year', 5: 'Memorial Day', 6: 'Summer Start',
                      7: 'Summer Peak', 8: 'Summer End', 9: 'Labor Day',
                      11: 'Thanksgiving', 12: 'Christmas'}

    fig, axes = plt.subplots(2, 2, figsize=(18, 14))

    # Time series with trend overlay
    axes[0, 0].plot(monthly_total['date'], monthly_total['total_visits'] / 1e6,
                   alpha=0.6, linewidth=1, label='Actual')
    axes[0, 0].plot(monthly_total['date'], monthly_total['trend'] / 1e6,
                   color='red', linewidth=2, label='12-month Trend')
    ...
    colors_seasonal = ['gold' if m in holiday_months else 'steelblue'
                       for m in range(1, 13)]
    axes[0, 1].bar(range(1, 13), seasonal_index['seasonal_index'],
                  color=colors_seasonal)
    ...
    axes[1, 0].bar(annual_totals['year'], annual_totals['yoy_growth'].fillna(0),
    ...
    monthly_total['deseasonalized'] = (
        monthly_total['total_visits'] / monthly_total['seasonal_index'].clip(lower=0.1)
    )
    monthly_total['residual'] = (
        monthly_total['total_visits'] - monthly_total['trend'] * monthly_total['seasonal_index']
    )
    axes[1, 1].plot(monthly_total['date'], monthly_total['residual'] / 1e6,
...
def build_forecasting_model(df):
    ...
    df_ml['visits_rolling_6'] = (
        df_ml.groupby('park_code')['recreation_visits']
        .rolling(window=6, min_periods=3).mean().reset_index(drop=True)
    )
    df_ml['visits_rolling_12_std'] = (
        df_ml.groupby('park_code')['recreation_visits']
        .rolling(window=12, min_periods=6).std().reset_index(drop=True)
    )
    ...
    features = ['park_encoded', 'year', 'month', 'month_sin', 'month_cos',
                'is_summer', 'is_shoulder', 'is_winter', 'is_covid', 'is_holiday_month',
                'park_acres', 'campground_capacity',
                'visits_lag_1', 'visits_lag_3', 'visits_lag_6', 'visits_lag_12',
                'visits_rolling_3', 'visits_rolling_6', 'visits_rolling_12_std',
                'visits_yoy']
    ...
    for name, model in models.items():
        with mlflow.start_run(run_name=f"park_{name.lower().replace(' ', '_')}"):
            ...
            all_results[name] = {'mae': mae, 'r2': r2, 'rmse': rmse,
                                  'cv_mae': cv_mae, 'predictions': y_pred}
    ...
    for i, (name, res) in enumerate(all_results.items()):
        if i < 3:
            ...
            axes.flatten()[i].plot([y_test.min(), y_test.max()],
...
def analyze_carrying_capacity(df):
    ...
    density_colors = {'Under Capacity': 'green', 'Moderate Use': 'yellow',
                      'Near Capacity': 'orange', 'Over Capacity': 'red'}
    bar_colors = [density_colors.get(str(s), 'gray') for s in top_density['capacity_status']]

    axes[0, 0].barh(top_density['park_name'], top_density['visits_per_acre'],
                   color=bar_colors)
    ...
    axes[0, 1].barh(top_density['park_name'], top_density['capacity_stress'],
                   color=bar_colors)
    ...
    axes[1, 0].scatter(park_capacity['visits_per_acre'],
                      park_capacity['peaking_factor'],
...
def predict_trail_congestion(df, capacity_results):
    ...
    df_congestion = df_congestion.merge(
        capacity_results[['park_name', 'avg_monthly', 'peak_monthly', 'capacity_stress']],
        on='park_name', how='left'
    )

    # Congestion index: current month vs park average
    df_congestion['congestion_index'] = (
        df_congestion['recreation_visits'] /
        df_congestion['avg_monthly'].clip(lower=1)
    ).round(3)
    ...
    monthly_congestion = df_congestion.groupby('month')['congestion_index'].agg(
        ['mean', 'std', 'median']
    ).reset_index()

    axes[0, 0].errorbar(monthly_congestion['month'], monthly_congestion['mean'],
                        yerr=monthly_congestion['std'], fmt='o-', capsize=5,
    ...
    cong_colors = {'Very Low': 'darkgreen', 'Low': 'green', 'Normal': 'steelblue',
    ...
    park_avg_cong = df_congestion.groupby('park_name').agg(
        avg_congestion=('congestion_index', 'mean'),
        max_congestion=('congestion_index', 'max'),
        cap_stress=('capacity_stress', 'first')
    ).reset_index()
    axes[1, 1].scatter(park_avg_cong['cap_stress'], park_avg_cong['max_congestion'],
                      s=60, alpha=0.6, c='teal')
...
def optimize_revenue(df, capacity_results):
    ...
    df_rev = df.copy()
    seasonal_multiplier = {
        1: 0.7, 2: 0.7, 3: 0.9, 4: 1.0, 5: 1.1, 6: 1.3,
        7: 1.4, 8: 1.4, 9: 1.2, 10: 1.0, 11: 0.8, 12: 0.7
    }
    ...
    park_revenue = df_rev.groupby('park_name').agg(
    ...
    park_revenue = park_revenue.merge(
        capacity_results[['park_name', 'capacity_stress', 'capacity_status']],
        on='park_name', how='left'
    )
    ...
    axes[0, 0].barh(top_rev['park_name'], top_rev['total_revenue'],
    ...
    axes[0, 1].scatter(park_revenue['total_visits'] / 1e6,
                      park_revenue['total_revenue'],
                      s=60, alpha=0.6, c='teal')
    ...
    if len(park_revenue) > 3:
        ...
        if valid_mask.sum() > 3:
            ...
            axes[0, 1].plot(x_line, np.polyval(z, x_line), 'r--', linewidth=2,
    ...
    rev_by_status = park_revenue.groupby('capacity_status').agg(
...
def analyze_satisfaction_correlations(df, capacity_results):
    ...
    park_analysis = df.groupby('park_name').agg(
        total_visits=('recreation_visits', 'sum'),
    ...
    park_analysis = park_analysis.merge(
        capacity_results[['park_name', 'capacity_stress', 'peaking_factor',
                           'visit_volatility', 'visits_per_acre']],
        on='park_name', how='left'
    )

    # Return visit proxy: coefficient of variation (lower = more consistent = happier)
    park_analysis['return_visit_proxy'] = (
        1 - park_analysis['visit_volatility'].rank(pct=True)
    ).round(3)

    # Crowding index
    park_analysis['crowding_index'] = (
        park_analysis['peak_monthly'] / park_analysis['acres'].clip(lower=1)
    ).round(4)

    fig, axes = plt.subplots(2, 2, figsize=(18, 14))

    # Correlation matrix
    corr_cols = ['total_visits', 'capacity_stress', 'peaking_factor',
                 'visit_volatility', 'visits_per_acre', 'return_visit_proxy',
                 'crowding_index']
    ...
    axes[0, 1].scatter(park_analysis['capacity_stress'],
                      park_analysis['return_visit_proxy'],
                      s=park_analysis['total_visits'] / 1e5, alpha=0.6, c='steelblue')
    ...
    axes[1, 0].hist(park_analysis['crowding_index'], bins=30, edgecolor='black',
    ...
    axes[1, 1].scatter(np.log10(park_analysis['acres'].clip(lower=1)),
                      park_analysis['peaking_factor'],
                      s=60, alpha=0.5, c='teal')
...
capacity_save = capacity_results[['park_name', 'total_visits', 'avg_monthly', 'peak_monthly',
                                   'visits_per_acre', 'peaking_factor', 'capacity_stress',
                                   'capacity_status']].copy()
...
revenue_save = revenue_results[['park_name', 'total_revenue', 'avg_monthly_revenue',
                                 'revenue_per_visit', 'total_visits']].copy()
...
(revenue_spark.write
 .mode("overwrite")
 .option("mergeSchema", "true")
 .saveAsTable("gold.gld_park_revenue_analysis"))

# Save congestion data (aggregated by park)
congestion_save = congestion_results.groupby('park_name').agg(
    avg_congestion=('congestion_index', 'mean'),
    max_congestion=('congestion_index', 'max'),
...
(congestion_spark.write
 .mode("overwrite")
 .option("mergeSchema", "true")
 .saveAsTable("gold.gld_park_congestion"))
