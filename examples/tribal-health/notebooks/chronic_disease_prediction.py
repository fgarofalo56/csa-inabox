...
def train_diabetes_model(df):
    ...
    features = ['tribe_encoded', 'su_encoded', 'gender_encoded', 'age_encoded',
                ...
                'telehealth_visits', 'outpatient_visits', 'unique_dx',
                'has_hypertension', 'has_depression', 'has_substance', 'has_obesity',
                'has_ckd', 'has_respiratory',
                'encounter_span_days', 'comorbidity_count',
                'encounters_per_month', 'ed_rate', 'telehealth_rate']
...
def analyze_multimorbidity(df):
    ...
    conditions = ['has_diabetes', 'has_hypertension', 'has_depression',
                  'has_substance', 'has_obesity', 'has_ckd', 'has_respiratory']
    condition_labels = ['Diabetes', 'Hypertension', 'Depression',
                        'Substance Use', 'Obesity', 'CKD', 'Respiratory']
    ...
    sns.heatmap(cooccurrence.astype(float), annot=True, fmt='.1f', cmap='YlOrRd',
                ax=axes[0, 0], cbar_kws={'label': 'Co-occurrence (%)'})
    ...
    axes[0, 1].bar(comorbidity_dist.index, comorbidity_dist.values,
                  color=sns.color_palette('RdYlGn_r', len(comorbidity_dist)))
    ...
    colors_or = ['red' if o > 3 else 'orange' if o > 1.5 else 'steelblue'
                 for o in top_or['odds_ratio']]
    ...
    prevalence = pd.DataFrame({
        ...
        'prevalence_pct': [df[c].mean() * 100 for c in conditions]
...
def analyze_sdoh_factors(df):
    ...
    sdoh_metrics = df.groupby('service_unit').agg(
    ...
    colors_sdoh = ['red' if s > 60 else 'orange' if s > 40 else 'green'
                   for s in sdoh_sorted['sdoh_risk_index']]
    axes[0, 0].barh(sdoh_sorted['service_unit'], sdoh_sorted['sdoh_risk_index'],
                   color=colors_sdoh)
    ...
    axes[0, 1].scatter(sdoh_metrics['telehealth_access'] * 100,
                      sdoh_metrics['ed_utilization'] * 100,
                      s=sdoh_metrics['n_patients'], alpha=0.7, c='steelblue')
    for _, row in sdoh_metrics.iterrows():
        axes[0, 1].annotate(row['service_unit'],
                           (row['telehealth_access'] * 100, row['ed_utilization'] * 100),
                           fontsize=7)
    ...
    corr_cols = ['diabetes_rate', 'hypertension_rate', 'depression_rate',
                 'access_score', 'fragmentation_score', 'ed_utilization']
    ...
    print(sdoh_sorted[['service_unit', 'n_patients', 'sdoh_risk_index',
                       'access_score', 'fragmentation_score', 'burden_score']].to_string(index=False))
...
def build_risk_stratification(df):
    ...
    axes[0, 0].hist(df_risk['composite_risk_score'], bins=40, edgecolor='black',
                   alpha=0.7, color='steelblue')
    axes[0, 0].axvline(x=df_risk['composite_risk_score'].mean(), color='red',
                      linestyle='--', label=f"Mean: {df_risk['composite_risk_score'].mean():.1f}")
    ...
    tier_stats = df_risk.groupby('risk_tier').agg(
        n_patients=('patient_id', 'count'),
    ...
    axes[0, 1].bar(tier_stats['risk_tier'].astype(str), tier_stats['n_patients'],
                  color=bar_colors)
    ...
    w = 0.25
    axes[1, 0].bar([x - w for x in x_pos], tier_stats['diabetes_pct'] * 100,
                  width=w, label='Diabetes', color='coral')
    axes[1, 0].bar(x_pos, tier_stats['hypertension_pct'] * 100,
                  width=w, label='Hypertension', color='steelblue')
    axes[1, 0].bar([x + w for x in x_pos], tier_stats['depression_pct'] * 100,
                  width=w, label='Depression', color='teal')
    ...
    risk_components = ['age_risk', 'comorbidity_risk', 'utilization_risk',
                       'ed_risk', 'fragmentation_risk']
...
def identify_care_gaps(df_risk, encounters):
    ...
    chronic_patients['actual_annual_encounters'] = (
        chronic_patients['total_encounters'] / span_years
    ).round(1)

    chronic_patients['care_gap_ratio'] = (
        chronic_patients['actual_annual_encounters'] /
        chronic_patients['expected_annual_encounters'].clip(lower=1)
    ).round(3)
    ...
    axes[0, 0].hist(chronic_patients['care_gap_ratio'].clip(upper=3), bins=40,
    ...
    # Care gaps by tribe
    gap_by_tribe = chronic_patients.groupby('tribal_affiliation').agg(
    ...
    colors_gap = ['red' if r > 0.4 else 'orange' if r > 0.2 else 'green'
                  for r in gap_by_tribe['gap_rate']]
    axes[0, 1].barh(gap_by_tribe['tribal_affiliation'], gap_by_tribe['gap_rate'] * 100,
                   color=colors_gap)
    ...
    gap_by_tier = chronic_patients.groupby('risk_tier').agg(
        n_patients=('patient_id', 'count'),
        gap_rate=('has_care_gap', 'mean'),
        avg_gap_ratio=('care_gap_ratio', 'mean')
    ).reset_index()
    ...
    axes[1, 1].bar(gap_by_tier['risk_tier'].astype(str), gap_by_tier['gap_rate'] * 100,
                  color=bar_c)
...
def compute_disparity_index(df):
    ...
    for col in ['diabetes_rate', 'hypertension_rate', 'depression_rate',
                'avg_comorbidities', 'ed_rate']:
    ...
    axes[1].scatter(tribe_metrics['telehealth_rate'] * 100,
                   tribe_metrics['disparity_index'],
                   s=tribe_metrics['n_patients'], alpha=0.7, c='teal')
    for _, row in tribe_metrics.iterrows():
        axes[1].annotate(row['tribal_affiliation'],
                        (row['telehealth_rate'] * 100, row['disparity_index']),
                        fontsize=7)
...
def compute_population_health_metrics(df_risk):
    ...
    age_metrics = df_risk.groupby('age_group').agg(
        n=('patient_id', 'count'),
    ...
    age_metrics['age_sort'] = age_metrics['age_group'].apply(
    ...
    w = 0.25
    axes[0].bar([x - w for x in x_pos], age_metrics['diabetes'] * 100,
              width=w, label='Diabetes', color='coral')
    axes[0].bar(x_pos, age_metrics['hypertension'] * 100,
              width=w, label='Hypertension', color='steelblue')
    axes[0].bar([x + w for x in x_pos], age_metrics['depression'] * 100,
              width=w, label='Depression', color='teal')
...
disparity_save = disparity_scores[
    ['tribal_affiliation', 'n_patients', 'diabetes_rate', 'hypertension_rate',
     'depression_rate', 'avg_encounters', 'avg_comorbidities', 'ed_rate',
     'disparity_index']
]
...
risk_save = df_risk[['patient_id', 'tribal_affiliation', 'service_unit',
                      'comorbidity_count', 'composite_risk_score', 'risk_tier']].copy()
...
(risk_spark.write
 .mode("overwrite")
 .option("mergeSchema", "true")
 .saveAsTable("gold.gld_patient_risk_stratification"))

# Save SDOH metrics
sdoh_spark = spark.createDataFrame(sdoh_results[
    ['service_unit', 'n_patients', 'sdoh_risk_index', 'access_score',
     'fragmentation_score', 'burden_score']
])
...
(sdoh_spark.write
 .mode("overwrite")
 .option("mergeSchema", "true")
 .saveAsTable("gold.gld_sdoh_risk_index"))

# Save care gap analysis (aggregated by tribe)
care_gap_agg = care_gap_results.groupby('tribal_affiliation').agg(
    n_chronic=('patient_id', 'count'),
    gap_rate=('has_care_gap', 'mean'),
    avg_gap_ratio=('care_gap_ratio', 'mean')
).reset_index()
...
(care_gap_spark.write
 .mode("overwrite")
 .option("mergeSchema", "true")
 .saveAsTable("gold.gld_care_gap_analysis"))
...
for _, row in tier_stats.iterrows():
    print(f"  {row['risk_tier']}: {row['n_patients']:,} patients "
          f"(Diabetes: {row['diabetes_pct']*100:.0f}%)")

print("\nCare Gaps:")
total_gap = care_gap_results['has_care_gap'].sum()
total_chronic = len(care_gap_results)
print(f"  Chronic patients with care gaps: {total_gap:,}/{total_chronic:,} "
      f"({total_gap/total_chronic*100:.1f}%)")
