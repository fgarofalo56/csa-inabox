...
def calculate_age_standardized_rates(encounters, patients):
    ...
    us_2000_standard = {
        '0-4': 0.0691, '5-14': 0.1453, '15-24': 0.1383,
        '25-34': 0.1351, '35-44': 0.1620, '45-54': 0.1340,
        '55-64': 0.0867, '65-74': 0.0661, '75+': 0.0634
    }
    ...
    diabetes_tribe = asr_df[
        (asr_df['condition'] == 'Diabetes (E11)') &
        (asr_df['group_type'] == 'tribal_affiliation')
    ].sort_values('age_std_rate_per_1000', ascending=True)

    axes[0, 0].barh(diabetes_tribe['group_name'], diabetes_tribe['age_std_rate_per_1000'],
    ...
    for _i, cond in enumerate(['Diabetes (E11)', 'Hypertension (I10)', 'Depression (F32)']):
        su_data = asr_df[
            (asr_df['condition'] == cond) &
            (asr_df['group_type'] == 'service_unit')
        ].sort_values('age_std_rate_per_1000', ascending=False).head(10)
        if len(su_data) > 0:
            axes[0, 1].barh(
                [f"{su[:15]}" for su in su_data['group_name']],
                su_data['age_std_rate_per_1000'],
    ...
    tribe_crude = encounters.merge(
        patients[['patient_id', 'tribal_affiliation']], on='patient_id', how='left'
    )
    crude_rates = {}
    ...
    crude_df = pd.DataFrame({'tribe': list(crude_rates.keys()),
                             'crude_rate': list(crude_rates.values())})
    std_df = diabetes_tribe.rename(columns={'group_name': 'tribe',
                                            'age_std_rate_per_1000': 'std_rate'})
    comparison = crude_df.merge(std_df[['tribe', 'std_rate']], on='tribe', how='inner')

    if len(comparison) > 0:
        axes[1, 0].scatter(comparison['crude_rate'], comparison['std_rate'],
                          s=80, alpha=0.7, c='teal')
        for _, row in comparison.iterrows():
            axes[1, 0].annotate(row['tribe'][:12],
                               (row['crude_rate'], row['std_rate']), fontsize=7)
    ...
    pivot = asr_df[asr_df['group_type'] == 'tribal_affiliation'].pivot_table(
        index='group_name', columns='condition', values='age_std_rate_per_1000',
        aggfunc='first'
    )
    if len(pivot) > 0 and len(pivot.columns) > 0:
        sns.heatmap(pivot, annot=True, fmt='.1f', cmap='YlOrRd',
                    ax=axes[1, 1], cbar_kws={'label': 'Rate per 1,000'})
...
def build_health_equity_index(encounters, patients, facilities, asr_df):
    ...
    for tribe in patients['tribal_affiliation'].dropna().unique():
        ...
        util_raw = (
            max(0, (1 - ed_pct / 50.0)) * 10 +
            min(wellness_pct / 40.0, 1.0) * 10 +
            min(visit_diversity / 4.0, 1.0) * 5
        )
        ...
        cont_raw = (
            min(pct_within_6mo / 80.0, 1.0) * 15 +
            min(avg_visits / 5.0, 1.0) * 10
        )
    ...
    colors_hei = ['red' if h < 40 else 'orange' if h < 60 else 'green'
                  for h in hei_sorted['health_equity_index']]
    axes[0, 0].barh(hei_sorted['tribal_affiliation'], hei_sorted['health_equity_index'],
                    color=colors_hei)
    ...
    for d, label in zip(domains, domain_labels):
        axes[0, 1].barh(hei_sorted_top['tribal_affiliation'], hei_sorted_top[d],
                       left=bottom, label=label, alpha=0.8)
    ...
    axes[1, 0].scatter(hei_df['ed_pct'], hei_df['health_equity_index'],
                      s=hei_df['n_patients'] / 3, alpha=0.7, c='coral')
    for _, row in hei_df.iterrows():
        axes[1, 0].annotate(row['tribal_affiliation'][:10],
                           (row['ed_pct'], row['health_equity_index']), fontsize=7)
    ...
    # HEI distribution
    axes[1, 1].hist(hei_df['health_equity_index'], bins=12, edgecolor='black',
                   alpha=0.7, color='teal')
    axes[1, 1].axvline(x=hei_df['health_equity_index'].median(), color='red',
                       linestyle='--', label=f"Median: {hei_df['health_equity_index'].median():.0f}")
    ...
    print(hei_df[['tribal_affiliation', 'n_patients', 'health_equity_index',
                  'access_score', 'utilization_score', 'outcome_score',
                  'continuity_score']].sort_values(
        'health_equity_index', ascending=False).to_string(index=False))
...
def calculate_hedis_measures(encounters, patients):
    ...
    enc_demo = enc_copy.merge(
        patients[['patient_id', 'tribal_affiliation', 'service_unit',
                  'age_group', 'gender']],
        on='patient_id', how='left'
    )
    ...
    diabetes_patients = enc_demo[
        enc_demo['primary_dx_icd10'].str.startswith('E11', na=False)
    ]['patient_id'].unique()
    ...
    htn_patients = enc_demo[
        enc_demo['primary_dx_icd10'].str.startswith('I10', na=False)
    ]['patient_id'].unique()
    ...
    if n_htn > 0:
        ...
        htn_followups = htn_enc.groupby('patient_id').agg(
            n_visits=('encounter_id', 'count'),
            has_outpatient=('encounter_type', lambda x: ('OUTPATIENT' in x.values))
        )
    ...
    sud_patients = enc_demo[
        enc_demo['primary_dx_icd10'].str.startswith(('F10', 'F11', 'F12', 'F13'), na=False)
    ]['patient_id'].unique()
    ...
    axes[0].bar([i + 0.35 for i in x], hedis_df['benchmark'], width=0.35,
               label='Benchmark', color='green', alpha=0.5)
    ...
    print(hedis_df[['measure', 'code', 'numerator', 'denominator',
                    'rate', 'benchmark', 'met_benchmark']].to_string(index=False))
...
(asr_spark.write
 .mode("overwrite")
 .option("mergeSchema", "true")
 .saveAsTable("gold.gld_age_standardized_rates"))
...
(hei_spark.write
 .mode("overwrite")
 .option("mergeSchema", "true")
 .saveAsTable("gold.gld_health_equity_index"))
...
(hedis_spark.write
 .mode("overwrite")
 .option("mergeSchema", "true")
 .saveAsTable("gold.gld_hedis_quality_measures"))
