...
def analyze_machine_performance(slots):
    ...
    machine_perf["actual_rtp"] = (machine_perf["total_won"] / machine_perf["total_wagered"].clip(lower=1) * 100).round(
        2
    )
    ...
    axes[0, 0].axvline(
        x=machine_perf["actual_rtp"].median(),
        color="red",
        linestyle="--",
    ...
    axes[1, 1].bar(
        [f"${d:.2f}" for d in denom_rev.index], denom_rev.values, color=sns.color_palette("coolwarm", len(denom_rev))
    )
...
def analyze_zone_density(sessions):
    ...
    zone_metrics["revenue_per_machine"] = (
        zone_metrics["total_theo_win"] / zone_metrics["machine_count"].clip(lower=1)
    ).round(2)
    zone_metrics["players_per_machine"] = (
        zone_metrics["unique_players"] / zone_metrics["machine_count"].clip(lower=1)
    ).round(2)
    ...
    axes[1].scatter(
        zone_metrics["machine_count"],
        zone_metrics["total_theo_win"],
        s=zone_metrics["unique_players"] * 2,
        alpha=0.7,
        c="steelblue",
    )
...
(machine_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_machine_performance"))

zone_spark = spark.createDataFrame(zone_metrics)
zone_spark = zone_spark.withColumn("analysis_date", current_date())

(zone_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_zone_revenue_density"))
...
def cluster_slot_machines(machine_perf):
    ...
    cluster_features = ["total_spins", "actual_rtp", "revenue", "avg_denomination", "jackpot_count", "error_events"]
    ...
    for c in range(optimal_k):
        ...
        axes[0, 1].scatter(
            machine_perf.loc[mask, "actual_rtp"],
    ...
    cluster_summary["total_revenue"].plot(kind="bar", ax=axes[1, 0], color=sns.color_palette("Set2", optimal_k))
    ...
    sns.heatmap(
        centroid_df.T, annot=True, fmt=".2f", cmap="RdYlGn", ax=axes[1, 1], cbar_kws={"label": "Standardized Value"}
    )
...
def revenue_heatmaps(sessions, slots):
    ...
    sns.heatmap(
        zone_dow, annot=True, fmt=".0f", cmap="YlOrRd", ax=axes[0, 0], cbar_kws={"label": "Theoretical Win ($)"}
    )
    ...
    sns.heatmap(zone_game, annot=True, fmt=".0f", cmap="Blues", ax=axes[0, 1], cbar_kws={"label": "Coin-In ($)"})
    ...
    sns.heatmap(zone_dur, annot=True, fmt=".0f", cmap="Greens", ax=axes[1, 0], cbar_kws={"label": "Avg Duration (min)"})
    ...
    sns.heatmap(zone_occ, annot=True, fmt=".0f", cmap="Purples", ax=axes[1, 1], cbar_kws={"label": "Session Count"})
...
def analyze_peak_hours(sessions, slots):
    ...
    axes2[1].scatter(
        weekly["unique_players"], weekly["total_coin_in"], s=weekly["n_sessions"] / 10, alpha=0.6, c="teal"
    )
...
def build_staffing_model(sessions):
    ...
    hourly_demand = (
        sessions_copy.groupby(["floor_zone", "session_date", "day_of_week", "is_weekend", "month"])
        .agg(
    ...
    features = ["zone_encoded", "day_of_week", "is_weekend", "month", "month_sin", "month_cos", "dow_sin", "dow_cos"]
    X_staff = hourly_demand[features]
    y_staff = hourly_demand["active_sessions"]

    X_train_s, X_test_s, y_train_s, y_test_s = train_test_split(X_staff, y_staff, test_size=0.2, random_state=42)
    ...
    with mlflow.start_run(run_name="staffing_optimization"):
        ...
        # Cross-validation
        cv_scores = cross_val_score(rf_staff, X_train_sc, y_train_s, cv=5, scoring="neg_mean_absolute_error")
    ...
    axes[0, 0].plot([y_test_s.min(), y_test_s.max()], [y_test_s.min(), y_test_s.max()], "r--")
    ...
    # Feature importance
    feat_imp = pd.DataFrame({"feature": features, "importance": rf_staff.feature_importances_}).sort_values(
    ...
    zone_staffing = (
        hourly_demand.groupby("floor_zone")
        .agg(
    ...
    axes[1, 1].bar(
        x_pos,
        zone_staffing["recommended_staff"],
        width=0.4,
    ...
    axes[1, 1].bar(
        [x + 0.4 for x in x_pos],
        zone_staffing["min_staff"],
        width=0.4,
        label="Minimum (avg)",
        color="lightblue",
        align="center",
...
def optimize_denomination_mix(machine_perf, sessions):
    ...
    denom_analysis = (
        machine_perf.groupby("avg_denomination")
        .agg(
            n_machines=("machine_id", "count"),
            avg_revenue=("revenue", "mean"),
            total_revenue=("revenue", "sum"),
    ...
    axes[0, 0].barh(
        [f"${d:.2f}" for d in denom_sorted["avg_denomination"]], denom_sorted["revenue_per_spin"], color="teal"
    )
    ...
    axes[0, 1].scatter(
        denom_analysis["n_machines"],
        denom_analysis["total_revenue"],
        s=denom_analysis["avg_spins"] / 50,
        alpha=0.7,
        c="steelblue",
    )
    for _, row in denom_analysis.iterrows():
        axes[0, 1].annotate(f"${row['avg_denomination']:.2f}", (row["n_machines"], row["total_revenue"]), fontsize=9)
    ...
    colors_eff = ["green" if s > 60 else "orange" if s > 40 else "red" for s in denom_sorted_eff["efficiency_score"]]
    axes[1, 0].barh(
        [f"${d:.2f}" for d in denom_sorted_eff["avg_denomination"]],
        denom_sorted_eff["efficiency_score"],
        color=colors_eff,
    )
...
def analyze_maintenance(machine_perf, slots):
    ...
    axes[0, 0].axvline(
        x=machine_health["error_rate"].quantile(0.90),
        color="red",
        linestyle="--",
    ...
    axes[1, 1].scatter(
        machine_health["error_rate"],
        machine_health["rtp_deviation"],
        c=machine_health["maintenance_risk"],
        cmap="RdYlGn_r",
        alpha=0.6,
        s=30,
    )
    ...
    if len(high_risk) > 0:
        print(
            high_risk[["machine_id", "floor_zone", "error_rate", "rtp_deviation", "maintenance_risk"]]
...
cluster_spark = spark.createDataFrame(
    machine_perf[["machine_id", "floor_zone", "cluster", "total_spins", "actual_rtp", "revenue", "avg_denomination"]]
)
cluster_spark = cluster_spark.withColumn("analysis_date", current_date())

(cluster_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_machine_clusters"))
...
(staffing_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_staffing_recommendations"))
...
(health_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_machine_maintenance_risk"))
