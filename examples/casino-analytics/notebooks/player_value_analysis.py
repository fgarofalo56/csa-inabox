...
def compute_rfm(sessions, fnb):
    ...
    fnb_spend = (
        ...
        .agg(fnb_total=("total", "sum"), fnb_visits=("transaction_id", "count"), comp_total=("comp_value", "sum"))
        .reset_index()
    )
    ...
    for col in ["recency_days", "frequency", "monetary_coin_in"]:
        if col == "recency_days":
            player_rfm[f"{col}_score"] = pd.qcut(player_rfm[col], 5, labels=[5, 4, 3, 2, 1], duplicates="drop").astype(
                int
            )
        else:
            player_rfm[f"{col}_score"] = pd.qcut(
                player_rfm[col].rank(method="first"), 5, labels=[1, 2, 3, 4, 5], duplicates="drop"
            ).astype(int)
    ...
    seg_revenue.plot(kind="barh", ax=axes[0, 1], color=[seg_colors.get(s, "gray") for s in seg_revenue.index])
    ...
    axes[1, 0].hist(
        np.log10(player_rfm["monetary_coin_in"].clip(lower=1)), bins=30, edgecolor="black", alpha=0.7, color="steelblue"
    )
    ...
    axes[1, 1].scatter(
        player_rfm["monetary_coin_in"],
        player_rfm["fnb_total"],
        alpha=0.5,
        s=30,
        c=player_rfm["frequency"],
        cmap="viridis",
    )
...
def analyze_game_preferences(sessions):
    ...
    game_revenue = (
        ...
        .sort_values("total_coin_in", ascending=True)
        .reset_index()
    )

    axes[0].barh(
        game_revenue["game_type"], game_revenue["total_coin_in"], color=sns.color_palette("viridis", len(game_revenue))
    )
    ...
    axes[1].barh(
        game_revenue["game_type"], game_revenue["avg_duration"], color=sns.color_palette("mako", len(game_revenue))
    )
...
(rfm_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_player_rfm_segments"))
...
def build_churn_model(player_rfm, sessions):
    ...
    # Win rate proxy
    df_churn["win_rate"] = (df_churn["total_actual_win"] / df_churn["monetary_coin_in"].clip(lower=1)).round(4)
    ...
    for name, model in models.items():
        with mlflow.start_run(run_name=f"churn_{name.lower().replace(' ', '_')}"):
            ...
            churn_results[name] = {"model": model, "f1": f1, "auc": auc, "y_pred": y_pred, "y_prob": y_prob}
...
def analyze_promotional_roi(player_rfm, fnb):
    ...
    axes[1, 0].scatter(
        promo_by_segment["total_comp_value"],
        promo_by_segment["total_theo_win"],
        s=promo_by_segment["n_players"] * 2,
        alpha=0.7,
    )
    for _, row in promo_by_segment.iterrows():
        axes[1, 0].annotate(row["segment"], (row["total_comp_value"], row["total_theo_win"]), fontsize=10)
    ...
    w = 0.35
    axes[1, 1].bar(x_pos, promo_by_segment["avg_coin_in"], width=w, label="Avg Gaming (Coin-In)", color="steelblue")
    axes[1, 1].bar(
        [x + w for x in x_pos],
    ...
    print(
        promo_by_segment[["segment", "n_players", "comp_to_theo_ratio", "roi", "revenue_per_comp_dollar"]].to_string(
            index=False
        )
    )
...
def forecast_player_ltv(player_rfm, sessions):
    ...
    # Tenure in days
    df_ltv["tenure_days"] = (pd.to_datetime(df_ltv["last_visit"]) - pd.to_datetime(df_ltv["first_visit"])).dt.days.clip(
        lower=1
    )
    ...
    axes[1, 1].barh(
        ltv_by_seg["segment"], ltv_by_seg["avg_ltv"], color=[colors_seg.get(s, "gray") for s in ltv_by_seg["segment"]]
    )
...
def analyze_player_behavior(sessions, player_rfm):
    ...
    # Merge segment info
    sessions_copy = sessions_copy.merge(player_rfm[["player_id", "segment"]], on="player_id", how="left")
    ...
    wk_behavior = (
        sessions_copy.groupby(["segment", "is_weekend"])
        .agg(avg_coin_in=("coin_in", "mean"), avg_duration=("duration_minutes", "mean"))
        .reset_index()
    )
    ...
    monthly_visits = (
        sessions_copy.groupby(["segment", "month"]).agg(unique_players=("player_id", "nunique")).reset_index()
    )
    ...
    for seg in ["VIP", "Loyal", "Regular", "At Risk"]:
        ...
        if len(seg_data) > 0:
            axes[1, 1].plot(seg_data["month"], seg_data["unique_players"], marker="o", label=seg, linewidth=1.5)
...
churn_spark = spark.createDataFrame(df_churn[["player_id", "segment", "rfm_score", "is_churned", "churn_probability"]])
churn_spark = churn_spark.withColumn("analysis_date", current_date())

(churn_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_player_churn_predictions"))

# Save LTV forecasts
ltv_spark = spark.createDataFrame(
    df_ltv[["player_id", "segment", "projected_annual_ltv", "predicted_ltv", "daily_revenue_rate", "tenure_days"]]
)
ltv_spark = ltv_spark.withColumn("analysis_date", current_date())

(ltv_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_player_ltv_forecast"))
...
(promo_spark.write.mode("overwrite").option("mergeSchema", "true").saveAsTable("gold.gld_promotional_roi"))
...
print(
    f"LTV model R2: {r2_score(np.log1p(df_ltv['projected_annual_ltv'].clip(lower=0)), np.log1p(df_ltv['predicted_ltv'].clip(lower=0))):.4f}"
)
print("\nOutputs:")
