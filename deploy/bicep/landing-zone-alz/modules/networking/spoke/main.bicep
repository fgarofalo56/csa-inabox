...
// General parameters
// Specify the parLocation for all resources.
@description('Specify the parLocation for all resources.')
...
var varSpokeVirtualNetworks = [for spoke in parSpokeNetworks: {
  ...
  vnetparLocation: spoke.parLocation
...
module modSpokeNetworking '../../networking/spoke/spokeNetworking.bicep' = [for vnet in varSpokeVirtualNetworks: {
  ...
  params: {
    ...
    parSpokeLocation: vnet.vnetparLocation
...
module modHubPeeringToSpoke '../..//networking/vnetPeering/vnetPeering.bicep' = [for (vnet, index) in parSpokeNetworks: {
  scope: resourceGroup(parSpokeNetworks[0].parHubSubscriptionId, parHubVirtualNetworkResourceGroup)
...
module modSpokePeeringToHub '../../networking/vnetPeering/vnetPeering.bicep' = [for (vnet, index) in parSpokeNetworks: {
  ...
  name: take('spokePeerToHub-${name}-${vnet.parSpokeNetworkName}-${vnet.parLocation}-${parHubVirtualNetworkName}-${uniqueString(subscription().subscriptionId, '${name}-${vnet.parSpokeNetworkName}-${vnet.parLocation}-${index}')}', 64)
  params: {
