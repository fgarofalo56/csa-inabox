...
if entity == "sample_orders":
    df_validated = (
        df_typed
        .withColumn("_is_missing_order_id", F.col("order_id").isNull())
        ...
        .withColumn("validation_errors", F.concat_ws("; ",
    ...
    df_validated = (
        df_typed
        .withColumn("_is_missing_id", F.col("customer_id").isNull())
        .withColumn("_is_invalid_email", ~F.col("email").rlike(email_regex))
        .withColumn("_is_missing_name",
    ...
    df_validated = (
        ...
        .withColumn("validation_errors", F.lit(""))
    )
...
df_final = (
    ...
    .withColumn("_processing_engine", F.lit("spark"))
)
