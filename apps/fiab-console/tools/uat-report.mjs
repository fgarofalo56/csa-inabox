 ...
 *
 * Idempotent — rewrites the marked block at the top of each tutorial from the
 * item's catalog `learnContent`. Anything an author wrote BELOW the
 * `<!-- end auto-generated -->` marker is read back and re-appended, so
 * per-editor depth survives a re-gen; edits made INSIDE the marked block do
 * not — fix those on the catalog entry instead. A tutorial that has NO marker
 * anywhere is fully hand-authored (an author replaced the generated doc
 * wholesale) — those are SKIPPED with a warning, never overwritten. The merge
 * policy lives in `tools/tutorial-merge.mjs` and is unit-tested in
 * `__tests__/tutorial-merge.test.ts`.
 *
...
import { END_MARKER, mergeTutorial } from './tutorial-merge.mjs';
...
for (const v of editorAs) {
  ...
  const md = [
    '<!-- auto-generated by tools/uat-report.mjs — the block ABOVE the end marker is REGENERATED from the item\'s catalog `learnContent`; hand-written sections BELOW the end marker are preserved. A tutorial with NO end marker is treated as fully hand-authored and is never overwritten. -->',
  ...
  const outFile = path.join(TUTORIALS, `${slug}.md`);
  const existing = fs.existsSync(outFile) ? fs.readFileSync(outFile, 'utf8') : null;
  const merged = mergeTutorial(md.join('\n'), existing);
  if (merged.action === 'skip') {
    // Fully hand-authored guide (no end marker anywhere) — writing would delete
    // the whole file. Leave it alone and say so loudly.
    console.warn(
      `  ! skipped ${path.relative(REPO, outFile)} — no '${END_MARKER}' marker, ` +
      `treated as fully hand-authored. Add the marker if you want the top of ` +
      `this guide regenerated from the catalog learnContent.`,
    );
    skippedAuthored++;
    continue;
  }
  fs.writeFileSync(outFile, merged.content);
  written++;
}
console.log(`✓ Wrote ${written} tutorial(s) to ${path.relative(REPO, TUTORIALS)}`);
if (skippedAuthored) {
  console.log(`  (${skippedAuthored} hand-authored tutorial(s) left untouched)`);
}
