 ...
 * not — fix those on the catalog entry instead.
 *
...
/**
 * Hand-written sections a guide has BELOW the end marker (per-editor depth an
 * author added that the catalog `learnContent` doesn't model). The generator
 * used to claim these were preserved while overwriting the whole file — this
 * reads them back so the claim is true. Everything ABOVE/INSIDE the markers is
 * regenerated from `learnContent`; author durable prose there or below the end
 * marker, never inside the block.
 */
function readAuthoredTail(file) {
  if (!fs.existsSync(file)) return '';
  const prev = fs.readFileSync(file, 'utf8');
  const i = prev.lastIndexOf(END_MARKER);
  if (i === -1) return '';
  return prev.slice(i + END_MARKER.length).replace(/^\s*\n/, '');
}
let written = 0;
for (const v of editorAs) {
  ...
  const tail = readAuthoredTail(outFile);
  fs.writeFileSync(outFile, tail ? `${md.join('\n')}\n\n${tail}` : md.join('\n'));
  written++;
}
