...
| Effective (inherited) permissions view | ✅ built (effective toggle, both backends) | `listEffectivePermissions()` → Databricks: `GET /api/2.1/unity-catalog/effective-permissions/{type}/{full_name}`, **plus a concurrent `GET` of the securable itself for its `owner`** (#2651 — Azure Databricks never returns an owner's implied privileges from the permissions APIs, so an empty answer is not evidence of an unowned securable; the owner is reported as a fact next to the grid, never synthesized into rows Databricks did not return, and a failed owner read degrades to a warning). Loom Unity / OSS: the BFF resolves the inheritance walk itself (LU-4 — containment chain, `ALL PRIVILEGES` expansion, **non-inheriting** ownership, `USE CATALOG`/`USE SCHEMA` prerequisites, transitive groups; `lib/azure/uc-effective-permissions.ts`). Narrower than the native endpoint on the OSS privilege vocabulary — disclosed as warnings, not claimed as identical. The pane's caption and empty state are selected per backend (`lib/azure/uc-grants-copy.ts`) so neither describes work the answering backend did not do. Scoping to a principal is tenant-admin-or-self and audited. |
