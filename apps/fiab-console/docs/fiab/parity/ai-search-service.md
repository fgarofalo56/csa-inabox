...
| Scale (SKU tier immutable; replicas/partitions) — **in the ⚙ Service editor** and on the admin scaling page | ✅ built (AIF-17 · Service → **Scale** tab: replica SpinButton 1–12 + partition dropdown 1/2/3/4/6/12 + live search-unit count + confirm) | `POST /api/ai-search/service {action:'scale'}` → `scaleService()` → ARM `PATCH properties.replicaCount/partitionCount` then re-GET; `validateScale()` guards ranges; admin scaling page `updateSearchService()` unchanged |
| **Service administration — in-editor ⚙ Service tab** (AIF-17 · `AiSearchServicePanel`) — **tenant/domain-admin gated** | ✅ built | `GET /api/ai-search/service` → `getServiceProperties()` (ARM) + keys + stats; both routes gated by `denyIfNoDlzAccess(session,'scaling')` (was getSession-only) |
