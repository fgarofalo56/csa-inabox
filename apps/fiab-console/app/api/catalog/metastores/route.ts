 ...
 *     - discoverableWorkspaces: every Microsoft.Databricks/workspaces the
 *       Console identity can read over ARM — powers the registration dropdown
 *
 *   Returns { ok, unity, accountAdminGate?, unityWorkspaceErrors?,
 *             discoverableWorkspaces, purview, onelake }
 *
 * POST /api/catalog/metastores
 *   Body: { source: 'unity-catalog', hostname: string }
 *   Registers (probes) a Databricks workspace: lists its UC CATALOGS (which
 *   does NOT require account-admin) and attempts the metastore list. Returns
 *   the catalogs + an honest `accountAdminGate` if the metastore list 403s
 *   because the identity isn't a Databricks account admin. Permanent
 *   registration still requires updating LOOM_DATABRICKS_HOSTNAMES (bicep).
 */
...
export async function POST(req: NextRequest) {
  ...
  let metastoreError: string | undefined;
  try {
    ...
    if (isAccountAdmin403(status, e?.message)) {
      ...
      metastoreError = e?.message || String(e);
    }
  }
