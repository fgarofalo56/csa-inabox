...
import {
  ...
  isFabricOnlySourceType, FABRIC_ONLY_SOURCE_TYPES, RTH_SOURCE_TYPES,
...
/**
 * connect-source decides which source types REQUIRE the opt-in Fabric backend.
 * The gate used to be
 *
 *     if (/^Fabric/.test(sourceType) && !FABRIC_OPT_IN)
 *
 * flagged by CodeQL as js/regex/missing-regexp-anchor (#2666). Two separate
 * defects sat on that one line.
 *
 * 1. CLASSIFY BY SPELLING. A prefix test asks how a string is written, not what
 *    it is. It was correct only because `isRthSourceType` had already narrowed
 *    `sourceType` to the documented enum on the line above — safety by accident
 *    of ordering. Nothing in the source said so, and it breaks in both
 *    directions as that enum grows.
 *
 * 2. HALF THE OPT-IN (the reachable one). The Fabric branch below the gate runs
 *    only when `FABRIC_OPT_IN && fabricWorkspaceId`, but the gate tested just
 *    `!FABRIC_OPT_IN`. With LOOM_EVENTSTREAM_BACKEND=fabric set and no
 *    `fabricWorkspaceId` in the body, a Fabric-emitted source passed the gate,
 *    missed the Fabric branch, and fell through to the Azure-native path —
 *    creating the silently-dead item the gate exists to prevent, and returning
 *    `ok: true` while doing it.
 *
 * These pin the SHIPPED helper rather than a local copy of the predicate: per
 * the #2772 post-mortem above, a test that re-implements its subject cannot
 * fail when the subject changes.
 */
describe('connect-source Fabric-only source classification (#2666)', () => {
  it.each([...FABRIC_ONLY_SOURCE_TYPES])('requires Fabric for %s', (t) => {
    expect(isFabricOnlySourceType(t)).toBe(true);
  });

  it.each([
    ['AzureEventHub', 'Event Hubs — the Azure-native eventstream runtime'],
    ['ApacheKafka', 'reached directly, no Fabric involved'],
    ['PostgreSQLCDC', 'same'],
    ['RealTimeWeather', 'a Loom-native Function into Event Hubs'],
    ['SampleData', 'generated by Loom'],
  ])('does NOT require Fabric for %s (%s)', (t) => {
    expect(isFabricOnlySourceType(t)).toBe(false);
  });
  ...
  it.each([
    ...
    'Fabrication',
    'Fabric',
  ])('does NOT classify the unknown name %s as Fabric-only', (t) => {
    // A prefix test answers `true` for every one of these. Membership answers
    // the question actually being asked: is this one of the four source types
    // whose events only exist inside a Fabric tenant?
    expect(isFabricOnlySourceType(t)).toBe(false);
  });

  it('enumerates every Fabric-emitted member of the source enum', () => {
    // Drift guard. The prefix test used to provide this implicitly; an explicit
    // list has to be maintained, so a new `Fabric*` source added to
    // RTH_SOURCE_TYPES without being classified fails here rather than silently
    // becoming an Azure-native item that can never emit.
    const spelledFabric = RTH_SOURCE_TYPES.filter((t) => t.startsWith('Fabric'));
    expect([...FABRIC_ONLY_SOURCE_TYPES].sort()).toEqual([...spelledFabric].sort());
  });
});

describe('POST /api/realtime-hub/connect-source — Fabric opt-in is BOTH halves (#2666)', () => {
  // FABRIC_OPT_IN is read at module scope, so the env has to be set before the
  // route module is evaluated. resetModules + dynamic import; the vi.mock
  // registry at the top of this file still applies to the re-imported graph.
  async function connectWithFabricBackend(body: any) {
    const prev = process.env.LOOM_EVENTSTREAM_BACKEND;
    process.env.LOOM_EVENTSTREAM_BACKEND = 'fabric';
    vi.resetModules();
    try {
      const mod = await import('../connect-source/route');
      return await mod.POST(jsonReq(body));
    } finally {
      if (prev === undefined) delete process.env.LOOM_EVENTSTREAM_BACKEND;
      else process.env.LOOM_EVENTSTREAM_BACKEND = prev;
      vi.resetModules();
    }
  }

  it('400s a Fabric-emitted source when the backend is on but NO workspace is bound', async () => {
    (getSession as any).mockReturnValue(AUTH);
    (createOwnedItem as any).mockResolvedValue({ ok: true, item: { id: 'es-dead' } });

    const res = await connectWithFabricBackend({
      workspaceId: 'ws-1', displayName: 'Job events', sourceType: 'FabricJobEvents',
      // no fabricWorkspaceId — the Fabric branch cannot run
    });

    expect(res.status).toBe(400);
    const j = await res.json();
    expect(j.ok).toBe(false);
    expect(j.error).toMatch(/Fabric workspace/i);
    // The whole point: no dead Azure-native item, and no Fabric call either.
    expect(createOwnedItem).not.toHaveBeenCalled();
    expect(connectEventstreamSource).not.toHaveBeenCalled();
  });

  it('still routes a Fabric-emitted source to Fabric when BOTH halves are present', async () => {
    (getSession as any).mockReturnValue(AUTH);
    (connectEventstreamSource as any).mockResolvedValue({ id: 'fab-es-1' });
    ...
    const j = await res.json();
    expect(j.ok).toBe(true);
    expect(j.backend).toBe('fabric');
    expect(connectEventstreamSource).toHaveBeenCalledTimes(1);
    expect(createOwnedItem).not.toHaveBeenCalled();
  });

  it('leaves Azure-native sources on the Azure-native path when no workspace is bound', async () => {
    // The gate must not widen into a general "fabricWorkspaceId is required"
    // rule — that would be the hard Fabric dependency no-fabric-dependency.md
    // forbids. Only the four Fabric-emitted types are affected.
    (getSession as any).mockReturnValue(AUTH);
    (createOwnedItem as any).mockResolvedValue({ ok: true, item: { id: 'es-eh' } });

    const res = await connectWithFabricBackend({
      workspaceId: 'ws-1', displayName: 'Orders', sourceType: 'AzureEventHub',
    });

    expect(res.status).toBe(200);
    const j = await res.json();
    expect(j.backend).toBe('azure-native');
    expect(createOwnedItem).toHaveBeenCalledTimes(1);
  });
});
