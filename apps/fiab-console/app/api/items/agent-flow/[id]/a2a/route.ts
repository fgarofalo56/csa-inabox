...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json(buildItemAgentCard({
    id, name: loaded.item.displayName || 'Agent flow', description: loaded.item.description,
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const card = buildItemAgentCard({
    id, name: flowName, description: item.description, endpoint: endpointFor(req, id),
    kind: 'agent flow', platformEndpoint: platformEndpointFor(req), state,
  });
