/**
 * GET /api/items/power-app/[id]?envId=<env> — Power App detail.
 */
import { NextRequest, NextResponse } from 'next/server';
import { getSession } from '@/lib/auth/session';
import { getPowerApp, PowerPlatformError } from '@/lib/azure/powerplatform-client';
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const app = await getPowerApp(envId, (await ctx.params).id);
    return NextResponse.json({ ok: true, envId, app });
