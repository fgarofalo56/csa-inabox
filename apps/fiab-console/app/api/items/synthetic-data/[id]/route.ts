/**
 * Base persistence route for the 'synthetic-data' item — the GET + PATCH handler the
 * editor's useItemState('synthetic-data', id) drives:
 *   GET   → { id, displayName, description, workspaceId, state, updatedAt }
 *   PATCH   { displayName?, description?, state? }  (owner-scoped upsert)
 *   DELETE  → cosmos delete (+ mirror cleanup via deleteOwnedItem)
 *
 * Without this the editor PATCHes a 404 and silently loses every edit while
 * showing "Saved" (no-vaporware grade F — caught by scripts/check-item-routes).
 * Action sub-routes under this item type are unaffected. Route-guard compliant
 * via the shared owner-scoped item-crud helpers.
 */
