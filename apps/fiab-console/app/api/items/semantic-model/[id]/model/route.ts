/**
 * Calculation Groups + Field Parameters — semantic-model model route.
 *
 *   GET  /api/items/semantic-model/[id]/model?workspaceId=...
 *     Returns { ok, backend, calculationGroups, fieldParameters } from:
 *       - loom-native (DEFAULT): the Cosmos item's state.content
 *       - fabric (opt-in):       Fabric getDefinition → decode model.bim TMSL
 *
 *   POST /api/items/semantic-model/[id]/model?workspaceId=...
 *     Body: { calculationGroups?: TmslCalcGroup[]; fieldParameters?: FieldParamDef[] }
 *     Writes calc groups + field parameters using the selected backend:
 *
 *       - loom-native (DEFAULT — no Fabric/Power BI dependency): persists to the
 *         Cosmos item's state.content.calculationGroups / .fieldParameters. They
 *         are emitted in TMSL when the model is provisioned (see
 *         lib/install/provisioners/semantic-model.ts buildTmsl). No external call.
 *
 *       - aas (opt-in): builds TMSL createOrReplace per object and executes over
 *         SOAP/XMLA against LOOM_AAS_SERVER / LOOM_AAS_DATABASE. GCC-High / IL5 /
 *         DoD: honest gate (AAS unavailable) — loom-native still works.
 *
 *       - fabric (opt-in): getDefinition → merge calc groups + field params into
 *         model.bim TMSL → updateDefinition. Requires a bound workspace.
 *
 *       - powerbi (opt-in): honest gate — writing these objects to a live Power
 *         BI model needs the XMLA endpoint (Premium/PPU/Fabric capacity). The
 *         config is still saved to Cosmos for provision time.
 *
 * Backend is chosen by LOOM_SEMANTIC_BACKEND (default 'loom-native'), matching
 * .claude/rules/no-fabric-dependency.md (Azure-native is the default; Fabric /
 * Power BI are strictly opt-in).
 */
  ...
  if (backend === 'fabric') {
    ...
    if (workspaceId && !isLoomContentId(id)) {
      try {
        ...
        return NextResponse.json({ ok: true, backend, calculationGroups, fieldParameters });
      } catch (e: any) {
        const status = e instanceof PowerBiError ? e.status : 502;
        return NextResponse.json({ ok: false, error: e?.message || String(e), backend }, { status });
      }
    }
  }

  // loom-native (and fabric without a bound workspace): read from Cosmos content.
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const body = (await req.json().catch(() => ({}))) as ModelWriteRequest;
  ...
  await persistToCosmos(id, tenantId, calculationGroups, fieldParameters, steps);
...
async function persistToCosmos(
  id: string,
  tenantId: string,
