/**
 * Generic CRUD for the 'ontology' item — the persistence handler the Phase-4
 * editor's useItemState(<type>, id) drives:
 *   GET   /api/items/ontology/<id>          → { id, displayName, state, updatedAt }
 *   PATCH /api/items/ontology/<id> { state } → persists, returns { updatedAt }
 *   DELETE /api/items/ontology/<id>          → removes the item
 *
