/**
 * Generic CRUD for the 'graph-model' item — the persistence handler the Phase-4
 * editor's useItemState(<type>, id) drives:
 *   GET   /api/items/graph-model/<id>          → { id, displayName, state, updatedAt }
 *   PATCH /api/items/graph-model/<id> { state } → persists, returns { updatedAt }
 *   DELETE /api/items/graph-model/<id>          → removes the item
 *
