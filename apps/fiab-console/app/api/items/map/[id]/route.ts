/**
 * Generic CRUD for the 'map' item — the persistence handler the Phase-4
 * editor's useItemState(<type>, id) drives:
 *   GET   /api/items/map/<id>          → { id, displayName, state, updatedAt }
 *   PATCH /api/items/map/<id> { state } → persists, returns { updatedAt }
 *   DELETE /api/items/map/<id>          → removes the item
 *
