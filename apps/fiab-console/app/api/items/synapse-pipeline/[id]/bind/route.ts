/**
 * Resource-binding endpoint for a Synapse pipeline Loom item.
 *
 *   GET  /api/items/synapse-pipeline/[id]/bind
 *        → { ok, bound: string|null, pipelines: [{name}] }
 *          Returns the current binding (state.pipelineName) plus the list of
 *          REAL pipelines in the workspace so the editor can render its picker.
 *
 *   POST /api/items/synapse-pipeline/[id]/bind
