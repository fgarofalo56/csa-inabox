...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // OUTER ENVELOPE — everything the guard does not run. `enforceRateLimit`,
  // `resolveAccessMode` and `dedicatedTarget()` all throw on a misconfigured
  // deployment; `dedicatedTarget()` in particular calls
  // `required('LOOM_SYNAPSE_DEDICATED_POOL')`.
  //
  // PRE-EXISTING, NOT A REGRESSION FROM THIS PR — recorded that way rather than
  // folded in with its sibling. `warehouse/[id]/query` LOST this envelope when
  // `withSession` was dropped here; this route never had one, because it was
  // already a bare `getSession()` handler. Closed together because the failure
  // mode is identical: an unconfigured pool returned Next's generic HTML 500,
  // which the editor's `await r.json()` cannot parse.
  try {
    ...
    // Annotated with the BRAND, never `string` — see the note in the warehouse
    // sibling: annotating `string` discards `SynapseScopedDatabase` at exactly
    // the assignment the brand exists to protect.
