/**
 * GET /api/items/ai-builder-model?envId=<env> — list AI Builder models.
 */
import { NextRequest, NextResponse } from 'next/server';
import { getSession } from '@/lib/auth/session';
import { listAiBuilderModels, PowerPlatformError } from '@/lib/azure/powerplatform-client';
...
export async function GET(req: NextRequest) {
  ...
  try {
    const models = await listAiBuilderModels(envId);
    return NextResponse.json({ ok: true, envId, models });
