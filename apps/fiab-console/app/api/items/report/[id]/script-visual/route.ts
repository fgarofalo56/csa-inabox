...
//
// Replicated here (the helpers are module-private in …/query) so the inline
// fallback resolves rows through the SAME real Synapse/AAS path — no new data
// machinery, no mock. Script visuals carry no Filters-pane channel (PBI applies
// report/page filters upstream; the pane's primary path already passes
// pre-filtered `body.rows`), so this projection runs filter-free.

/** Collect the model-table names a visual's wells reference. */
 ...
 *  silently dropping a second table's fields — same contract as …/query. */
function toSqlSource(
  ...
  const referenced = Array.from(new Set(referencedTables(visual).filter((t) => map[t])));
 ...
 *  the same row shape …/query returns from every backend. */
