/**
 * GET /api/items/dataverse-table?envId=<env> — list tables (EntityDefinitions)
 */
import { NextRequest, NextResponse } from 'next/server';
import { getSession } from '@/lib/auth/session';
import { listTables, PowerPlatformError } from '@/lib/azure/powerplatform-client';
...
export async function GET(req: NextRequest) {
  ...
  try {
    const tables = await listTables(envId);
    return NextResponse.json({ ok: true, envId, tables });
