/**
 * Generic CRUD for the 'graphql-api' item — the persistence handler the Phase-4
 * editor's useItemState(<type>, id) drives:
 *   GET   /api/items/graphql-api/<id>          → { id, displayName, state, updatedAt }
 *   PATCH /api/items/graphql-api/<id> { state } → persists, returns { updatedAt }
 *   DELETE /api/items/graphql-api/<id>          → removes the item
 *
