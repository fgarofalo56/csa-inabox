/**
 * GET /api/items/digital-twin/[id]/source-schema  (FGC-12)
 *   Live ADX source-schema browser for the Digital Twin Builder's source-mapping
 *   wizard (entity/relationship → source-table + column pickers).
 ...
 *   Owner-checked (the caller must own the twin item) so the guard is satisfied
 *   without allow-listing. When ADX isn't configured we return an honest gate
 *   naming the exact env var — never a Fabric prompt.
 */
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const item = await loadOwnedItem(id, 'digital-twin', session.claims.oid, { allowReadRoles: true });
