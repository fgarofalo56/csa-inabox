/**
 * GET  /api/items/copy-job       — list copy-job items owned by tenant.
 * POST /api/items/copy-job       body { workspaceId, displayName,
 *                                       description?, state? } → create.
 *
 * `state` shape:
 *   {
 *     source: { linkedService, type, query?, table?, path? },
 *     sink:   { linkedService, type, table?, path? },
 *     mappings: Array<{ source: string; sink: string }>
 *   }
 *
 * Runs materialise a one-off Synapse pipeline named `loom-copy-<itemId>`
 * with a single Copy activity, then invoke it.
 */
