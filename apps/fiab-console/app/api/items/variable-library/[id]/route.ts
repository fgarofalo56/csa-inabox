/**
 * Generic CRUD for the 'variable-library' item — the persistence handler the Phase-4
 * editor's useItemState(<type>, id) drives:
 *   GET   /api/items/variable-library/<id>          → { id, displayName, state, updatedAt }
 *   PATCH /api/items/variable-library/<id> { state } → persists, returns { updatedAt }
 *   DELETE /api/items/variable-library/<id>          → removes the item
 *
