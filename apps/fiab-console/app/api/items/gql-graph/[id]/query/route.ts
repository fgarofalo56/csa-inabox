 ...
 * is preview; if LOOM_FABRIC_GRAPH_WORKSPACE is not bound we return 503
 * (Service Unavailable) with an explicit deferred-reason payload + the exact
 * env var / role to set (rendered by ResultsPreview as a warning MessageBar).
 */
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (backend === 'cosmos-gremlin-translate') {
    return NextResponse.json({
      ok: false,
      deferred: true,
      error: 'GQL → Gremlin translation passthrough is dispatched by the editor directly to /api/items/cosmos-gremlin-graph/[id]/query. If you reached this branch, update the client to dispatch there instead.',
    }, { status: 503 });
  }
