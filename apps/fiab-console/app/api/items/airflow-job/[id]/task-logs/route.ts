 ...
 * (resolveAirflowConn: managed LOOM_AIRFLOW_ENDPOINT default + BYO override).
 * Azure-native managed Apache Airflow — no Fabric.
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (taskId) {
      let logUrl: URL;
      try { logUrl = new URL(`${base}/${encodeURIComponent(taskId)}/logs/${encodeURIComponent(tryNumber)}`, conn.webserverUrl); }
      ...
      if (!r.ok) return err(`Airflow ${r.status}: ${text.slice(0, 240)}`, r.status, { webserverUrl: conn.webserverUrl });
      return NextResponse.json({ ok: true, dagId, runId, taskId, tryNumber, log: text });
    }

    // Task-instance list for the run.
    let listUrl: URL;
    try { listUrl = new URL(base, conn.webserverUrl); }
    catch { return err('webserverUrl invalid on stored item', 400); }
    const r = await fetch(listUrl.toString(), { headers, cache: 'no-store' });
    const txt = await r.text();
    let bd: any = null;
    try { bd = txt ? JSON.parse(txt) : null; } catch { /* leave as text */ }
    if (!r.ok) return err(`Airflow ${r.status}: ${bd?.title || bd?.detail || txt.slice(0, 240)}`, r.status, { webserverUrl: conn.webserverUrl });
