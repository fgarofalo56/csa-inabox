/**
 * GET /api/items/power-automate-flow?envId=<env> — list flows.
 */
import { NextRequest, NextResponse } from 'next/server';
import { getSession } from '@/lib/auth/session';
import { listFlows, PowerPlatformError } from '@/lib/azure/powerplatform-client';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, envId, flows });
