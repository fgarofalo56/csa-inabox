 ...
 * binds a STATIC itemType at module load and this route's `[type]` is dynamic, so
 * it composes `withSession` (the shared session+401+500 toolkit prologue) with an
 * explicit `loadOwnedItem(id, type, …)` — read-roles admitted on GET, write-scoped
...
export const GET = withSession<Params>(async (_req, { session, params }) => {
  const { type, id } = params;
  // READ: admit shared read-only roles (Viewer/Contributor) — same as the other
  // read GETs. Owner-scope is enforced by loadOwnedItem; a caller who cannot
  // reach the item gets null → 404 (never a cross-item read).
...
export const PUT = withSession<Params>(async (req: NextRequest, { session, params }) => {
  const { type, id } = params;
  ...
  // Optimistic concurrency: compare the caller's If-Match to the item's CURRENT
  // tag. A stale tag → 412; the client opens a diff (N5/N6).
