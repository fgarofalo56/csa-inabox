...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (accessMode === 'user') {
      ...
      result = await executeQueryAsUser(serverlessTarget(database), sqlText, userToken, session.claims.oid, 60_000, parameters);
    } else {
      result = await executeQuery(serverlessTarget(database), sqlText, 60_000, parameters);
    }
