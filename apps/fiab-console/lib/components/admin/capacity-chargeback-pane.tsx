...
/**
 * CapacityChargebackPane — the unified capacity + chargeback dashboard.
 *
 ...
 *
 * IA-03 (loom-apex Phase B): this surface used to be its own admin page at
 * /admin/usage-chargeback. It is now the "Capacity & LCU" TAB of the FinOps hub
 * (/admin/finops?tab=capacity); the old route survives as a redirect stub. The
 * component is UNCHANGED — it was moved, not rewritten — and is still embedded
 * verbatim by the top-level Reports hub (/org-reports).
 *
...
/**
 * CapacityChargebackPane — the unified capacity + chargeback surface with no
 * page chrome, so it embeds identically as the FinOps hub's "Capacity & LCU"
 * tab and in the top-level Reports hub (/org-reports). Real Cost Management +
 * Azure Monitor; a non-admin who reaches the tenant-admin-gated API sees the
 * honest 403 message.
 */
export function CapacityChargebackPane() {
  const styles = useStyles();
