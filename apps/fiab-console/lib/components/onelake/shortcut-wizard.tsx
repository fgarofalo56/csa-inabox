      ...
      {sourceType === 'gcs' && (
        <>
          ...
          <Field label="Service-account JSON" required hint="Paste the service-account key file (.json) — stored in Key Vault, never echoed.">
            <Textarea value={saJson} onChange={(_, d) => setSaJson(d.value)} rows={5} placeholder={'{ "type": "service_account", "client_email": "…", "private_key": "…" }'} disabled={!!value.secretName} resize="vertical" />
          </Field>
        </>
      )}
