...
/** A labeled group of admin destinations (IA-01 nav audit 2026-07-24). The
 * previously-flat ~44-row sidebar is now organized under scannable headers,
 * mirroring the grouped Purview sidebar (governance-sections.ts). Every
 * destination is preserved — this is grouping + labels, not deletion. */
interface AdminGroup { label: string; items: Section[]; }

// IA-01: the flat SECTIONS list is grouped into eight labeled clusters. Group
// headers render in the sidebar above each cluster (subtle Caption1, uppercase),
// so the admin nav scans like the rest of the product. NB: order within each
// group preserves the original list order where practical.
const ADMIN_GROUPS: AdminGroup[] = [
  {
    ...
    items: [
      ...
      { href: '/admin/rum', label: 'Real-user monitoring', icon: GaugeRegular, description: 'What real browsers experience: page-load p50/p95 by surface, Web Vitals (LCP/FCP/TTFB/CLS/INP), and top client errors — first-party capture (no CDN), PII-scrubbed, shipped to App Insights and charted from Log Analytics. Kill-switch: the rum1-client-telemetry runtime flag.' },
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/admin/finops', label: 'FinOps', icon: Money24Regular, description: 'The FinOps cockpit: real Cost Management forecast (Forecast API + honest fallback labeling), a live cost-anomaly feed + rules editor (the C3 monitor), per-scope spend breakdown tied to the chargeback model, and real Azure Budgets CRUD (create/update/delete). Absorbs the chargeback + usage-chargeback views.' },
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/admin/copilot-quality', label: 'Copilot quality', icon: TargetArrow24Regular, description: 'Per-surface Copilot answer quality: retrieval hit-rate / MRR against the golden eval sets, LLM-judge grounding, pass-rate vs the E3 floor, run-history trends, worst-question drill-in, and on-demand "Run now" — real copilot-evaluator runs from Cosmos, no Fabric dependency.' },
