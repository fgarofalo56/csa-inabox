...
export const UC_CAPABILITIES: UcCapability[] = [
  ...
  { id: 'system-tables', label: 'System tables (audit/billing/query/classification)', databricks: 'full', oss: 'partial', loomSurface: '/catalog/unity — System tables', note: 'LU-3: the OSS backend has a REAL access-audit equivalent — every catalog call funnels through the BFF audit choke point (ucFetch → recordUnityAccess) into the Cosmos _auditLog trail and the LoomAudit_CL SIEM stream, so who/what/when/outcome (including DENIALS) is queryable on the System tables pane. Billing and warehouse query history have no Loom Unity equivalent (no DBU meter, no query engine) — the pane names /admin/finops and the engine surfaces instead.' },
