...
/** Microsoft Graph service-root base URL for the active cloud (no trailing slash). */
export function graphBase(): string {
  // Explicit per-surface overrides win (used by mip/dlp clients for testing).
  const override = process.env.LOOM_GRAPH_BASE;
  if (override) return override.replace(/\/+$/, '');
  switch (detectCloud()) {
    case 'AzureDOD':
      return 'https://dod-graph.microsoft.us';
    case 'AzureUSGovernment':
      // IL5 (DoD M365) uses the dod-graph root; GCC High uses graph.microsoft.us.
      return (process.env.LOOM_CLOUD_BOUNDARY || '').toLowerCase() === 'il5'
...
/** AAD `.default` scope matching the active cloud's Graph root. */
export function graphScope(): string {
  return `${graphBase()}/.default`;
}
