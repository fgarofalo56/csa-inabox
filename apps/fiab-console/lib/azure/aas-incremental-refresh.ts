/**
 * Azure Analysis Services XMLA + async-refresh REST client — the *incremental
 * refresh* surface (hybrid tables + Power-BI-style enhanced refresh) for the
 * semantic-model editor.
 *
 * AAS is an Azure-native PaaS — NOT Microsoft Fabric — so it is an allowed
 * OPT-IN backend per no-fabric-dependency.md: the semantic model's default
 * backend stays `loom-native`; this client is only reached when the operator
 * sets LOOM_SEMANTIC_BACKEND=analysis-services + LOOM_AAS_XMLA_ENDPOINT.
 *
 ...
 *
 * No mocks. All calls hit real AAS; errors surface verbatim so the BFF can
 * render a precise MessageBar.
 *
 * The broader Analysis-Services / Power-BI tabular client (composite TMSL,
 * Fabric updateDefinition, DAX query, Direct-Lake shim, model-view writes) is
 * in `aas-client.ts`. This file is the focused REST/XMLA surface the
 * incremental-refresh editor calls; `AasError` is re-exported here so both
 * files share the same error type.
 *
 * Docs:
...
// Re-export AasError so the route / tests can `instanceof`-check against a
// single shared error class regardless of which AAS surface raised it.
export { AasError } from './aas-client';
 ...
 * AAS incremental-refresh backend is configured. The BFF turns a non-null
 * result into an honest `intent="warning"` MessageBar (no fabricated success).
 *
 * (Named distinctly from aas-client.ts's `aasConfigGate` — that one gates the
 * AAS *refresh* path on LOOM_AAS_SERVER/LOOM_AAS_MODEL; this one gates the
 * XMLA endpoint used by the incremental-refresh editor.)
 */
...
/**
 * tmslExecute — send any TMSL command (Alter / CreateOrReplace / Refresh) to
 * the AAS XMLA endpoint. Returns the raw XML response on success; throws
 * AasError on SOAP fault or HTTP error.
 */
 ...
 * reshuffles the partition structure per the current policy: on first apply
 * this is a full import of the historical window plus — when mode:'Hybrid' —
 * a live DirectQuery partition for the current period.
 *
 * effectiveDate (ISO date) overrides "today" for the rolling-window calc.
 */
...
/**
 * asyncRefresh — POST to the AAS async-refresh REST API (same body shape as
 * the Power BI enhanced refresh). Returns the requestId parsed from the 202
 * Location header so the caller can poll GET /refreshes/{requestId}.
 *
