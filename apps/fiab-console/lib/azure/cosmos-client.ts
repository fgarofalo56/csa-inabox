...
async function ensure() {
  ...
  // key-results store, one row per OKR, partitioned by the parent
  // data-product item id so the editor's OKR list hits a single physical
  // partition. Created lazily so a fresh environment needs no extra
  // ARM/Bicep step beyond the account+database.
