...
export const MCP_CATALOG: readonly McpCatalogEntry[] = [
  ...
  // ── 2026-06 catalog expansion — verified-real Docker MCP catalog images ─────
  // (HTTP 200 on hub.docker.com/v2/repositories/mcp/<id> at audit time). These
  // close the gap vs temp/mcp-gov-research.md (context7 = research #14) and add
  // the most-requested Azure-native data planes (Cosmos DB for MongoDB,
  // Elasticsearch) + a keyless privacy search.
  {
    id: 'context7',
    name: 'Context7',
    description: 'Version-pinned documentation injection for popular frameworks (React, Next.js, FastAPI, Azure SDK, …). Reaches the Upstash CDN; cacheable locally for gov.',
    image: 'mcp/context7',
    license: 'MIT',
    maintainer: 'Upstash (vendor)',
    category: 'Reference',
    egress: 'external-saas',
    port: 8080,
    healthPath: '/health',
  ...
  },
  {
    id: 'mongodb',
    name: 'MongoDB',
    description: 'Query, aggregate, and inspect schema against Azure Cosmos DB for MongoDB (vCore) or any MongoDB endpoint over the VNet. Prefer a read-only user.',
    image: 'mcp/mongodb',
    license: 'Apache-2.0',
    maintainer: 'MongoDB (vendor)',
    category: 'Database',
    egress: 'azure-internal',
    port: 8080,
    healthPath: '/health',
    ...
    defaultRecommended: false,
    externalHosts: [],
    secretEnv: 'MDB_MCP_CONNECTION_STRING',
    preview: true,
  },
  {
    id: 'elasticsearch',
    name: 'Elasticsearch',
    description: 'Full-text search, aggregations, and index exploration against an Elasticsearch / OpenSearch cluster over the VNet.',
    image: 'mcp/elasticsearch',
    license: 'Apache-2.0',
    maintainer: 'Elastic (vendor)',
    category: 'Database',
    egress: 'azure-internal',
    port: 8080,
    healthPath: '/health',
    ...
    defaultRecommended: false,
    externalHosts: [],
    secretEnv: 'ES_API_KEY',
    preview: true,
  },
  {
    id: 'duckduckgo',
    name: 'DuckDuckGo Search',
    description: 'Privacy-preserving real-time web search. Keyless (no credential); reaches duckduckgo.com — route via an approved gov proxy on gcc-high/il5.',
    ...
    airGapSafe: false,
    defaultRecommended: false,
    externalHosts: ['duckduckgo.com'],
    preview: true,
  },
] as const;

// A compile-time check that we ship exactly the curated 29 (a missing/extra
