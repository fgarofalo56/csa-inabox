 ...
 *   them may be left unterminated. `"…"` is treated as an identifier rather than
 *   rejected because that reading is safe under BOTH `QUOTED_IDENTIFIER` settings
 *   — ON it is a delimited identifier, OFF it is a string literal, and both use
 *   the same `""` doubling and both make the enclosed text inert.
 *
...
function validCheckExpression(expr: string): string | null {
  /**
   * Consume a quoted region opening at `start`, honouring the doubled-delimiter
   * escape. Returns the index of the closing delimiter, or -1 if unterminated.
   */
  const consumeQuoted = (start: number, close: string): number => {
