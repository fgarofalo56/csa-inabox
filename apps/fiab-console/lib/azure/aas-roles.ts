/**
 * aas-roles — Analysis-Services tabular-engine client for the semantic-model
 * RLS + OLS Security tab.
 *
 * (Split out of the conflicting add/add aas-client.ts so it composes with the
 * accumulated aas-client.ts surface — see no-vaporware.md integration. Shares
 * the SAME AasError class as aas-client so route `instanceof` checks hold.)
 *
 * WHAT IT DOES
 * ------------
...
/**
 * AAD `.default` token scope for an Azure Analysis Services XMLA-over-HTTP
 * endpoint. The XMLA audience is the region-specific data-plane host
 * (`https://<region>.<aasSuffix()>/.default`) — identical to the scope the
 * aas-client calc-group/agg XMLA paths use. (The literal-`*` form in
 * cloud-endpoints' aasScope() is only for the AAS *REST* async-refresh API.)
 */
function aasRegionScope(region: string): string {
  return `https://${region}.${aasSuffix()}/.default`;
}
...
export function resolveBackend(): ResolvedBackend {
  ...
  if (aasServer) {
    ...
    return {
      ...
      scope: aasRegionScope(parsed.region),
