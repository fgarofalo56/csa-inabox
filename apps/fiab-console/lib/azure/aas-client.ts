 ...
 *
 * This module carries TWO complementary, independent feature sets:
 *
 ...
 * wildcard) — derived via aasScope() (isGovCloud-aware). The Console UAMI must
 * hold the **server administrator** role on the AAS server (set via
 * `Microsoft.AnalysisServices/servers.properties.asAdministrators.members[]`,
 * NOT an Azure RBAC role assignment — see aas.bicep). The Fabric apply path
 * requests the Fabric `.default` scope instead.
 *
 ...
 *
 * No mocks. Real AAS / Fabric REST only.
 *
