/**
 * Azure Analysis Services (AAS) client — the Azure-native semantic-model
 * backend for CSA Loom (no Fabric / Power BI dependency, per
 * no-fabric-dependency.md). AAS hosts Import-mode (and DirectQuery / Hybrid)
 * tabular databases and is the default backend the SemanticModelEditor's
 * Storage Mode + Refresh surfaces wire to.
 *
 ...
 * The data-plane audience is the literal `https://*.{suffix}` per Microsoft
 * Learn (the `*` is NOT a wildcard) — see cloud-endpoints.aasScope().
 *
 * SCHEDULED REFRESH
 * -----------------
 ...
 * portal, auditable, and requires no Fabric / Cosmos dependency. A scheduler
 * (Logic App / Azure Function timer, deployed alongside aas.bicep) reads this
 * tag and calls refresh() at the configured times — the editor writes the
 * authoritative schedule and reads it straight back.
 *
...
function serverName(): string { return required('LOOM_AAS_SERVER_NAME'); }
function region(): string { return required('LOOM_AAS_REGION'); }
...
function armServerBase(): string {
  return `${armBase()}/subscriptions/${sub()}/resourceGroups/${rg()}`
...
function dataPlaneBase(db: string): string {
  return `https://${region()}.${getAasSuffix()}/servers/${serverName()}/models/${encodeURIComponent(db)}`;
}

/** AAS XMLA endpoint URL. */
function xmlaBase(): string {
  return `https://${region()}.${getAasSuffix()}/servers/${serverName()}/xmla`;
}

/**
 * Honest config gate for the AAS-backed semantic-model surfaces. Returns the
 * first missing env var (with operator-facing detail) so the BFF can render a
 * precise Fluent MessageBar instead of a raw 500/throw. Returns null when the
 * client is configured to attempt a real call.
 */
...
/** The AAS server name (for honest-gate MessageBar copy). Empty when unset. */
export function aasServerName(): string {
  return process.env.LOOM_AAS_SERVER_NAME || '';
}

/** The AAS server region (for MessageBar copy). Empty when unset. */
export function aasServerRegion(): string {
