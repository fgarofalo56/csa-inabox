...
export const DATA_PLANE_GATE_META: Record<string, GateMeta> = {
  ...
  'svc-ducklake-catalog': {
    ...
    fixit: { kind: 'env-picker' },
    autoResolveNote:
      'DEFAULT-ON: data-plane/ducklake-catalog-postgres.bicep provisions the private-endpoint-only Postgres store and admin-plane/main.bicep binds LOOM_DUCKLAKE_CATALOG_URL as a Key Vault secretRef, so a from-scratch deploy arrives wired — in BOTH clouds (PostgreSQL Flexible Server is an Azure Government service; gcc-high/il5 default postgresQuotaAvailable to true, opt-out only). Reading the catalog additionally needs the N2 DuckDB tier (LOOM_DUCKDB_URL), which admin-plane/main.bicep also deploys by default (duckdbTierActive) — it is the engine that runs the read-only ATTACH. It is unset in exactly four cases: the apps tier is off (deployAppsEnabled=false — the store is deliberately NOT billed before a Console exists to read it), an AKS boundary (the DuckDB engine that runs the ATTACH is Container-Apps-only, so a store there would have no reader), the Azure Postgres SUBSCRIPTION quota gate is deliberately turned off (LOOM_POSTGRES_QUOTA_AVAILABLE=false), or you point it at your own server. Unset → the DuckLake editor renders a guided empty state and honest-gates; N1\'s Iceberg REST Catalog (LOOM_ICEBERG_CATALOG_URL) and every other surface are unaffected.',
  ...
  'svc-s3-gateway': {
    ...
    fixit: { kind: 'env-picker' },
    autoResolveNote:
      'DEFAULT-ON: data-plane/s3-gateway-aca.bicep deploys the Apache-2.0 s3proxy Container App (internal ingress, dedicated Storage-Blob-Data-Reader identity, read-only, always-signed, scale-to-zero) and the deploy binds LOOM_S3_GATEWAY_URL. It is unset in three cases, and on the shipped param files the THIRD is the one that fires: the apps tier is off, an AKS boundary (cluster GitOps path), or NO LAKE ACCOUNT IS BOUND YET — the normal state of a first tenant-topology install, which deploys no landing zone. Fronting a lake that does not exist would be a URL that 404s every bucket, so the gateway rides the dlz-attach pass instead: attaching a domain landing zone deploys it into the hub and patches this var onto the running Console (main.bicep dlzAttachS3Gateway → hub-console-dlz-env.bicep), with no manual step. Unset → the surface documents that the N1 Iceberg REST Catalog + native ADLS/abfss path already give external engines governed access, so most deployments lose nothing. The AGPL MinIO path is never used.',
