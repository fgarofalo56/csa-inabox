...
export const DATA_PLANE_GATE_META: Record<string, GateMeta> = {
  ...
  'svc-redis-result-cache': {
    ...
    fixit: { kind: 'env-picker' },
    autoResolveNote:
      'WHAT THE PICKER IS FOR, BECAUSE IT IS NOT THE ROUTINE ANSWER IN ANY BOUNDARY (#4265 nit 10): hand-setting LOOM_RESULT_CACHE_REDIS is correct only when you are pointing the Console at a shared cache YOU already run. In every shipped boundary the gate resolves another way — GCC-High/IL5 auto-bind, GCC resolves with the apps lane, Commercial resolves through a scheduled cutover — so if you arrived here from one of those, the picker is not the step you want. FixitKind has no "no-op" member, hence a note rather than a disabled control. ' +
      'DEFAULT-ON IN GCC-HIGH AND IL5 (#2642): admin-plane/main.bicep deploys the OSS Valkey cache (shared/redis-oss-aca.bicep) on those two boundaries and binds LOOM_RESULT_CACHE_REDIS, LOOM_RESULT_CACHE_REDIS_PASSWORD (a Key Vault secretRef) and LOOM_RESULT_CACHE_REDIS_TLS=0 on the Console, so a from-scratch GCC-High / IL5 deploy arrives wired with no operator step. They get an OSS cache rather than Azure Managed Redis because AMR is Azure Public cloud only with no announced Government date, and their only Azure-managed alternative — the classic Azure Cache for Redis — is turned off on 2028-10-01. NOT YET ON IN GCC: redisOssActive also requires deployAppsEnabled, and params/gcc.bicepparam deliberately leaves it unset because GCC has no image producer lane yet (#3078) — a GCC deploy stands up zero Container Apps, so nothing is deployed and nothing is bound there. This gate being open on GCC is the expected state and resolves with the GCC apps lane, not by hand-setting the var. IN COMMERCIAL this is deliberately unset too: the shared tier there is Azure Managed Redis, and binding it means migrating the live redis-loom-hband-* cache, which is a SCHEDULED CHANGE WINDOW rather than a config edit (docs/fiab/runbooks/redis-amr-cutover.md). Unset in ANY boundary → the built-in per-replica in-memory result cache serves everything with zero loss of function; the shared tier only makes the cache coherent across Console replicas.',
  },
