...
export const AZURE_SERVICES_GATE_META: Record<string, GateMeta> = {
  ...
  'svc-powerplatform': {
    surfaces: [
      ...
      // refusal there now renders THIS gate. Listed so /admin/gates names the
      // surfaces the gate actually fires on rather than only the power-* ones.
    ...
    // Loom error CODE for the admission refusals — they return prose ("The user
    // is not a member of the organization"). `legacyCodes` is the code→gate
    ...
    // The prose is classified where it arrives, by
    // `isPowerPlatformAdmissionError` in lib/editors/copilot-studio-editors.tsx,
    // which renders THIS gate (#3544).
  },
