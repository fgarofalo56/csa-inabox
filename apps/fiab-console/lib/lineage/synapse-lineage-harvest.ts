 ...
 * caller can surface.
 *
 * DEPENDENCY — this module is the FIRST code in the product that persists a
 * physical dataset URI as a `thread-edges` endpoint. That is what makes the
 * Cosmos document-id derivation load-bearing: with the historical
 * `[^A-Za-z0-9_-] -> _` id, `…/a/b` and `…/a_b` collide on upsert (one edge
 * silently overwrites the other) and a deep sovereign `abfss://…usgovcloudapi
 * .net/…` pair exceeds Cosmos's 255-byte id limit, so the write throws and
 * `recordThreadEdge`'s best-effort catch swallows it. `threadEdgeDocId` fixes
 * both, and it ships as its OWN change with its own migration reasoning — this
 * module must not land before it.
 */
