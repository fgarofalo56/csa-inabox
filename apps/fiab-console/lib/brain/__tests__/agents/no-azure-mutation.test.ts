 ...
 *
 * "Import" means EVERY import form — see {@link IMPORT_FORM}. Anchoring three of
 * these on the keyword `from` is what made this scan defeatable until
 * 2026-08-23, and the dynamic shapes that walked past it are now pinned as
 * controls below.
 *
 * ── WHAT THIS SCAN DOES NOT ESTABLISH ──────────────────────────────────────
 * It reads SOURCE. It is not a runtime sandbox, and it cannot see egress reached
 * through a transitive import of a module outside this directory. It is defence
 * in depth behind the type pin and the inert-value proof, not a substitute.
 *
 * ── IT READS COMMENTS TOO, AND THAT IS DELIBERATE ──────────────────────────
 * The scan does not parse; it matches raw text, so a DOCSTRING that spells one
 * of these shapes literally trips it. That happened while writing this very fix:
 * a paragraph in `remediator.ts` describing the dynamic forms turned the scan
 * red, and it had to be reworded. The alternative — stripping comments before
 * scanning — buys tidier prose at the cost of a place to hide, and this file
 * already resolves the same tension the same way (see the note above about
 * `remediator.ts`'s destructive-command strings). If a comment trips it, reword
 * the comment; do NOT loosen the pattern.
 */
...
/**
 * A specifier reached by ANY import form — `from '…'`, `import('…')`, or
 * `require('…')`.
 *
 * ── WHY THIS IS NOT ANCHORED ON `from` ─────────────────────────────────────
 * It used to be. Three of the patterns below matched only `from '…'`, and an
 * independent review on 2026-08-23 defeated all three by switching to a dynamic
 * import. Measured: a real ARM DELETE against `management.azure.com`, reached
 * via `await import('node:https')`, sat inside `critic.ts` with the suite at
 * **RC 0 and 226/226 green**. The same held for `await import('@azure/…')` and
 * `await import('axios')`.
 *
 * That evasion is not exotic — it is the HOUSE IDIOM. The one sanctioned egress
 * in this directory, `model-client.ts`, is itself
 * `await import('@/lib/azure/aoai-chat-client')`, and its header spends a
 * paragraph justifying exactly that choice. So a developer copying the
 * established local pattern produced an egress this scan could not see. A guard
 * blind to the idiom used one file over is not a guard.
 */
const IMPORT_FORM = String.raw`(?:from\s+|import\s*\(\s*|require\s*\(\s*)`;

const FORBIDDEN: readonly { name: string; re: RegExp }[] = [
  { name: 'fetch-call', re: /\bfetch\s*\(/ },
  { name: 'xhr', re: /\bXMLHttpRequest\b/ },
  { name: 'azure-sdk-import', re: new RegExp(`${IMPORT_FORM}['"]@azure\\/`) },
  { name: 'child-process', re: /child_process/ },
  { name: 'exec', re: /\b(?:execSync|execFileSync|spawnSync)\s*\(/ },
  { name: 'node-http', re: new RegExp(`${IMPORT_FORM}['"](?:node:)?(?:http|https|net)['"]`) },
  { name: 'axios', re: new RegExp(`${IMPORT_FORM}['"]axios['"]`) },
];
...
/**
 * Every `import(...)` / `require(...)` call site, matched at its OPENING only.
 *
 * The argument is inspected by looking AHEAD rather than by capturing up to a
 * `)`, so a nested call inside the specifier cannot make the match fail and
 * thereby read as clean.
 */
const DYNAMIC_CALL = /\b(?:import|require)\s*\(/g;

/** The argument is a single string literal that closes the call immediately. */
const LITERAL_ARG_AHEAD = /^\s*(['"])[^'"]*\1\s*\)/;
...
function scanSource(file: string, src: string): Violation[] {
  ...
  // A NON-LITERAL specifier is a violation, not a non-match.
  //
  // `import('@/lib/azure/' + name)` does not satisfy AZURE_LIB_IMPORT — and a
  // pattern that fails to match reads exactly like a clean pass. Measured: that
  // exact line planted in `critic.ts` left the suite at RC 0. So the computed
  // form has to be flagged POSITIVELY. There is no legitimate reason for a
  // module in this directory to compute an import specifier at runtime, so this
  // is a violation everywhere — including in `model-client.ts`.
  for (const m of src.matchAll(DYNAMIC_CALL)) {
    const rest = src.slice(m.index + m[0].length);
    if (!LITERAL_ARG_AHEAD.test(rest)) {
      out.push({
        file,
        pattern: 'computed-specifier',
        detail: rest.slice(0, 60).replace(/\s+/g, ' ').trim(),
      });
    }
  }
  return out;
...
describe('lib/brain/agents — nothing here can call Azure', () => {
  ...
  it('CONTROL — every DYNAMIC evasion shape is detected', () => {
    // Each line below is an evasion MEASURED to pass the `from`-anchored scan on
    // 2026-08-23, planted in `critic.ts`, with the suite at RC 0 / 226 green.
    // They are pinned here so that if dynamic detection ever regresses, this
    // control goes red BEFORE a real egress can hide behind the same shape.
    const cases: readonly { readonly src: string; readonly expect: string }[] = [
      { src: "const nh = await import('node:https');", expect: 'node-http' },
      { src: "const nh = await import('https');", expect: 'node-http' },
      { src: "const nh = require('node:http');", expect: 'node-http' },
      { src: "const arm = await import('@azure/arm-resources');", expect: 'azure-sdk-import' },
      { src: "const ax = await import('axios');", expect: 'axios' },
      { src: "const ax = require('axios');", expect: 'axios' },
    ];
    for (const c of cases) {
      const hits = scanSource('critic.ts', c.src).map((h) => h.pattern);
      expect(hits, `${c.expect} not detected in: ${c.src}`).toContain(c.expect);
    }
    // A COMPUTED specifier is flagged positively rather than left to not-match.
    expect(
      scanSource('critic.ts', "const m = await import('@/lib/azure/' + n);").map((h) => h.pattern),
    ).toContain('computed-specifier');
    expect(
      scanSource(LLM_EGRESS_FILE, 'const m = await import(specifier);').map((h) => h.pattern),
    ).toContain('computed-specifier');
    // NEGATIVE control — the sanctioned literal dynamic import is NOT flagged,
    // so the check above cannot be passing merely by flagging everything.
    expect(scanSource(LLM_EGRESS_FILE, `await import('${ALLOWED_AZURE_SPECIFIER}');`)).toEqual([]);
  });
