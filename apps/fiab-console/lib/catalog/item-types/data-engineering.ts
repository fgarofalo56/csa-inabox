...
export const dataEngineeringItems: FabricItemType[] = [
  ...
  { slug: 'ducklake-catalog', displayName: 'DuckLake catalog', restType: 'DuckLakeCatalog', category: 'Data Engineering', noRestApi: true, preview: true,
    ...
    learnContent: {
      ...
      "steps": [
        { "title": "Point at a Postgres store", "body": "Nothing to do on a normal deployment: LOOM_DUCKLAKE_CATALOG_URL is wired by the deploy itself — data-plane/ducklake-catalog-postgres.bicep provisions a private-endpoint-only Azure Database for PostgreSQL flexible server and the Console binds the connection string as a Key Vault secretRef. It is only unset in a sovereign subscription that is quota-restricted from provisioning flexible servers, or when you point it at your own; the editor's Fix-it wizard writes it in that case." },
  ...
  { slug: 's3-gateway', displayName: 'S3-compatible ADLS gateway', restType: 'S3Gateway', category: 'Data Engineering', noRestApi: true, preview: true,
    description: 'Preview — expose an S3-compatible endpoint over your ADLS Gen2 so s3://-native OSS clients connect. Runs an Apache-2.0 s3proxy deployed with Loom (never AGPL MinIO), internal-ingress and read-only. Most engines need no gateway: the Iceberg REST Catalog + native abfss:// path already cover it. Azure-native — no Fabric.',
    learnContent: {
      "overview": "This Preview lab lets s3://-native OSS clients (Trino, Spark, DuckDB's s3 extension) address the deployment's ADLS Gen2 through an S3 API. The MinIO gateway path is deliberately dropped — MinIO's gateway is deprecated AND AGPL-licensed (banned by Loom's permissive-license rule); the permissive path is an Apache-2.0 s3proxy Container App, which platform/fiab/bicep/modules/data-plane/s3-gateway-aca.bicep deploys BY DEFAULT in front of ADLS: internal ingress only, managed-identity storage auth (no account key, no SAS), read-only, and scaled to zero so it costs nothing at idle. Loom bundles nothing (no AGPL, no s3proxy in the console image) — the upstream image is pulled unmodified. Crucially, most deployments need NO gateway at all: the N1 Iceberg REST Catalog plus the native abfss:// path already give external engines governed, audited access to the same data. The surface is honest either way: when LOOM_S3_GATEWAY_URL is unset (apps tier off) it documents the IRC/ADLS path and gates the connect panel with a Fix-it; when set it shows the real endpoint + copy-paste connect snippets. Azure-native, in-boundary, IL5-safe — no Microsoft Fabric.",
      "steps": [
        ...
        { "title": "The gateway is already deployed", "body": "s3-gateway-aca.bicep runs the Apache-2.0 s3proxy on your own Container Apps environment with the deployment. Never the AGPL MinIO gateway. Its S3 access key / secret key are stored in the Loom Key Vault as loom-s3-gateway-access-key / loom-s3-gateway-secret-key." },
        { "title": "Wire the endpoint", "body": "LOOM_S3_GATEWAY_URL is set by the deploy, so the surface renders the real endpoint and per-engine connect snippets straight away. The editor's Fix-it wizard covers the case where the apps tier was deployed without it." },
        { "title": "Connect s3://-native clients", "body": "Copy the DuckDB / Trino connect snippet from the editor and pair it with the Key Vault credential; the client addresses your lake with the S3 API while the data stays in your ADLS Gen2. The gateway is read-only by default." }
      ],
