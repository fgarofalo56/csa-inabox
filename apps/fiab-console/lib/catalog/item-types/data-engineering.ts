...
export const dataEngineeringItems: FabricItemType[] = [
  ...
  { slug: 'ducklake-catalog', displayName: 'DuckLake catalog', restType: 'DuckLakeCatalog', category: 'Data Engineering', noRestApi: true, preview: true,
    ...
    learnContent: {
      ...
      "steps": [
        { "title": "Point at a Postgres store", "body": "Nothing to do on a Commercial deployment: LOOM_DUCKLAKE_CATALOG_URL is wired by the deploy itself — data-plane/ducklake-catalog-postgres.bicep provisions a private-endpoint-only Azure Database for PostgreSQL flexible server and the Console binds the connection string as a Key Vault secretRef. GCC-High and IL5 currently set postgresQuotaAvailable=false, so the store is skipped there and this stays gated — point it at your own in-VNet flexible server with the editor's Fix-it wizard, or see the params-file comment for the sovereign enablement work in flight." },
