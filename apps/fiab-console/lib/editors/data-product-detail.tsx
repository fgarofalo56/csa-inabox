...
/**
 * Data product details page — adaptive owner / consumer surface.
 *
 * Azure-native parity with the Microsoft Purview Unified Catalog "data product
 * details" page. Reads a REAL product from the Cosmos `items` store (itemType
 * 'data-product') via GET /api/data-products/[id] (no Fabric / Purview / Power
 * BI dependency on the default path). The route returns `isOwner` (resolved
 * server-side from the owning workspace's tenant), so this component renders:
 *
 *   - OWNER (isOwner): the full F3 owner view — sticky header, Details tab
 *     (governance grid, editable owner contact labels, real subscribers,
 *     terms-of-use + documentation, a real DQ-score gauge or honest-gate,
 *     health-action cards, custom attributes) and a Data Observability tab.
 *     "Edit" / "Manage policies" hand off to the working editor via ?view=edit.
 *   - CONSUMER (!isOwner): the F15 read-only view — overview / datasets /
 *     glossary / my-data-access tabs plus a purpose-bound "Request access" CTA.
 *
...
import {
  Avatar, Badge, Body1, Button, Caption1, Card, CardHeader, Divider, Field, Input, Label,
...
import {
  ArrowClockwise20Regular, BookRegular, CheckmarkCircle16Filled, CheckmarkCircleRegular,
  DatabaseRegular, DocumentText20Regular, Edit20Regular, KeyRegular, LockClosedRegular,
  Open16Regular, PersonRegular, ShieldTask20Regular,
