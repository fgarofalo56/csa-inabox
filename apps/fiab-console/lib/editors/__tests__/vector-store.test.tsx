/**
 * VectorStoreEditor — Vitest contract test (auto-generated).
 *
 ...
 *
 * Per .claude/rules/no-vaporware.md grading rubric, this brings vector-store
 * from B-grade (functional, untested) to A-grade (functional + Vitest).
 */
import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import { render, screen, waitFor } from '@testing-library/react';
import { VectorStoreEditor } from '../graph-editors';
import { makeItem, installFetchMock } from './test-helpers';

describe('VectorStoreEditor', () => {
  ...
  it('mounts and surfaces at least one ribbon button', async () => {
    ...
    try {
      render(<VectorStoreEditor item={makeItem('vector-store', 'Vector store')} id="new" />);
