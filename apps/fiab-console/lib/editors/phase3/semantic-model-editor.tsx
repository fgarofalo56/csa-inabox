...
interface PfaColumnFlag { column: string; exposed: boolean }
interface PfaTableFlag { table: string; exposed: boolean; columns: PfaColumnFlag[] }
interface PfaAnswer {
  id: string; question: string; dax: string;
  lastVerifiedAt?: string; lastVerifiedOk?: boolean; lastVerifiedNote?: string;
  createdAt?: string; updatedAt?: string;
}
interface PfaState { aiInstructions: string; schema: PfaTableFlag[]; verifiedAnswers: PfaAnswer[] }

const usePrepForAiStyles = makeStyles({
...
function tableExposed(schema: PfaTableFlag[], table: string): boolean {
  const t = schema.find((x) => x.table === table);
  return t ? t.exposed !== false : true;
}
function columnExposed(schema: PfaTableFlag[], table: string, column: string): boolean {
  const t = schema.find((x) => x.table === table);
  if (!t) return true;
