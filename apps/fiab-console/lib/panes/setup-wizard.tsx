...
import { type ServiceChoiceMap } from '@/lib/panes/setup-service-choices';
...
interface WizardState {
  ...
  /** The reviewable adopt-or-create plan (deploy-integrity.md R5). Threaded into
   *  the deploy POST via `payload = {...state}`, so the operator's brownfield
   *  decisions reach the deploy — not only the 503 copy-paste fallback. */
  plan?: DeploymentPlan;
}
...
const RAIL_STEPS: { key: Step; label: string; hint: string }[] = [
  ...
  { key: 'scope', label: 'Analysis scope', hint: 'Subscriptions to read' },
  { key: 'services', label: 'Reuse or deploy', hint: 'Per-service decision' },
...
export function SetupWizardPane() {
  ...
  /**
   * The greenfield/brownfield planner (scope → discovery → per-service
   * decision → reviewable plan). Its state lives in its own hook so this file's
   * diff stays small; see lib/panes/setup-adoption-planner.ts.
   */
  const planner = useAdoptionPlanner();
  ...
  /**
   * Run the estate analysis for the consented scope and build the initial
   * adopt-or-create plan. Called when the operator leaves the scope step and
   * from the "Re-run analysis" button — never on mount, because R5.1 requires
   * the multi-subscription analysis to be OFFERED rather than just run.
   */
  const runEstateScan = useCallback(async () => {
    await planner.runScan(
      {
        boundary: PLAN_BOUNDARY[state.boundary ?? 'Commercial'],
        topology: state.mode === 'single-sub' ? 'single-sub' : 'tenant',
        installSubscriptionId: config?.adminSubscriptionId || state.subscriptionId || '',
        region: state.location || config?.location || '',
        createdBy: 'setup-wizard',
      },
      Object.fromEntries(subs.map((s) => [s.subscriptionId, s.displayName])),
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state.boundary, state.mode, state.subscriptionId, state.location, subs, config, planner]);
  ...
  function isStepComplete(step: Step): boolean {
    switch (step) {
      ...
      // Scope is complete once the operator has confirmed a non-empty set of
      // subscriptions to analyse (default is all of them, pre-checked).
      case 'scope': return planner.scope.length > 0;
      // Complete once a plan exists. One always does after a scan — even the
      // all-`create` plan an empty estate yields, i.e. greenfield.
      case 'services': return !!planner.plan;
      ...
      <div className={styles.panel}>
        ...
        {state.step === 'capacity' && (
          <>
            ...
            <Footer onBack={() => go('domain')} nextDisabled={!state.capacitySku} onNext={() => go('scope')} />
          </>
        )}
        ...
        {state.step === 'scope' && (
          <>
            <ScopeStepBody
              ...
              onReload={() => void loadSubscriptions()}
              selected={planner.scope}
              onChange={(next) => {
                // Changing the scope invalidates any plan built from the old
                // one — a plan whose ledger no longer matches its scan scope
                // would assert coverage it never had.
                planner.reset();
                setState((s) => ({ ...s, plan: undefined }));
                planner.setScope(next);
              }}
            />
            <Footer
              onBack={() => go('capacity')}
              nextDisabled={planner.scope.length === 0}
              onNext={() => {
                go('services');
                void runEstateScan();
              }}
        ...
        {state.step === 'services' && (
          <>
            <AdoptionStepBody
              ...
              error={planner.error}
              hubRegion={state.location ?? config?.location ?? ''}
              onRescan={() => void runEstateScan()}
              onDecide={(key, mode, target) => planner.decide(key, mode, target)}
              scopeSubscriptions={subs
                .filter((s) => planner.scope.includes(s.subscriptionId))
                .map((s) => ({ subscriptionId: s.subscriptionId, displayName: s.displayName }))}
            />
            <Footer
              onBack={() => go('scope')}
              nextDisabled={!planner.plan}
              onNext={() => {
                // The plan is copied into wizard state here so `payload =
                // {...state}` carries it into the deploy POST.
                if (planner.plan) setState((s) => ({ ...s, plan: planner.plan!, step: 'review' }));
              }}
            />
          </>
        )}
        ...
        {state.step === 'review' && (
          <>
            ...
            {/* The adopt-or-create plan in full, BEFORE anything runs (R5.2):
            ...
            <Footer
              ...
              // Validation BLOCKS (design §4): a resource Loom could not confirm
              // usable — including one it could not verify — is never adopted,
              // because a mid-deploy failure leaves a partial estate.
