 ...
 *
 * Azure-native backend (LOOM_WAREHOUSE_BACKEND=synapse-dedicated, the default):
 * uses synapse-sql-client.executeQuery against the dedicated pool's TDS endpoint
 * to run the bundled DDL. Per no-fabric-dependency.md this is the only
 * provisionable warehouse backend — no Fabric capacity or workspace is required.
 *
 * We split the bundle's WarehouseContent.ddl on semicolons and run each batch.
 * dbtModels[] are run after the DDL in bronze→silver→gold order, each wrapped
