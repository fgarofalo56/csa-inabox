...
export const TENANT_SETTING_GROUPS: ToggleGroupDef[] = [
  ...
  },
  {
    ...
    toggles: [
      ...
      { id: 'ai.fabricCopilotOptIn', label: 'Fabric / Power BI Copilot (opt-in)', help: 'Informational. When LOOM_COPILOT_BACKEND=fabric and a Fabric workspace id are set under Copilot & Agents, the cross-item orchestrator validates the bound Fabric capacity workspace via api.fabric.microsoft.com before each session. LLM inference still runs on Azure OpenAI. Requires F2+ Fabric capacity and "Service principals can use Fabric APIs" in the Fabric admin portal. The Azure-native AOAI path is always the silent default — never active without the flag. Not available in GCC-High / IL5 / DoD (Fabric Copilot is not supported in sovereign clouds).', default: false, learnUrl: 'https://learn.microsoft.com/fabric/fundamentals/copilot-enable-fabric' },
    ],
  },
  {
    id: 'mirroring',
