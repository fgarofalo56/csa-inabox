...
export const DATA_PLANE_ENV_CHECKS: EnvSpec[] = [
...

  {
    ...
    remediation:
      'LOOM_S3_GATEWAY_URL is the internal-ingress endpoint of the S3-compatible gateway in front of your ADLS Gen2 — an Apache-2.0 s3proxy Container App (the AGPL-licensed MinIO gateway path is NOT used). It lets s3://-native OSS clients (Trino, Spark, DuckDB with the s3 extension) address the lake with an S3 API. NOTHING TO DO on a normal deployment: platform/fiab/bicep/modules/data-plane/s3-gateway-aca.bicep deploys the gateway by DEFAULT and the deploy binds this var. It is unset in exactly three cases, and on the shipped param files the THIRD is the one that actually fires: (1) the apps tier is off (deployAppsEnabled=false); (2) an AKS boundary, where the workload deploys via the cluster GitOps path; (3) NO LAKE ACCOUNT IS BOUND YET. Case 3 is the normal state of a first tenant-topology install — every shipped bicepparam sets topology=tenant, which deploys no landing zone, so there is no ADLS Gen2 to front and standing a proxy up anyway would be a URL that 404s every bucket. The lake arrives on the dlz-attach pass, and that same pass deploys the gateway into the hub Container Apps environment and patches this var onto the running Console (main.bicep dlzAttachS3Gateway → landing-zone/hub-console-dlz-env.bicep). So: attach a domain landing zone and this clears with no manual step. Note you often do not NEED the gateway: the N1 Iceberg REST Catalog (LOOM_ICEBERG_CATALOG_URL) plus the native ADLS/abfss path already give external engines governed, audited access to the same data. Unset → the S3 gateway editor renders a guided empty state documenting the IRC/ADLS path and honest-gates the connection panel; nothing else changes. No Microsoft Fabric.',
    docs: 'https://github.com/gaul/s3proxy',
    provisionedBy: 'platform/fiab/bicep/modules/data-plane/s3-gateway-aca.bicep — DEFAULT-ON. Invoked by admin-plane/main.bicep (s3GatewayActive) when a lake is bound at admin-plane time, and by main.bicep (dlzAttachS3Gateway) on the dlz-attach pass, which is where a tenant-topology estate first has one. Apache-2.0 s3proxy Container App: internal ingress only; identity-based ADLS auth via DefaultAzureCredential resolved through the Container Apps IDENTITY_ENDPOINT/IDENTITY_HEADER pair (ACA does not expose IMDS) — no account key, no SAS, no connection string; s3proxy.read-only-blobstore=true; S3 signature checking is always on (there is no reachable anonymous mode); minReplicas 0 so it costs nothing at idle. The image is the pinned upstream Apache-2.0 s3proxy:3.3.0 MIRRORED into the deployment\'s own ACR (full-app-deploy-commercial.yml mirror-upstream for Commercial, gov-provision-dataplane-images.yml for GCC-High/IL5), so a locked-egress estate needs no docker.io egress. The S3 wire credential is derived from the gateway\'s own dedicated identity and is STABLE across redeploys, but it is DERIVABLE rather than secret — a deterministic hash of that identity\'s principalId with a salt that is public in the module source, so any principal holding Reader on the admin resource group can recompute it. What actually holds against such a principal is the network perimeter (internal ingress, no public listener) plus the READ-only blast radius (Storage Blob Data Reader + read-only-blobstore). Estates that hand out broad Reader AND have in-VNet footholds should pass the module an orchestrator-supplied unpredictable pair derived from loomGeneratedSecretSeed instead (pin that seed to keep it stable). Either way it is delivered as Container Apps secrets and mirrored to the Loom Key Vault as loom-s3-gateway-access-key / loom-s3-gateway-secret-key.',
    role: 'The s3proxy instance runs as a DEDICATED user-assigned identity (uami-loom-s3gw-<region>), created by its own module and granted ONLY Storage Blob Data READER on the lake, at the lake\'s own resource group / subscription scope (data-plane/s3-gateway-lake-rbac.bicep). It deliberately does NOT run as the Console UAMI, which also holds Blob Data Contributor, Key Vault Secrets User and Network Contributor: read-only-blobstore is a proxy-layer control that would not survive a compromise of a Java process parsing attacker-influenced S3 signatures, so the IAM boundary carries the posture. The Console UAMI is attached only as the ACR pull credential. The Console itself reads just the endpoint URL to render connect info.',
    availability: {
      commercial: 'ga', gccHigh: 'ga', il5: 'ga',
