...
export const DATA_PLANE_ENV_CHECKS: EnvSpec[] = [
  ...
  },
  {
    ...
    // Sovereign boundaries now get the value FROM THE DEPLOY.
    remediation: 'SOVEREIGN BOUNDARIES (GCC / GCC-High / IL5): nothing to do — admin-plane/main.bicep deploys the OSS Valkey cache (shared/redis-oss-aca.bicep) and SETS LOOM_RESULT_CACHE_REDIS, LOOM_RESULT_CACHE_REDIS_PASSWORD (Key Vault secretRef) and LOOM_RESULT_CACHE_REDIS_TLS=0 on the Console itself. Unset here means the tier was opted out (loomBackends.redisOss = "disabled") or the deploy has not rolled yet. COMMERCIAL: the shared tier is Azure Managed Redis, and binding it is a SCHEDULED change window, not a config edit — follow docs/fiab/runbooks/redis-amr-cutover.md, which provisions the AMR cluster and sets this var from its endpoint output (<host>:10000, NOT the classic 6380 and NOT the OSS 6379 — never hard-code a port). Either way the in-memory default is fully functional in the meantime.',
    provisionedBy: 'SOVEREIGN: modules/shared/redis-oss-aca.bicep (OSS Valkey on Container Apps), deployed + auto-bound by modules/admin-plane/main.bicep (redisOssActive) → LOOM_RESULT_CACHE_REDIS / _PASSWORD / _TLS on the Console app. COMMERCIAL: modules/shared/managed-redis.bicep (Azure Managed Redis) via the cutover runbook — Azure Managed Redis is Azure Public cloud only, which is why the two boundaries take different backends.',
    role: 'SOVEREIGN: Valkey requirepass, resolved from Key Vault by the Console UAMI (LOOM_RESULT_CACHE_REDIS_PASSWORD secretRef) — Key Vault Secrets User on the estate vault. COMMERCIAL: Entra data-plane on the Azure Managed Redis database (access-policy assignment for the Console UAMI); no access key is minted.',
  },
