...
export const BUILDERS_ENV_CHECKS: EnvSpec[] = [
  ...
  },
  {
    ...
    remediation: 'Set LOOM_WEAVE_PG_FQDN so the Weave ontology store persists to its governed Postgres database (weave_ontology_not_configured). HONEST STATE (re-measured 2026-08-18): the useSingleDlz cliff described here previously is NO LONGER the reason this is empty — since #3371/#3372 admin-plane/main.bicep deploys the Weave server itself on the tenant and multi-sub topologies (weavePgLocalActive) and emits the FQDN from its output, so a Commercial deploy on any topology fills this in. What DOES leave it empty is the Postgres quota gate: 3449d put postgresQuotaAvailable into weavePgLocalActive, and params/gcc-high.bicepparam + params/il5.bicepparam pin that false, so GCC-High / IL5 skip the server by design rather than failing the deploy on it. Clearable there: observabilityConfig.backendOverrides={weavePostgres:\'enabled\'} on the tenant topology, or postgresQuotaAvailable=true (read those param files first — they enumerate everything else that flag turns on). The useSingleDlz cliff DOES still affect LOOM_BATCH_ACCOUNT, LOOM_POSTGRES_HOST and the AML default compute (main.bicep derives all three only when useSingleDlz).',
    provisionedBy: 'modules/landing-zone/postgres-weave.bicep (Apache AGE PostgreSQL flexible server), invoked TWO ways: admin-plane/main.bicep weavePg (tenant / multi-sub — the default path, gated on weavePgLocalActive) and landing-zone/main.bicep postgresWeave (single-sub, gated on weavePgActive, whose FQDN main.bicep reconstructs into loomWeavePgFqdn). Either way it reaches the Console as apps[] env LOOM_WEAVE_PG_FQDN. Previously this field named modules/deploy-planner/postgres-flexible.bicep — a path that does not exist.',
