...
export const OBSERVABILITY_ENV_CHECKS: EnvSpec[] = [
  {
    ...
    provisionedBy: 'modules/admin-plane/uat-results-storage.bicep (account + uat-results container + lifecycle + blob private endpoint + Console UAMI role) + modules/admin-plane/synthetic-monitor-job.bicep (scheduled Microsoft.App/jobs) + modules/admin-plane/main.bicep (observabilityConfig bag → apps[] env LOOM_UAT_RESULTS_ACCOUNT / LOOM_UAT_RESULTS_CONTAINER / LOOM_SYNTHETIC_MONITOR_ENABLED)',
