...
export const SECURITY_ENV_CHECKS: EnvSpec[] = [
  ...
  // deployed catalog answers anonymous callers.
  {
    ...
    provisionedBy: 'admin-plane/main.bicep → modules/compute/loom-unity-app.bicep (deployed by DEFAULT on every boundary with authMode=entra + entraClientId + a CAE-subnet consoleAllowedCidrs pin) + modules/data-plane/loom-unity-postgres.bicep; the Console half (LOOM_UNITY_CLIENT_ID / _AUDIENCE / _AUTH_MODE) is emitted by the same template, or stamped by scripts/csa-loom/bootstrap-msal-app-reg.sh in deploy phase 3 when the Entra app registration is created there',
