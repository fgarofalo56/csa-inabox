...
export const SECURITY_ENV_CHECKS: EnvSpec[] = [
  ...
  // deployed catalog answers anonymous callers.
  {
    ...
    warnOnMiss: true,
    remediation: 'Loom Unity (the Unity-Catalog-compatible OSS catalog Loom deploys for the sovereign path) must not be reachable anonymously. (1) Redeploy platform/fiab/bicep/modules/compute/loom-unity-app.bicep with authMode=entra (the default) and entraClientId=<the Entra app registration fronting Loom Unity — normally the same one as LOOM_MSAL_CLIENT_ID>, plus consoleAllowedCidrs=<the Container Apps infrastructure subnet> to pin ingress to the Console. (2) Set LOOM_UNITY_CLIENT_ID on the Console (or LOOM_UNITY_AUDIENCE for an explicit scope) so the BFF mints an Entra bearer from the Console managed identity on every catalog call — the BFF is the single audited choke point; nothing else talks to the catalog. A pre-shared server-minted token via LOOM_UNITY_TOKEN (Key Vault secretref) is the alternative. If the catalog reports SEALED (authorizationMisconfigured / authorizationSealed true), it is up and rejecting everything because no Entra app registration existed at deploy time — run csa-loom-post-deploy-bootstrap (bootstrap-msal-app-reg.sh), which creates it, stamps LOOM_UNITY_ENTRA_CLIENT_ID on the catalog and these vars on the Console, and records the client id in Key Vault so later redeploys stay pinned. Leave unset ONLY where loom-unity is not deployed (Commercial estates on the Databricks Unity Catalog path). Threat model: docs/fiab/security/loom-unity-threat-model.md.',
    provisionedBy: 'admin-plane/main.bicep → modules/compute/loom-unity-app.bicep (deployed by DEFAULT on every boundary with authMode=entra + a CAE-subnet consoleAllowedCidrs pin) + modules/data-plane/loom-unity-postgres.bicep. LOOM_UNITY_AUTH_MODE=entra is emitted by the template WHENEVER the catalog is deployed, because the catalog enforces Entra authorization in both of its states — audience pinned (usable) or SEALED (up, sentinel .invalid audience nothing can mint, scaled to zero, every caller rejected). LOOM_UNITY_CLIENT_ID / _AUDIENCE follow once an Entra app registration exists: supplied at template time via LOOM_MSAL_CLIENT_ID / loomBackends.unityClientId (the deploy workflows resolve any existing client id from Key Vault first), or stamped by scripts/csa-loom/bootstrap-msal-app-reg.sh in deploy phase 3. An app registration is a Microsoft Graph object ARM cannot create, which is why the sealed state exists at all',
