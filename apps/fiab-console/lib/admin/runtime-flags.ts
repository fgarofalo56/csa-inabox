...
export const RUNTIME_FLAGS: readonly RuntimeFlagDef[] = [
  ...
  },
  {
    ...
    description:
      'The N7e "Federated SQL (Trino)" engine choice in the SQL Lab engine picker. DEFAULT-ON (opt-out) since the engine stopped requiring an AKS cluster: every push-button deploy now stands up a single-node Trino OSS Container App with internal ingress and minReplicas 0 (data-plane/loom-trino-aca.bicep) and wires LOOM_TRINO_URL, so idle cost is nothing and the capability is really there. It queries the Loom lake through the N1 Iceberg REST Catalog, which the same deploy stands up. Turn it OFF to hide the Trino option from the engine dropdown entirely — DuckDB / Synapse Serverless are unaffected and remain the engine the picker starts on. The option still honest-gates if LOOM_TRINO_URL is unset (backend opted out, or the loom-trino image not yet in ACR).',
    ownerItem: 'N7e',
