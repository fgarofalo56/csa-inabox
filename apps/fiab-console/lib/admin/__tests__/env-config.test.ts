...
describe('admin/env-config registry', () => {
  ...
  it('exposes exactly the 62 editable runtime variables (catalog completeness)', () => {
    ...
    // The recipient credential pin (LOOM_SHARING_AUDIENCE / LOOM_SHARING_SCOPE)
    // is NOT in the spec on this branch: the recipient-facing endpoint it pins
    // was split out of this change, and an operator should not be told to
    // configure a credential for an endpoint this build does not serve. It
    // returns with the endpoint (+2 -> 192 then).
