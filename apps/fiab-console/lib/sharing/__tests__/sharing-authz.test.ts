/**
 * LU-9 — the RECIPIENT RESOLVER: \`authenticateRecipient\` + \`assertShareAccess\`.
 *
 * This is the credential half of the data-egress surface. The recipient resolver is
 * exercised with a REAL RS256 keypair (locally generated, JWK injected via the test
 * hook) so the signature / issuer / audience / expiry / token-TYPE checks all run
 * the production code path with no network.
 *
 * The PURE model (\`recipientCanAccessShare\`, \`visibleShares\`,
 * \`matchRecipientByPrincipal\`, validation, manifest rendering) is covered by
 * ./sharing-model.test.ts, which landed with the control plane. Name
 * canonicalisation — the round-4 cross-recipient read — is ./name-collision.test.ts.
 */
import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import crypto from 'node:crypto';
import { type LoomRecipient } from '../model';
