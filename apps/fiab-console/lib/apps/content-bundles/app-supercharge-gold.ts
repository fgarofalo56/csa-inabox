...
const CELLS_GOLD_17_GOLD_AI_FUNCTIONS_COMPLIANCE: NotebookCell[] = [
  {
    ...
    "type": "markdown",
    "source": "# Gold Layer: AI Enrichment for Compliance Analysis (Azure OpenAI)\n\n**Notebook:** `17_gold_ai_functions_compliance`\n**Layer:** Gold (Analytics)\n**Source:** Silver compliance tables, federal report tables (ADLS Gen2 Delta)\n**Target:** `gold_compliance_ai_analysis`\n\n## Overview\nThis notebook performs **sentiment analysis**, **text classification**, **entity\nextraction**, and **language detection / translation** on compliance narratives and\nfederal report text using **Azure OpenAI** chat completions, applied as Spark UDFs\ndirectly over the Silver Delta tables (ADLS Gen2). It does **not** depend on Fabric\nWarehouse T-SQL AI functions or a Fabric capacity — it runs on Synapse Spark /\nDatabricks against ADLS Gen2 Delta with `LOOM_DEFAULT_FABRIC_WORKSPACE` unset.\n\n## Backend\n- **Primary:** Azure OpenAI (`AZURE_OPENAI_ENDPOINT` + `AZURE_OPENAI_DEPLOYMENT`),\n  authenticated with Managed Identity (`DefaultAzureCredential`) or `AZURE_OPENAI_API_KEY`.\n- **Fallback (honest, disclosed):** when no Azure OpenAI endpoint is configured the\n  notebook uses a deterministic lexical heuristic so the pipeline stays runnable in\n  dev / CI. The fallback is clearly labeled in the `_ai_model_version` column.\n\n## Output Tables:\n- **gold_compliance_ai_analysis** - Enriched compliance filings with sentiment, category, entities, and risk score\n\n## AI operations:\n| Operation        | Azure OpenAI prompt task                                  |\n|------------------|-----------------------------------------------------------|\n| `ai_sentiment`   | Score narrative urgency/sentiment in [-1.0, 1.0]          |\n| `ai_classify`    | Pick the best matching filing-type label + confidence     |\n| `ai_extract`     | Pull structured entities into JSON                        |\n| `ai_detect_lang` | Identify ISO-639-1 language code                          |\n| `ai_translate`   | Translate non-English text to English                     |"
  },
  {
  ...
  },
  {
    ...
    "type": "markdown",
    "source": "## AI helpers — Azure OpenAI chat completions (with deterministic fallback)\n\nEach helper issues a single, tightly-scoped Azure OpenAI chat completion. The\nclient is created lazily inside the executor so the bearer token / API key is\nresolved on the worker, and a deterministic heuristic is used when no endpoint\nis configured. These run as Spark UDFs so enrichment is distributed across the\ncluster — the same outcome as a Warehouse AI function, but Azure-native."
  },
  {
  ...
  },
  {
    ...
    "type": "markdown",
    "source": "## Register Spark UDFs\n\nThe helpers are registered as UDFs so the enrichment is distributed across the\nSpark cluster and applied column-wise over the Silver Delta tables."
  },
  {
  ...
  {
    "id": "gold-17-gold-ai-functions-compliance-c17",
    "type": "markdown",
    "source": "---\n## Validation & Summary"
  },
  {
    "id": "gold-17-gold-ai-functions-compliance-c18",
    "type": "code",
