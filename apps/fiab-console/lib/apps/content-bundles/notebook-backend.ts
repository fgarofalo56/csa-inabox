...
export const NOTEBOOK_BACKEND_SHIM: string = [
  ...
  "    if rt in ('notebookutils', 'mssparkutils'):",
  '        # Fail FAST + honest when the vault is unreachable — getSecret otherwise',
  '        # hangs minutes on a raw TCP connect timeout (e.g. placeholder vault name).',
  "        _vault = (vault_url or scope)",
  "        _host = _vault.replace('https://', '').split('/')[0]",
  "        if '.' not in _host: _host = _host + '.vault.azure.net'",
  '        import socket as _s',
  '        try:',
  '            _s.create_connection((_host, 443), timeout=5).close()',
  '        except Exception as _e:',
  "            raise RuntimeError('Key Vault %r is not reachable (%s). Pass the real vault name/URI (notebook parameter key_vault or spark.loom.keyVault), or grant the workspace identity access.' % (_host, _e))",
