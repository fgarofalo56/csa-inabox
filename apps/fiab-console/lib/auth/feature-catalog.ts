...
export const CAPABILITY_CATALOG: Capability[] = [
  ...
  { id: 'admin.parity-autopilot',    domain: 'Admin',     workload: 'Tenant Admin',      name: 'Parity Autopilot', kind: 'admin', description: '/admin/parity-autopilot — WS-10.5: schedule Playwright captures, run an AOAI vision diff against the parity docs, and auto-file gap issues with a proposed plan. Read the run ledger + trigger a run. Admin-only by default; delegable at /admin/permissions.', parentId: 'workload.admin' },
];
