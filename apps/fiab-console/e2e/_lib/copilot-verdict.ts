 ...
 *
 * EVERY ENTRY HERE IS EMITTED BY A ROUTE, and that is now a testable claim
 * rather than a comment: `__tests__/copilot-uat-verdict.test.ts` greps
 * `app/api/**` for each one. The list previously carried
 * `copilot_studio_not_enabled` when NOTHING under `app/api/**` emitted it — the
 * real Copilot Studio gate came back as a CODELESS 503 from
 * `lib/azure/copilot-studio-client.ts`, which the codeless-5xx rule below then
 * correctly scored as a fault. A classifier matching on a string that cannot
 * arrive is not tolerance, it is a rule that silently does not apply.
 */
 ...
 * (app/api/copilot/complete/route.ts), `admin_only` is a role check
 * (app/api/governance/govern/copilot/route.ts), and
 ...
 * As a set in the module it is covered: the test asserts every member fails on
 * a gate, so dropping a persona changes data under test.
 *
 * Each entry names the route it is measured against, because membership is a
 * CLAIM ABOUT A BACKEND and a claim with no referent is how
 * `persona:governance-copilot` sat on the wrong side of this line — excluded as
 * "Power Platform" by a test that asserted the exclusion, over a route whose own
 * header says "Real backend: Azure OpenAI chat-completions via
 * resolveAoaiTarget()" and which emits `code:'no_aoai'` with
 * `missingEnvVar:'LOOM_AOAI_ENDPOINT'`. A `no_aoai` from an AOAI-backed route
 * scoring PASS is the exact shape this rule exists to reject, and a regression
 * test had pinned it.
 */
export const AOAI_BACKED_PERSONAS: ReadonlySet<string> = new Set([
  'persona:notebook-in-cell-copilot',   // app/api/notebook/[id]/assist
  'persona:warehouse-copilot',          // app/api/items/synapse-dedicated-sql-pool/[id]/assist
  'persona:azure-sql-copilot',          // app/api/items/azure-sql-database/[id]/copilot
  'persona:cross-item-copilot',         // app/api/copilot/status + /api/copilot/orchestrate
  'persona:help-copilot',               // app/api/help-copilot/chat
  'persona:notebook-inline-complete',   // app/api/copilot/complete
  'persona:governance-copilot',         // app/api/governance/govern/copilot
]);

/**
 * Personas whose backend Loom does NOT provision, so an honest gate there is a
 * supported outcome.
 *
 * This list exists to make the classification a PARTITION rather than a filter.
 * Every test previously asked "is each member of AOAI_BACKED_PERSONAS driven by
 * the spec?" — set → spec only. Nothing asked the other direction, so a persona
 * the spec drives that belongs in NEITHER list passed silently, and a new
 * AOAI-backed surface could be added to `copilot.uat.ts` and simply never
 * acquire the must-answer rule. Requiring the two sets to COVER the driven
 * population closes that: an unclassified persona fails the test.
 *
 * Power Platform / Dataverse / BAP (Loom provisions no Power Platform
 * environment) and Cosmos-backed galleries both sit here.
 */
export const NON_AOAI_BACKED_PERSONAS: ReadonlySet<string> = new Set([
  'persona:copilot-studio-agent',       // Dataverse + BAP — app/api/items/copilot-studio-agent
  'persona:copilot-studio-channel',     // Dataverse + Azure Bot Service registration
