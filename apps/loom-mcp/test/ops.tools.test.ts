...
test('loom.run.start (WRITE) is denied end-to-end for a read-only token — SDK never called', async () => {
  ...
  const handler = buildToolHandler(tool('loom.run.start'), { server: 'loom-ops', auth: patAuth('read-only'), audit: (e) => events.push(e), authz: { allowMutations: true } });
...
test('loom.run.start (WRITE) succeeds for a read-write token and actually calls POST /run', async () => {
  hits.start = 0;
  const events: AuditEvent[] = [];
  const handler = buildToolHandler(tool('loom.run.start'), { server: 'loom-ops', auth: patAuth('read-write'), audit: (e) => events.push(e), authz: { allowMutations: true } });
...
test('loom.run.cancel (WRITE) is refused for read-only, allowed for read-write', async () => {
  hits.cancel = 0;
  const ro = buildToolHandler(tool('loom.run.cancel'), { server: 'loom-ops', auth: patAuth('read-only'), audit: () => {}, authz: { allowMutations: true } });
  assert.equal((await ro({ id: 'itm-1', runId: '42' })).isError, true);
  assert.equal(hits.cancel, 0, 'a read-only token cannot cancel');

  const rw = buildToolHandler(tool('loom.run.cancel'), { server: 'loom-ops', auth: patAuth('read-write'), audit: () => {}, authz: { allowMutations: true } });
...
test('loom.run.list (read) works for a read-only token and returns real runs', async () => {
  hits.runs = 0;
  const events: AuditEvent[] = [];
  const handler = buildToolHandler(tool('loom.run.list'), { server: 'loom-ops', auth: patAuth('read-only'), audit: (e) => events.push(e), authz: { allowMutations: true } });
  const res = await handler({ id: 'itm-1' });
...
test('loom.run.get SCRUBS secrets in a run’s activity output (data leaves via M4 too)', async () => {
  hits.runs = 0;
  const handler = buildToolHandler(tool('loom.run.get'), { server: 'loom-ops', auth: patAuth('read-only'), audit: () => {}, authz: { allowMutations: true } });
