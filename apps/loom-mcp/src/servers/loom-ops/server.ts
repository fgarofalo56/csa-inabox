 ...
 * the write authorization policy `{ allowMutations:true }` so the core gate will
 * dispatch the two non-`readOnly` tools (`loom.run.start`, `loom.run.cancel`) —
 * but ONLY after they clear the core scope floor (`minScope:'read-write'`), so a
 * read-only PAT is still refused. The policy is the SAME shared `AuthzPolicy`
 * M3 `loom-author` uses — there is no per-server authz fork. The scrub,
 * hashed-args audit, and error normalization are the core's, reused unchanged;
 * the write path is the one new capability M4 exercises.
 */
...
/** M4 authorization policy: mutations allowed; per-tool `read-write` scope floor still applies. */
export const OPS_AUTHZ: AuthzPolicy = { allowMutations: true, server: OPS_SERVER_NAME };
...
export async function createOpsServer(opts: CreateOpsServerOptions = {}): Promise<McpServer> {
  ...
  return createLoomMcpServer({
    ...
    // clearing the per-tool read-write scope floor in the shared core gate.
    authz: OPS_AUTHZ,
  });
}
