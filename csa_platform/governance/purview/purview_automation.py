...
class PurviewAutomation:
    ...
    def load_classification_rules(
        ...
        logger.info("classification_rules.loaded", count=len(rules), path=str(rules_path))
        return rules
    ...
    def apply_classification_rules(
        ...
        *,
        ...
        for rule in rules:
            ...
            if dry_run:
                logger.info("classification_rule.dry_run", rule_name=rule.name)
            ...
            try:
                ...
                logger.info("classification_rule.applied", rule_name=rule.name)
                results.append(
                    {
                        "name": rule.name,
                        "status": "applied",
                ...
                logger.error("classification_rule.failed", rule_name=rule.name, error=str(exc))
                results.append(
                    {
                        "name": rule.name,
                        "status": "error",
    ...
    def load_glossary_terms(
        ...
        logger.info("glossary_terms.loaded", count=len(terms), path=str(glossary_path))
        return terms
    ...
    def import_glossary_terms(
        ...
        *,
        ...
        for term in terms:
            ...
            if dry_run:
                logger.info("glossary_term.dry_run", term_name=term.name)
            ...
            try:
                ...
                logger.info("glossary_term.imported", term_name=term.name)
                results.append({"name": term.name, "status": "imported", "guid": response.get("guid")})
            except Exception as exc:
                logger.error("glossary_term.failed", term_name=term.name, error=str(exc))
    ...
    def _ensure_glossary(self, glossary_name: str) -> str:
        ...
        try:
            ...
            pass  # Glossary endpoint may 404 if none exist yet
        except Exception as e:
            logger.warning("Failed to list glossaries: %s", e)
    ...
    def register_adf_lineage(
        ...
        *,
        ...
        if dry_run:
            logger.info("adf_lineage.dry_run", pipeline_name=pipeline_name)
        ...
        try:
            ...
            logger.info(
                "adf_lineage.registered",
                pipeline_name=pipeline_name,
                sources=len(source_datasets),
                sinks=len(sink_datasets),
            )
            return {"status": "registered", "response": response}
        except Exception as exc:
            logger.error("adf_lineage.failed", pipeline_name=pipeline_name, error=str(exc))
            return {"status": "error", "error": str(exc)}
    ...
    def register_dbt_lineage(
        ...
        *,
        ...
        try:
            ...
            logger.info("dbt_lineage.registered", relationship_count=len(relationships))
            return {
                "status": "registered",
                "relationships": len(relationships),
                "response": response,
            ...
            logger.error("dbt_lineage.failed", error=str(exc))
            return {"status": "error", "error": str(exc)}
    ...
    def apply_sensitivity_labels(
        ...
        *,
        ...
        for policy in policies:
            ...
            # Build the auto-labeling policy payload for the Purview API
            payload = {
                "name": policy_name,
                "properties": {
                    "policyName": policy_name,
                    "sensitivityLabel": target_label,
            ...
            try:
                response = self._make_request(
                    "PUT",
                    f"/scan/autolabelingrules/{policy_name}",
                    body=payload,
                )
                logger.info(
                    "label_policy.applied",
