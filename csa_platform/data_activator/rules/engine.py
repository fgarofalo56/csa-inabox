...
class RuleEngine:
    ...
    def _evaluate_condition(
        ...
        return self._compare(condition, value, rule_name=rule.name), value

    def _compare(self, condition: Condition, value: float, rule_name: str = "") -> bool:
        ...
        if op == ConditionOperator.ANOMALY:
            return self._detect_anomaly(condition, value, rule_name)

        return False

    def _detect_anomaly(self, condition: Condition, current_value: float, rule_name: str = "") -> bool:
        ...
        Args:
            ...
            rule_name: Name of the rule (used for buffer key lookup).

        Returns:
            ``True`` if the value is anomalous (z-score exceeds threshold).
        """
        # We need at least 7 data points for meaningful z-score
        buffer_key = (rule_name, condition.field)
        if buffer_key not in self._window_buffer:
            return False
