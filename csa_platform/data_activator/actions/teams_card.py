...
DEFAULT_DOMAIN_RECOMMENDATIONS: dict[str, list[str]] = {
    "epa": [
        "Issue public health advisory for affected area",
        "Check sensor calibration and data quality",
        "Review historical AQI trend for context",
...
_DEFAULT_FALLBACK = ["Investigate the alert and take appropriate action"]


def get_recommended_actions(
    rule_name: str,
    domain: str = "",
    custom_recommendations: dict[str, list[str]] | None = None,
    custom_domain_recommendations: dict[str, list[str]] | None = None,
) -> list[str]:
    """Get recommended actions based on the alert type.

    Args:
        rule_name: Name of the alert rule.
        domain: Domain tag (e.g. ``"epa"``, ``"nps"``).
        custom_recommendations: Override keyword-to-actions mapping.
        custom_domain_recommendations: Override domain-to-actions mapping.

    Returns:
        List of recommended action strings.
    """
    recommendations = custom_recommendations or DEFAULT_RECOMMENDATIONS
    domain_recs = custom_domain_recommendations or DEFAULT_DOMAIN_RECOMMENDATIONS

    rule_lower = rule_name.lower()
    for keyword, actions in recommendations.items():
        if keyword in rule_lower:
            return actions

    domain_lower = domain.lower()
    if domain_lower in domain_recs:
        return domain_recs[domain_lower]
...
def _get_recommended_actions(payload: AlertPayload) -> list[str]:
    ...
    return get_recommended_actions(payload.rule_name, domain=domain)
