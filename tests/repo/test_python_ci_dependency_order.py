...
# ---------------------------------------------------------------------------
# The compiled locks must stay OUT of the loop, and the exclusion must be doing
# real work (#3485). This is the anti-vacuity test above pointed the other way:
# there, the loop must not be empty; here, it must not have grown to include
# 495 exact pins that would decide the stack under test.
# ---------------------------------------------------------------------------


def test_the_compiled_locks_are_excluded_from_the_install_loop() -> None:
    find_line = next(
        line
    ...
    found = _find_requirements(find_line)
    leaked = [p for p in found if "requirements/locks/" in p.as_posix()]
    assert not leaked, (
        "the compiled locks are in the per-domain install loop. They are 495 exact "
        "pins — including fastapi and starlette — installed into the ONE shared test "
        "environment, which is #2615 verbatim. Add "
        '`-not -path "./requirements/locks/*"` to the find. Leaked: '
        + ", ".join(p.as_posix() for p in leaked)
    )

    # The exclusion must be excluding something that EXISTS, or this passes for
    # the wrong reason the day the locks move again.
    locks = sorted((REPO_ROOT / "requirements" / "locks").glob("*/requirements.txt"))
    assert len(locks) >= 9, (
        f"expected the compiled locks under requirements/locks/, found {len(locks)}; "
        "if the layout moved, move the exclusion with it"
    )
    # And they must be the kind of file that WOULD have decided the stack.
    pinned = {
        name
        for lock in locks
        for name in parse_pins(lock.read_text(encoding="utf-8"))
    }
    assert {"fastapi", "starlette"} <= pinned, (
        "the locks no longer pin fastapi/starlette, so this test would pass even "
        "without the exclusion; re-derive the hazard before weakening it"
    )
...
class TestPinParsing:
    ...
    def test_a_hash_pinned_lock_line_is_seen(self) -> None:
        """A ``--generate-hashes`` pin ends in a continuation; it is still a pin.

        THE BLIND SPOT (#3485). ``_PIN`` is anchored with ``$`` and every pin
        line in a compiled lock ends in ``` \\``` because its hashes follow on
        continuation lines. Measured over the ten compiled locks: 495 pin lines,
        ``parse_pins`` returned 0 — so this whole module's static model read an
        EMPTY file while ``pip install -r`` applied all 495 at runtime. The guard
        reported green over exactly the hazard it exists for.

        Both directions are asserted, because the previous behaviour would still
        pass a test that only checked the non-continuation shape.
        """
        lock = (
            "#\n"
            ...
            "    --hash=sha256:0badc0de\n"
        )
        assert parse_pins(lock) == {"cryptography": "50.0.0", "uvicorn": "0.51.0"}
        # A hash line is an option line and must never be read as a pin.
        assert parse_pins("    --hash=sha256:deadbeef \\\n") == {}
