...
def encode_statement_query(sql: str) -> bytes:
    ...
    return bytes(pbcodec.pack_any(pbcodec.TYPE_STATEMENT_QUERY, inner))


class TestProtobufCodec:
    def test_varint_roundtrip_across_byte_boundaries(self) -> None:
    ...
    def test_statement_query_roundtrip(self) -> None:
    ...
    def test_unicode_and_long_queries_survive(self) -> None:
    ...
    def test_ticket_statement_query_roundtrip(self) -> None:
    ...
    def test_unknown_fields_are_skipped_for_forward_compatibility(self) -> None:
    ...
    def test_bare_sql_payload_is_reported_as_unwrapped_not_guessed(self) -> None:
...
# ── ticket verification ─────────────────────────────────────────────────────
def mint(payload: dict[str, Any], secret: str | None) -> str:
...
class TestTicketVerification:
    def test_valid_signed_ticket_yields_the_entra_principal(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_bearer_prefix_is_accepted(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("LOOM_FLIGHT_TICKET_SECRET", "s3cret")
        assert tickets.verify_ticket("Bearer " + mint(valid_payload(), "s3cret")).oid == "oid-1"

    def test_tampered_payload_fails_signature(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("LOOM_FLIGHT_TICKET_SECRET", "s3cret")
    ...
    def test_expired_ticket_is_refused(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("LOOM_FLIGHT_TICKET_SECRET", "s3cret")
        with pytest.raises(tickets.TicketInvalidError) as err:
            tickets.verify_ticket(mint(valid_payload(exp=int(time.time()) - 1), "s3cret"))
        assert "expired" in str(err.value)

    def test_wrong_audience_is_refused(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("LOOM_FLIGHT_TICKET_SECRET", "s3cret")
        with pytest.raises(tickets.TicketInvalidError) as err:
            tickets.verify_ticket(mint(valid_payload(aud="something-else"), "s3cret"))
        assert "audience" in str(err.value)

    def test_missing_ticket_names_where_to_get_one(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("LOOM_FLIGHT_TICKET_SECRET", "s3cret")
        with pytest.raises(tickets.TicketInvalidError) as err:
            tickets.verify_ticket("")
        assert "Connect tab" in str(err.value)

    def test_unsigned_deployment_reports_unverified_rather_than_trusting_silently(self, monkeypatch: pytest.MonkeyPatch) -> None:
