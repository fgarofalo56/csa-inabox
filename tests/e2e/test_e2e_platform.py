...
_GOLD_MODELS = {p.stem for p in (_DBT_MODELS_DIR / "gold").glob("*.sql")}
...
class TestBicepParams:
    ...
    def test_params_files_have_parameters_key(self) -> None:
        for pf in self._find_param_files():
            with open(pf) as fh:
                data = json.load(fh)
            assert "parameters" in data, f"{pf.relative_to(_REPO_ROOT)} missing 'parameters' key"
...
class TestSilverTransforms:
    ...
    def test_silver_customers_has_surrogate_key(self, duckdb_conn: Any) -> None:
        rows = duckdb_conn.execute("SELECT customer_sk FROM silver.customers WHERE customer_sk IS NULL").fetchall()
    ...
    def test_silver_customers_email_lowered(self, duckdb_conn: Any) -> None:
        rows = duckdb_conn.execute("SELECT email FROM silver.customers WHERE email != lower(email)").fetchall()
    ...
    def test_silver_orders_has_surrogate_key(self, duckdb_conn: Any) -> None:
        rows = duckdb_conn.execute("SELECT order_sk FROM silver.orders WHERE order_sk IS NULL").fetchall()
    ...
    def test_silver_orders_status_uppercased(self, duckdb_conn: Any) -> None:
        rows = duckdb_conn.execute("SELECT status FROM silver.orders WHERE status != upper(status)").fetchall()
...
class TestGoldDataQuality:
    ...
    def test_dim_customers_unique_ids(self, duckdb_conn: Any) -> None:
        ...
        unique = duckdb_conn.execute("SELECT COUNT(DISTINCT customer_id) FROM gold.dim_customers").fetchone()[0]
    ...
    def test_dim_products_unique_ids(self, duckdb_conn: Any) -> None:
        ...
        unique = duckdb_conn.execute("SELECT COUNT(DISTINCT product_id) FROM gold.dim_products").fetchone()[0]
    ...
    def test_fact_orders_no_negative_amounts(self, duckdb_conn: Any) -> None:
        negatives = duckdb_conn.execute("SELECT COUNT(*) FROM gold.fact_orders WHERE total_amount < 0").fetchone()[0]
    ...
    def test_fact_orders_valid_dates(self, duckdb_conn: Any) -> None:
        bad_dates = duckdb_conn.execute("SELECT COUNT(*) FROM gold.fact_orders WHERE order_date IS NULL").fetchone()[0]
    ...
    def test_monthly_revenue_year_reasonable(self, duckdb_conn: Any) -> None:
        bad = duckdb_conn.execute(
            "SELECT COUNT(*) FROM gold.gld_monthly_revenue WHERE revenue_year < 2020 OR revenue_year > 2030"
        ).fetchone()[0]
    ...
    def test_clv_no_negative_revenue(self, duckdb_conn: Any) -> None:
        negatives = duckdb_conn.execute(
            "SELECT COUNT(*) FROM gold.gld_customer_lifetime_value WHERE lifetime_revenue < 0"
        ).fetchone()[0]
...
class TestContractSchemaAlignment:
    ...
    def test_gold_schema_has_contract_columns(
        ...
        gold_table_columns = {tbl: self._get_duckdb_columns(duckdb_conn, tbl) for tbl in _GOLD_TABLES}
...
class TestNoOrphanModels:
    ...
    def test_all_gold_models_declared_in_schema_yml(self) -> None:
        ...
        assert not orphans, f"Gold SQL models without schema.yml entries: {orphans}"
    ...
    def test_all_schema_yml_models_have_sql_files(self) -> None:
        ...
        assert not missing_sql, f"schema.yml references models with no SQL file: {missing_sql}"
    ...
    def test_all_domains_with_gold_have_contracts(self) -> None:
        ...
        for contract in _REPO_ROOT.joinpath("domains").glob("*/data-products/**/contract.yaml"):
            # domains/<domain>/data-products/...
            domain = contract.relative_to(_REPO_ROOT / "domains").parts[0]
            domains_with_contracts.add(domain)

        missing = domains_with_gold - domains_with_contracts
        assert not missing, f"Domains with data-products/ but no contract.yaml: {missing}"
