...
export const CI_PLUMBING = {
  ...
  'scripts/ci/deploy-classify.mjs': 'The failure taxonomy consumer. It runs ONLY on an already-failed step, reads stderr and emits a class + an exit code so the caller can branch. It submits nothing to ARM and produces no artifact, so editing it cannot change what a successful run deployed — only how a failed one is described. Same reasoning as deploy-retry.mjs above, and the same boundary: it is plumbing because `unknown` is not a pass.',
