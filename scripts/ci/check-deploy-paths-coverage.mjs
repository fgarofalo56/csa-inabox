...
export const CI_PLUMBING = {
  ...
  'scripts/ci/deploy-classify.mjs': 'The failure taxonomy consumer. It runs ONLY on an already-failed step, reads stderr and emits a class + an exit code so the caller can branch. It submits nothing to ARM and produces no artifact, so editing it cannot change what a successful run deployed — only how a failed one is described. THE BOUNDARY, stated precisely because the first draft of this loan stated it WRONGLY: the hazard is NOT that `unknown` might become a pass. It is `--assert-signal config.resource-group-not-found`, which IS a pass — deploy-fiab-commercial.yml:1322, deploy-fiab-gcch.yml:625 and deploy-fiab-il5.yml:370 each `exit 0` on it and SKIP the image-tag preflight, and assert-no-silent-image-tag-revert.mjs:311 and adopt-image-tags.mjs:283 both return `{greenfield:true}` on it. A classifier that emitted that signal for a failure which was really an auth denial would turn "I could not read the estate" into "there is no estate", skipping a gate on a LIVE estate. WHAT HOLDS THAT SHUT, corrected in #3899 because the first statement of it credited the wrong mechanism: it is the `config.resource-group-not-found` signal\'s `not[]`, which carries `authorizationfailed` and REFUSES the match before class precedence is ever consulted. `classPrecedence` (["defect","permission","quota","config",…], so `permission` at index 1 outranks `config` at index 3) is a second, weaker line — measured during review of #3889, reordering it to config > permission left the auth-denial verdict UNCHANGED and every suite green, so it was never the load-bearing part. THIS LOAN\'S CONDITION IS EXECUTABLE, NOT PROSE: `scripts/ci/__tests__/deploy-classify.test.mjs` asserts the outcome (an auth denial classifies `permission.authorization-failed`, never the greenfield signal), the `not[]` entry, and the precedence order — each mutation-proved to go red. If any of the three is deliberately changed, delete this loan and watch this file.',
