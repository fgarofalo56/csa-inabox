...
const ALLOWLIST = {
  ...
  "apps/fiab-console/lib/azure/cosmos-client.ts": { max: 1832, reason: "the single exhaustive Cosmos container registry — one container decl + ensure() + accessor per feature (WS-4.2 function-registry; WS-5.2 a2a-tasks; WS-9 agent-registry; WS-10.1 autopilot; WS-10.3 time-branches; WS-10.4 marketplace; WS-10.5 parity-autopilot-runs; C3 loom-cost-anomaly-rules; N9 loom-semantic-contract; N10 loom-answer-receipts; N13 loom-prompt-registry + loom-token-budgets; N5 loom-assets); ratchet-frozen, decomposition tracked in decomposition-plan.md (extend-then-decompose: this registry grows ~4 LOC per new container — N13's two containers took it 1698→1702; N1's loom-lakehouse-interop took it 1720→1740; N5's loom-assets took it 1745→1758) 4b-batch2 adds N5 loom-assets + N6's contracts container in ONE wave (each agent bumped independently to 1758/1760; the COMBINED file is 1784, so the ceiling is 1790). N17 adds loom-monitors + loom-incidents (the observability incident console's two containers) in ONE wave: +4 LOC each → 1790→1798. M2 adds loom-migration-copy-jobs (the copy-in monitor's single-partition job store, MIG1): +12 LOC → 1798→1810. §P2 wave adds A14's collab-stream accessor growth (+6 combined across the wave's 9 parallel agents; real combined file 1816) → 1810→1820. LU-5 adds `uc-governance` (the Loom Unity governance-overlay row store, PK /tenantId): decl + ensure() + accessor + KNOWN_CONTAINER_IDS = +4 LOC → 1820→1832 (the 1830 the first LU-5 pass reached was held there by jamming the accessor and the NEXT function's JSDoc onto one physical line; that concatenation is undone here and the ceiling bumped instead — the sanctioned path)." },
