...
const ALLOWLIST = {
  ...
  "apps/fiab-console/lib/azure/cosmos-client.ts": { max: 1745, reason: "the single exhaustive Cosmos container registry — one container decl + ensure() + accessor per feature (WS-4.2 function-registry; WS-5.2 a2a-tasks; WS-9 agent-registry; WS-10.1 autopilot; WS-10.3 time-branches; WS-10.4 marketplace; WS-10.5 parity-autopilot-runs; C3 loom-cost-anomaly-rules; N9 loom-semantic-contract; N10 loom-answer-receipts; N13 loom-prompt-registry + loom-token-budgets); ratchet-frozen, decomposition tracked in decomposition-plan.md (extend-then-decompose: this registry grows ~4 LOC per new container — N13's two containers took it 1698→1702; N1's loom-lakehouse-interop took it 1720→1740)" },
