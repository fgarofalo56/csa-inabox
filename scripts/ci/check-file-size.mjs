...
const ALLOWLIST = {
  ...
  "apps/fiab-console/lib/editors/report-designer.tsx": { max: 1400, reason: "WS-11.1 decomposed (5135→1280 LOC); ratchet-frozen at 1400 post-decomp" },
  "apps/fiab-console/lib/editors/report-designer/use-report-mutations.tsx": { max: 1400, reason: "useReportMutations hook — all IO + mutation + ribbon callbacks extracted from report-designer (WS-11.1); ratchet-frozen" },
  "apps/fiab-console/lib/editors/phase3/semantic-model-editor.tsx": { max: 3050, reason: "WS-E1 decomposed (4617→3018 LOC); 9 sibling modules extracted (types/constants/styles/helpers/aas-panel/security-tab/copilot-pane/prep-for-ai-pane/loom-native-model-view). Residual is the single SemanticModelEditorInner — a further split needs prop-threading (behavior change), so ratchet-frozen at 3050." },
  "apps/fiab-console/lib/editors/notebook-editor.tsx": { max: 3550, reason: "WS-E1 decomposed (3875→3515 LOC); sibling modules extracted (styles/types + helpers). Ratchet-frozen at 3550." },
  ...
  "apps/fiab-console/lib/editors/apim-editors/data-product-editor.tsx": { max: 1650, reason: "WS-E1 sibling — the cohesive DataProductEditor extracted verbatim from apim-editors.tsx; pure-move (not split further to preserve behavior), like report-designer's large siblings. Ratchet-frozen at 1650." },
  ...
  "apps/fiab-console/lib/azure/cosmos-client.ts": { max: 1600, reason: "the single exhaustive Cosmos container registry — one container decl + ensure() + accessor per feature (WS-4.2 function-registry; WS-5.2 a2a-tasks; WS-9 agent-registry; WS-10.1 autopilot; WS-10.3 time-branches; WS-10.4 marketplace; WS-10.5 parity-autopilot-runs); ratchet-frozen, decomposition tracked in decomposition-plan.md" },
