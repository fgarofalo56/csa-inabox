...
const ALLOWLIST = {
  ...
  "apps/fiab-console/lib/azure/databricks-client.ts": { max: 2706, reason: "pre-existing large module — ratchet-frozen. +6 LOC for GHSA-v2g8-gp3r-rg4r: a `tasks` field on JobRun and an `expandTasks` option on listJobRuns. `items/databricks-notebook/[id]/runs` returned recent runs — and, via getRunOutput, notebook CELL OUTPUT — across the entire shared Databricks workspace; the only coordinate that attributes a run to a notebook is tasks[].notebook_task.notebook_path, which `runs/list` omits unless expand_tasks is requested. Both changes sit on the client that owns the URL, so they cannot live elsewhere; the doc comments were already compressed to two lines each rather than contorting the code to beat the counter. Decomposition of this module should ratchet the ceiling DOWN, not up again." },
