...
export const ALLOWLIST = {
  ...
  "apps/fiab-console/lib/azure/foundry-client.ts": { max: 2700, reason: "pre-existing large module — ratchet-frozen; +WS-2.2 AI Search deleteDocuments + semantic-rerank config for Delta-synced vector search. Re-baselined 2600→2700 for #4432: shieldPrompt/moderateContent documented 'fail-open on a transient error' but implemented it only for a non-2xx response, so a THROWN fetch escaped and became the bare Next.js 500 the chat pane showed as 'Error: HTTP 500' — measured live, the Content Safety host does not resolve from the console. The try/catch that closes that hole, plus contentSafetyHealth() (which replaces a bare env read that CLAIMED prompts were filtered while nothing was screened — R7), cost ~+100 LOC, roughly half of it the measurement record that keeps the fail-open from being re-silenced. The right decomposition is lifting the whole content-safety block to a sibling lib/azure/content-safety.ts; that is a ~7-import-site move which does not belong inside a P0 chat-outage fix, and is tracked with the ceiling drop in #4445." },
  ...
  "apps/fiab-console/lib/azure/foundry-cs-client.ts": { max: 1700, reason: "pre-existing large module — ratchet-frozen. Re-baselined 1600→1700 for #4432: every ARM list call read body.value and dropped nextLink, and Accounts_List is RBAC-filtered PER PAGE — measured live, page 1 came back HTTP 200 value:[] with a nextLink while the subscription held three Cognitive Services accounts, so the AI-model pickers rendered empty with no error because nothing had failed. armListAll() (bounded nextLink following) plus listAccountsDetailed() (which surfaces the per-subscription failures listAccounts had been swallowing into that same indistinguishable empty list) cost ~+80 LOC. Decomposition — splitting the account/deployment/quota/keys families into siblings — is tracked with the ceiling drop in #4445." },
