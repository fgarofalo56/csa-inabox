...
const ORPHAN_ALLOWLIST = new Map([
  ...
  ['platform/fiab/bicep/modules/data-plane/duckdb-aca.bicep', 'standalone entrypoint (N2b/N3 DuckDB serving tier + Arrow Flight SQL wire): internal-ingress Container App running an embedded OSS DuckDB (azure/httpfs/delta/iceberg extensions baked into the image) that reads Delta/Iceberg/Parquet IN PLACE on the customer-owned ADLS Gen2 — the interactive fast path BELOW Spark — plus the Flight SQL gRPC port for ADBC/JDBC clients. Identity-based lake access (UAMI + in-module Storage Blob Data READER role assignment; no keys, no secrets, and the engine refuses every write verb). Deployed out-of-band (admin-plane/main.bicep at the 256-param ceiling), then LOOM_DUCKDB_URL (+ optionally LOOM_FLIGHTSQL_URL) is set on the console app; unset => SQL Lab executes the identical statement on Synapse Serverless and the Connect tab still streams Arrow over the audited HTTP tier, so nothing is blocked. Azure-native/OSS, no Fabric/OneLake/Power BI, nothing SaaS in the query path (runs disconnected in IL5).'],
  ['platform/fiab/bicep/modules/data-plane/loom-trino-aks.bicep', 'standalone entrypoint (N7e Trino Federated SQL — THE single opt-in carve-out of the Openness program): PRIVATE AKS cluster (in-VNet, private API server) running Trino OSS (Apache-2.0) registered against the N1 Iceberg REST Catalog + external connectors, so ONE statement can join a Loom Iceberg table with an external Postgres table. Identity-based lake access via AKS Workload Identity (in-module UAMI federated to the trino k8s ServiceAccount + Storage Blob Data Reader on the DLZ lake; no keys, no secrets). OPT-IN by design (heavy AKS infra = real disclosed cost) — this is the documented exception to loom_default_on_opt_out; it gates NO feature because DuckDB (N2b) is the default SQL Lab engine. Deployed out-of-band (admin-plane/main.bicep at the 256-param ceiling), the Trino Helm chart installed as a separate phase (like the loom-duckdb image build), then LOOM_TRINO_URL is set on the console app; unset => the "Federated SQL (Trino)" engine option honest-gates with a Fix-it that discloses the AKS cost and SQL Lab keeps working on DuckDB. Azure-native/OSS, no Fabric/OneLake/Power BI, no SaaS query federation (runs disconnected in IL5; SaaS-only external connectors stay honestly gated).'],
  ['platform/fiab/bicep/modules/data-plane/loom-migrate-aca.bicep', 'standalone entrypoint (M1 estate-assessment reader / inbound-migration on-ramp): internal-ingress FastAPI Container App the Console BFF (/api/migrate/assess) calls to ENUMERATE a source estate (Snowflake / Databricks Unity Catalog / Microsoft Fabric / Power BI) into a canonical inventory the console assessment engine maps to Loom item types. The reader holds NO standing source credentials and NO storage keys/secrets in app settings — each request carries the source connection the BFF resolved from Key Vault; its UAMI is used ONLY for ACR pull (it never touches the lake, so no data-plane role). Deployed out-of-band (admin-plane/main.bicep at the 256-param ceiling), then LOOM_MIGRATE_URL is set on the console app; unset => /admin/migrate renders (guided empty state) and the assess route honest-gates with a Fix-it. A Fabric/Power BI estate is only ever a migration SOURCE (Loom needs no Fabric); with no source connection the reader runs disconnected in IL5.'],
  ['platform/fiab/bicep/modules/data-plane/loom-risingwave-aca.bicep', 'standalone entrypoint (N7a streaming-SQL tier / Openness T2-A): internal-ingress single-node RisingWave (Apache-2.0) Container App that authors streaming MATERIALIZED VIEWS in SQL over Azure Event Hubs (Kafka endpoint) and sinks the maintained results to Delta/Iceberg on the customer-owned ADLS Gen2 (the N1 lake) or serves them over the Postgres wire — the STATEFUL tier above Azure Stream Analytics (ASA stays the light default). TCP ingress on the Postgres-wire frontend port; identity-based lake WRITE (UAMI + in-module Storage Blob Data Contributor role assignment; no keys, no secrets — source/sink credentials are injected per-DDL from Key Vault by the BFF). Deployed out-of-band (admin-plane/main.bicep at the 256-param ceiling), then LOOM_RISINGWAVE_URL is set on the console app; unset => the streaming-sql editor renders fully (guided empty state) and /api/streaming-sql/* honest-gates with a Fix-it. Opt-in stateful-streaming tier (~$150-300/mo/cloud); the streaming-sql item type itself is default-ON. Azure-native/OSS, no Fabric/OneLake/Power BI, no SaaS streaming service (runs disconnected in IL5).'],
  ['platform/fiab/bicep/modules/data-plane/loom-unity-postgres.bicep', 'standalone entrypoint (LU-1 Loom Unity metastore persistence): Azure Database for PostgreSQL Flexible Server that backs the Loom Unity (Unity-Catalog-compatible) catalog. Entra-ONLY (authConfig.passwordAuth=Disabled - there is no database credential to rotate or leak), publicNetworkAccess=Disabled, reachable only through an in-module private endpoint + privatelink.postgres.<sovereign-suffix> DNS zone, PITR backups, diagnostics to the Loom LAW. Replaces the previous H2-file-DB-on-Azure-Files default, which CrashLoopBackOffs on Azure Government SMB, has no backup, and is single-writer (which pinned the catalog Container App to exactly ONE replica). Deployed out-of-band (admin-plane/main.bicep at the 256-param ceiling), then its fqdn/databaseName/aadUser outputs are passed to compute/loom-unity-app.bicep and the data-plane grants are applied by scripts/csa-loom/loom-unity-postgres-bootstrap.sh. Azure-native, no Fabric/Power BI dependency. See docs/fiab/unity-gov.md.'],
  ['platform/fiab/bicep/modules/compute/loom-unity-app.bicep', 'standalone entrypoint (GOV-PARITY): self-hosted OSS Unity Catalog server ACA app ("Loom Unity") for Azure Government (Databricks Unity Catalog has no Gov endpoint). Internal-ingress + LU-2 Entra bearer authorization ON by default (issuer/audience pinned; Key Vault secretrefs for the Entra + ADLS-vending client secrets; optional consoleAllowedCidrs IP pin). LU-1 persistence: an Entra-only, private-endpoint-only PostgreSQL Flexible Server (data-plane/loom-unity-postgres.bicep) is the DEFAULT store and lifts the old single-replica cap; the legacy single-writer H2-on-Azure-Files store remains only as a documented fallback. Deployed out-of-band (admin-plane/main.bicep at the 256-param ceiling), then LOOM_UC_BACKEND=oss + LOOM_UNITY_URL + LOOM_UNITY_CLIENT_ID set on the console app; the UC client (lib/azure/uc-backend.ts) honest-gates (OssUcNotConfiguredError) until wired and Commercial keeps using Databricks UC. Azure-native, no Fabric/Power BI dependency. See docs/fiab/unity-gov.md + docs/fiab/security/loom-unity-threat-model.md.'],
