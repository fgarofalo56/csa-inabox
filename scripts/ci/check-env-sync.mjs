...
const ALLOWLIST = new Set([
  ...
  'LOOM_MIGRATE_URL',                // M1 opt-in estate-assessment reader URL (internal-ingress loom-migrate Container App: enumerates a Snowflake/Databricks-UC/Fabric/Power-BI source estate for the /admin/migrate readiness report). Deployed out-of-band via data-plane/loom-migrate-aca.bicep (admin-plane/main.bicep at the 256-param ceiling), then set on the console app. Unset => /admin/migrate renders (guided empty state) and the assess route honest-gates with a Fix-it. A Fabric/Power-BI estate is only ever a migration SOURCE (Loom needs no Fabric).
  ...
  'LOOM_DUCKLAKE_CATALOG_URL',       // N8 lab 1 (Preview) opt-in Postgres connection backing the DuckLake catalog metadata; the N2 DuckDB tier ATTACHes it. Unset => the DuckLake catalog editor honest-gates with a Fix-it and the N1 Iceberg REST Catalog is unaffected. Operator-provided in-VNet Azure Database for PostgreSQL; not bicep-emitted.
  'LOOM_S3_GATEWAY_URL',             // N8 lab 3 (Preview) opt-in internal-ingress endpoint of an operator-deployed Apache-2.0 s3proxy in front of ADLS. Unset => the surface documents the N1 IRC + ADLS SDK path (no gateway needed) and honest-gates the connect panel. Deliberately not bicep-emitted (the default path needs no gateway; the AGPL MinIO path is not used).
  'LOOM_FLIGHTSQL_URL',              // N3 opt-in Arrow Flight SQL gRPC endpoint on the same loom-duckdb app (additionalPortMappings). Unset => the Connect tab still renders and Loom keeps streaming the same Arrow batches over the audited HTTP tier past the Arrow threshold; only the external ADBC/JDBC hop is absent. (LOOM_FLIGHT_TICKET_SECRET auto-allowed by /_SECRET$/.)
  ...
  'LOOM_UNITY_CLIENT_ID',           // LU-2 console half of Loom Unity authorization: the Entra app registration fronting the catalog. Set on the console app alongside LOOM_UNITY_URL (same out-of-band path — loom-unity-app.bicep emits the SERVER half as entraClientId). Unset => the svc-loom-unity-authz gate + the probe-loom-unity-authz live probe report the unauthenticated posture with the exact remediation.
  ...
  'LOOM_SHARING_AUDIENCE',          // LU-9 explicit Entra AUDIENCE that an external Delta Sharing RECIPIENT's token must carry when the estate registered a dedicated app registration for the sharing API. Unset (the norm) => the Console's own app registration (LOOM_MSAL_CLIENT_ID, already bicep-emitted) is the pinned audience, so this is a runtime-only override, never a deploy dependency. The recipient endpoint fails CLOSED with 503 when NO audience can be pinned.
