...
const ALLOWLIST = new Set([
  ...
  // (LOOM_DUCKLAKE_CATALOG_URL and LOOM_S3_GATEWAY_URL used to be allowlisted here as
  //  "operator-provided / not bicep-emitted". Both are now DEFAULT-ON and emitted by
  //  admin-plane/main.bicep — LOOM_DUCKLAKE_CATALOG_URL as a Key Vault secretRef backed by
  //  data-plane/ducklake-catalog-postgres.bicep, LOOM_S3_GATEWAY_URL from the
  //  data-plane/s3-gateway-aca.bicep internal endpoint. The allowlist entries were removed
  //  DELIBERATELY so this guard fails if either wiring is ever regressed away.)
