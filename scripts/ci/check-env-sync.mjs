...
const ALLOWLIST = new Set([
  ...
  'LOOM_S3_GATEWAY_URL',             // N8 lab 3 (Preview) opt-in internal-ingress endpoint of an operator-deployed Apache-2.0 s3proxy in front of ADLS. Unset => the surface documents the N1 IRC + ADLS SDK path (no gateway needed) and honest-gates the connect panel. STILL not bicep-emitted: PR #2640 round 4 pulled the default-ON s3proxy module back out because the only shipped lane that could deploy it (dlz-attach) resolved the image to an anonymous docker.io pull — the hub-ACR coordinate never reaches main.bicep's `hubCoordinates` from ANY dlz-attach parameter producer — and because that lane passed keyVaultId='' so the Key Vault secrets the docs told operators to read were never created. Re-enabling it (image mirrored into every cloud's ACR + the ACR/Key-Vault coordinates threaded end to end) removes this entry again.
