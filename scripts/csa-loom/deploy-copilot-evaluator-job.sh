...
LOOM_COPILOT_EVAL_JUDGE_DAILY_CAP="${LOOM_COPILOT_EVAL_JUDGE_DAILY_CAP:-500}"
...
# Build context = REPO ROOT (see the Dockerfile header for why).
...
if az containerapp job show -n loom-copilot-evaluator -g "$ADMIN_RG" --subscription "$SUB" >/dev/null 2>&1; then
  ...
  az containerapp job create -n loom-copilot-evaluator -g "$ADMIN_RG" --subscription "$SUB" \
    --environment "$CAEID" \
