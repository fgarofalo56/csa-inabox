...
grant_role_if_absent() {
  ...
  if [ "$rc" -ne 0 ] || [ "$existing" != "0" ]; then
    ...
    echo "::warning::${label}: AcrPull-class grant is ABSENT, so it is being created here. That mints a CLI name, not the template's; scripts/csa-loom/converge-role-assignment.mjs converges it on the next infra deploy (#3439)."
  fi
  rm -f "$errfile"
